...
namespace CodeBase {
	public class ODImaging {
		...
		///<summary>Rotates the passed in image based on Windows orientation.</summary>
		...
		public static Bitmap ImageScaleMaxHeightAndWidth(Image image,long maxHeight,long maxWidth) {
			...
			if(image.PhysicalDimension.Height>maxHeight || image.PhysicalDimension.Width>maxWidth) {//image is too large
				//Image is larger than given constraints, resize to fit.
				if(image.PhysicalDimension.Width/maxWidth>image.PhysicalDimension.Height/maxHeight) {//resize image based on width
