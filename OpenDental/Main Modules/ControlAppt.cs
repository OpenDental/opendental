...
namespace OpenDental {
	...
	public partial class ControlAppt:UserControl {
		...
		private void toolBarLists_Click() {
			using FormApptLists formApptLists=new FormApptLists();
			formApptLists.ShowDialog();
			if(formApptLists.DialogResult==DialogResult.Cancel) {
				return;
			}
			switch(formApptLists.ApptListSelectionResult) {
		...
		private bool DoApptBreakRequired(Appointment appointment,Patient patient=null) {
			if(PrefC.GetBool(PrefName.BrokenApptRequiredOnMove) && appointment.AptStatus==ApptStatus.Scheduled) {
				using FormApptBreakRequired formApptBreakRequiredForce=new FormApptBreakRequired();
				formApptBreakRequiredForce.ShowDialog();
				if(formApptBreakRequiredForce.DialogResult!=DialogResult.OK) {
					return false;
				}
				if(patient==null) {
					patient=Patients.GetPat(appointment.PatNum);
				}
				AppointmentL.BreakApptHelper(appointment,patient,formApptBreakRequiredForce.ProcedureCodeBrokenSelected);
			}
			return true;
		}
