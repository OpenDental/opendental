...
namespace OpenDental {
	...
	public partial class ControlAppt:UserControl {
		...
		private void menuApt_Click(object sender,System.EventArgs e) {
			switch(((ToolStripMenuItem)sender).Name) {
				...
				case MenuItemNames.SendConfirmationText:
					...
					Appointment aptOld=appt.Copy();
					Patient pat=Patients.GetPat(appt.PatNum);
					string message=PatComm.BuildConfirmMessage(ContactMethod.TextMessage,pat,appt.AptDateTime,appt.DateTimeAskedToArrive);
		...
		public void LayoutControls(){
			//Calendar2 size is automatic based on region, and other sizes are dependent on it. Example, Chinese calendar.
			Calendar2.PerformLayout();
			LayoutManager.MoveLocation(Calendar2,new Point(0,labelDate.Bottom+LayoutManager.Scale(3)));
			int widthMinimum=LayoutManager.Scale(219);
			if(Calendar2.Width<widthMinimum){
				LayoutManager.MoveWidth(panelCalendar,widthMinimum);//probably will never happen
			}
			else{
				LayoutManager.MoveWidth(panelCalendar,Calendar2.Width);
			}
			LayoutManager.MoveLocation(panelCalendarLower,new Point(0,Calendar2.Bottom+1));
