...
namespace OpenDental {
	...
	public partial class ControlAppt:UserControl {
		...
		private void DisplayFormAsapForWebSched(Appointment appt=null) {
			...
			long apptNum=0;
			if(appt!=null) {
				//If the current time is less than 20 minutes before the appts start time or anytime after the appt start time, do not prompt to fill for ASAP list.  This is hard coded for now but will be adjusted to a user specified time limit as a preference in a later enhancement.
				TimeSpan timeSpanApptDifference=appt.AptDateTime-DateTime.Now;
				if(timeSpanApptDifference<=TimeSpan.FromMinutes(20)) {
					return;
				}
			...
			_formASAP.ShowFormForWebSched(dateTimeSelected,dateRange.Start,dateRange.End,opNum);
