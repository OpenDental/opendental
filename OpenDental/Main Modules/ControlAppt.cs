		...
		private void menuApt_Click(object sender,System.EventArgs e) {
			...
			switch(((ToolStripMenuItem)sender).Name) {
				...
				case MenuItemNames.SendText:
					...
					GlobalFormOpenDental.SendTextMessage?.Invoke(appointment.PatNum,"");
					break;
				case MenuItemNames.SendConfirmationText:
					...
					bool wasTextSent=GlobalFormOpenDental.SendTextMessage?.Invoke(patient.PatNum,message)??false;
					if(wasTextSent) {
				...
				case MenuItemNames.SendComeInText:
					...
					GlobalFormOpenDental.SendTextMessage?.Invoke(appointment.PatNum,message);
					break;
				case MenuItemNames.SendPaymentLinkText:
					...
					using (FormTextPayLink formTextPayLink=new FormTextPayLink(appointment,patient)){ 
						...
						bool didSend=GlobalFormOpenDental.SendTextMessage?.Invoke(appointment.PatNum,formTextPayLink.Message)??false;
						if(didSend) {
