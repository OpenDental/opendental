...
namespace OpenDental
{
	...
	public partial class ControlImagesJ : UserControl{
		...
		private bool? _showDrawingsPearl=null;
		...
		public void LayoutToolBarDraw() {
			...
			if(_showDrawingsPearl is null){//happens once on startup
				if(Programs.IsEnabled(ProgramName.Pearl)){
					...
					if(programProperty==null) {
						...
						_showDrawingsPearl=programProperty.PropertyValue=="true";
					}
				}
				else{
					_showDrawingsPearl=false;
			...
			formImageFloat.SetShowDrawings(_showDrawingsOD,_showDrawingsPearl.Value);
			/*
		...
		public void ModuleSelected(long patNum,long docNum=0){
			...
			if(_patCur!=null && _patCur.PatStatus==PatientStatus.Archived && !Security.IsAuthorized(EnumPermType.ArchivedPatientSelect,suppressMessage:true)) {
				FormOpenDental.S_Contr_PatientSelected(new Patient(),false);
				try {
					RefreshModuleData(0);
				}
		...
		private void toolBarMain_ButtonClick(object sender, ODToolBarButtonClickEventArgs e){
			...
			if(e.Button.Tag is Program program) {
				if(program.ProgName==ProgramName.Pearl.ToString()){
					//these first few ifs are copied form ProgramL, because we are skipping that and doing our own here.
					...
					if(_showDrawingsPearl==false){//if Pearl annotations are set to not show right now
						//turn them on for the rest of this session.
						_showDrawingsPearl=true;
					}
					...
					ProgramL.Execute(program.ProgramNum,_patCur);
				}
				return;
			}
		...
		private void ShowFilterWindow(){
			...
			frmImageFilter.ShowPearl=_showDrawingsPearl==true;
			frmImageFilter.ShowDialog();
			if(!frmImageFilter.IsDialogOK){
				return;
			}
			_showDrawingsOD=frmImageFilter.ShowOD;
			_showDrawingsPearl=frmImageFilter.ShowPearl;
			formImageFloat.SetShowDrawings(_showDrawingsOD,_showDrawingsPearl==true);
		}
		...
		private void ToolBarMountAcquire_Click(){
			...
			if(mountDef!=null){
				...
				SecurityLogs.MakeLogEntry(EnumPermType.ImageCreate,_patCur.PatNum,logText);
				FillTree(false);
		...
		private void ToolBarMoveToPatient(object sender, EventArgs e){
			...
			if(mount!=null){
				...
				string logText = "Mount Deleted: "+mount.Description+" with category "
					+defDocCategory.ItemName;
				SecurityLogs.MakeLogEntry(EnumPermType.ImageDelete,_patCur.PatNum,logText);
			}
		}
		...
		private void ToolBarPrint_Click(){
			...
			if(IsDocumentShowing()){
				SecurityLogs.MakeLogEntry(EnumPermType.Printing,_patCur.PatNum,"Patient image "+GetDocumentShowing(0).FileName+" "+GetDocumentShowing(0).Description+" printed");
			}
			if(IsMountShowing()){
				SecurityLogs.MakeLogEntry(EnumPermType.Printing,_patCur.PatNum,"Patient mount "+GetMountShowing().Description+" printed");
			}
		}
