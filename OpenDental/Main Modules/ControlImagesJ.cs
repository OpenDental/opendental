...
namespace OpenDental
{
	...
	public partial class ControlImagesJ : UserControl{
		...
		private void _fileSystemWatcher_FileCreated(object sender,FileSystemEventArgs e) {
			...
			formImageFloat.DocumentAcquiredForMount(document,bitmap,mountItem);
			bitmap?.Dispose();
			File.Delete(e.FullPath);
...
//todo: dpi for floaters?
		...
		private void ToolBarMountAcquire_Click(){
			...
			using FormMountSelect formMountSelect=new FormMountSelect();
			formMountSelect.ShowDialog();
			if(formMountSelect.DialogResult!=DialogResult.OK){
				return;
			}
			MountDef mountDef=formMountSelect.MountDefSelected;
			if(mountDef!=null){
				long defNumCategory;
			...
			ImagingDevice imagingDevice=formMountSelect.ImagingDeviceSelected;
...
//Need to test/simulate twain multi spilling into unmounted.
//S33682 - Added pref ImagingDefaultScaleValue, but not yet implemented
//Add new pref ImagingAfterAcquireAdjust to put some people into adjust mode instead of pan mode.
//

//Future improvements:
//MountItem rotate
//Move mountItem outside bounds, and bounds expand.
