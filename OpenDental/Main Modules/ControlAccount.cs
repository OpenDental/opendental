	...
	public partial class ControlAccount:UserControl {
		...
		public void LayoutToolBar() {
			...
			if(!PrefC.GetBool(PrefName.EasyHideRepeatCharges)) {
				...
				toolBarButton.Style=ODToolBarButtonStyle.PushButton;
		...
		private void FillPaymentPlans() {
			...
			if(PrefC.GetBool(PrefName.FuchsOptionsOn)) {
				...
				labelTotalPtOwes.Text=(_patientPortionBalanceTotal + (decimal)_family.ListPats[0].BalTotal - (decimal)_family.ListPats[0].InsEst).ToString("F");
			}
		}
		...
		private bool AddProcAndValidate(string procString,Provider provider) {
			...
			Procedures.Insert(procedure,skipDiscountPlanAdjustment:orthoCaseProcLinkingData.CanProcLinkToOrthoCase(procedure));
