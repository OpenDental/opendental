...
namespace OpenDental {
	...
	public partial class ControlAccount:UserControl {
		...
		private void menuItemLimitedCustom_Click(object sender,EventArgs e) {
			...
			using FormLimitedStatementSelect formLSS=new FormLimitedStatementSelect(table,listPayClaimNums,listAdjNums,listPayNums,listProcNums,listPatNums,listPayPlanChargeNums);
			...
			listPayPlanChargeNums=formLSS.ListSelectedPayPlanChargeNums;
		...
		private void toolBarButStatement_Click() {
			...
			if(textDateStart.IsValid() && textDateStart.Text!="") {//textDateStart has ultimate precedence. User may have intentionally set the date range for statement.
				...
				if(PrefC.GetBool(PrefName.BillingShowTransSinceBalZero)) {
					DateTime dateTimeAsOf=DateTime.Today;//Used to determine when the balance on this date began.
					if(PrefC.GetBool(PrefName.AgingCalculatedMonthlyInsteadOfDaily)) {//If aging calculated monthly, use the last aging date instead of today.
						dateTimeAsOf=PrefC.GetDate(PrefName.DateLastAging);
					}
					...
					DataTable tableBals=Ledgers.GetDateBalanceBegan(patAges,dateTimeAsOf,isSuperBills:false);//More Options selection has a super family option. We would need new checkbox here.
