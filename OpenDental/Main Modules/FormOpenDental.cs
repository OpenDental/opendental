...
namespace OpenDental{
    ...
    private FormSmsTextMessaging _formSmsTextMessaging;
		...
		public void FormOpenDentalShown() {
			...
			if(patient!=null && (_strCmdLineShow=="popup" || _strCmdLineShow=="popups") && moduleBar.SelectedModule!=EnumModuleType.None) {
				...
				formPopupsForFam.PatCur=patient;
		...
		private long GetAptNumForLetter() {
			Appointment[] appointmentArray=Appointments.GetForPat(PatNumCur);
			long aptNum;
			if(appointmentArray.Length==0) {
				...
				using FormApptsOther formApptsOther=new FormApptsOther(PatNumCur,null);
		...
		private void toolButPopups_Click() {
			using FormPopupsForFam formPopupsForFam=new FormPopupsForFam(_listPopupEvents);
			formPopupsForFam.PatCur=Patients.GetPat(PatNumCur);
			formPopupsForFam.ShowDialog();
		}
		#endregion ToolBar
			...
			List<UserOdPref> listUserOdPrefBlockedTasks) 
		{
			...
			for(int i=0;i<Application.OpenForms.Count;i++) {
				...
				if(listSignalTasksNums.Contains(formTaskEdit.TaskNumCur)) {
					formTaskEdit.OnTaskEdited();
					listTaskNumsOpen.Add(formTaskEdit.TaskNumCur);
				}
			}
	...
	private void TaskGoTo(TaskObjectType taskObjectType,long keyNum){
		...
		private void menuItemOperatories_Click(object sender, System.EventArgs e) {
			...
			formOperatories.ContrApptRef=controlAppt;
		...
		private void menuItemQuickPasteNotes_Click(object sender,EventArgs e) {
			...
			formQuickPaste.QuickType=QuickPasteType.None;
		...
		private void menuItemPendingPayments_Click(object sender,EventArgs e) {
			FormPendingPayments formPendingPayments=new FormPendingPayments();
			formPendingPayments.Show();//Non-modal so the user can view the patient's account
		}
		...
		private void menuItemAlerts_Click(object sender,EventArgs e) {
			...
			switch(alertItem.FormToOpen) {
				case FormType.FormPendingPayments:
					FormPendingPayments formPendingPayments=new FormPendingPayments();
					formPendingPayments.Show();//Non-modal so the user can view the patient's account
					formPendingPayments.FormClosed+=this.alertFormClosingHelper;
					break;
