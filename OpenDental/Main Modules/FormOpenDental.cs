...
namespace OpenDental{
    ...
    private FormSmsTextMessaging _formSmsTextMessaging;
		...
		private bool IsCloudUserIpAllowed() {
			if(!ODBuild.IsWeb() || Security.IsAuthorized(Permissions.AllowLoginFromAnyLocation, true)) {
				return true;
			}
			string[] addresses=PrefC.GetString(PrefName.CloudAllowedIpAddresses).Split(",", StringSplitOptions.RemoveEmptyEntries);
			string addressCur=Browser.GetComputerIpAddress();
			if(addresses.Contains(addressCur)) {
				return true;
			}
