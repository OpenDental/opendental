...
namespace OpenDental{
	...
	public class ControlManage : UserControl{
		...
		private void butClockOut_Click(object sender,EventArgs e) {
			...
			progressOD.ActionMain=() => {
				bool[] isAuthorized=new bool[1] { false };
				if(Plugins.HookMethod(this,"ContrStaff.butClockOut_Click_ClockOut",isAuthorized,_employee,_listTimeClockStatusesShown[listBoxStatus.SelectedIndex])) {
					if(!isAuthorized[0]) {
      ...
      if(_formEmailInbox==null || _formEmailInbox.IsDisposed) {
		...
		private void FillEmps(bool selectUserEmployee) {
			...
			int index=-1;
			if(selectUserEmployee) {//Only select current user's employee when refreshing the module.
		...
		private void FillMessages() {
			...
			foreach(SigMessage sigMessage in _listSigMessages) {
				...
				if(sigMessage.ToUser!=""//blank user always shows
					...
					&& _sigElementDefArrayUsers[comboBoxViewUser.SelectedIndex-1].SigText!=sigMessage.ToUser)//and users don't match
				{
					continue;
				}
				row=new GridRow();
