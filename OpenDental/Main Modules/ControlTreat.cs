...
//		private System.Windows.Forms.PrintDialog printDialog2;
    ...
    private ArrayList ALPreAuth;
		...
		private void InitializeComponent(){
			...
			this.dateTimeTP.Leave += new System.EventHandler(this.dateTimeTP_OnLeave);
			// 
			// tabShowSort
			// 
      ...
      FillPreAuth();
		...
		private void FillPlans(bool doRefreshData=true){
			...
			for(int i=0;i<_listTreatPlans.Count;i++){
				...
				row.Cells.Add(str);
				if(string.IsNullOrEmpty(treatPlanCur.Signature)) {
					row.Cells.Add("");
				}
				else{
					row.Cells.Add("X");
					...
					switch(fields[j].InternalName) {
						...
						case "Description":
							...
							if(RowsMain[i].Description!=null) {
								description=RowsMain[i].Description.ToString();
							}
							//Do not display tax in description if tax column is being displayed, the tax amount is zero, or in the subtotal/total rows. 
							if(!hasSalesTax && !CompareDecimal.IsZero(RowsMain[i].TaxEst) && !(RowsMain[i].Description=="Subtotal") && !(RowsMain[i].Description=="Total")) {
								description+="\nTax Est: "+RowsMain[i].TaxEst.ToString("F");
							}
		...
		private void FillSummary(){
			...
			if(_tpData.DiscountPlanSub!=null) {
				...
				userControlIndIns.RefreshInsurance(PatCur,InsPlanList,SubList,PatPlanList,BenefitList,HistList,dateTimeTP.Value);
			}
		}		
    ...
    private void FillPreAuth(){
      ...
      for(int i=0;i<ALPreAuth.Count;i++) {
			...
			if(PrefC.GetBool(PrefName.InsChecksFrequency)) {
				...
				for(int i=0;i<listProcForTP.Count;i++) {
					...
					Procedures.ComputeEstimates(listProcForTP[i],PatCur.PatNum,ref ClaimProcList,false,InsPlanList,PatPlanList,BenefitList,
						...
						orthoProcLink,orthoCase,orthoSchedule,listOrthoProcLinksForOrthoCase,_loadActiveData.BlueBookEstimateData);
				...
				for(int i=0;i<listProcForTP.Count;i++) {
					...
					Procedures.ComputeEstimates(listProcForTP[i],PatCur.PatNum,ref ClaimProcList,false,InsPlanList,PatPlanList,BenefitList,HistList,LoopList
						...
						blueBookEstimateData:_loadActiveData.BlueBookEstimateData);
					//then, add this information to loopList so that the next procedure is aware of it.
					LoopList.AddRange(ClaimProcs.GetHistForProc(ClaimProcList,listProcForTP[i].ProcNum,listProcForTP[i].CodeNum));
				}
				SyncCanadianLabs(ClaimProcList,listProcForTP);
			...
			for(int i=0;i<listProcForTP.Count;i++) {
				...
				if(PatPlanList.Count>0) { //Primary
					...
					if((claimproc!=null && claimproc.NoBillIns) || (claimproc==null && procCodeCur.NoBillIns)) {
						...
						Appointment appointmentForProc=listAppts.Find(x=>x.AptNum==listProcForTP[i].AptNum);
						if(appointmentForProc==null) {
							appointmentForProc=listAppts.Find(x=>x.AptNum==listProcForTP[i].PlannedAptNum); //If a scheduled appt doesn't exist, check for a planned one.
						}
						allowed=ComputeAllowedAmount(listProcForTP[i],claimproc,this._listSubstLinks,lookupFees,_loadActiveData.BlueBookEstimateData,appointmentForProc);
					}
				}
				if(PatPlanList.Count>1) { //Secondary
			...
			,BlueBookEstimateData blueBookEstimateData,Appointment appointment=null)
		{
			...
			if(claimProc!=null) {				
				if(claimProc.AllowedOverride!=-1) {//check for allowed override
					...
					allowed=InsPlans.GetAllowedForProc(procedure,claimProc,InsPlanList,listSubstitutionLinks,lookupFees,blueBookEstimateData,appointment);
		...
		private void ToolBarMainUpdate_Click() {
			...
			BlueBookEstimateData blueBookEstimateData=new BlueBookEstimateData(InsPlanList,SubList,PatPlanList,ProcListTP.ToList(),_listSubstLinks);
			for(int i=0;i<ProcListTP.Length;i++) {
