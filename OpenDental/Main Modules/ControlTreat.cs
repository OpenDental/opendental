	...
	public partial class ControlTreat : System.Windows.Forms.UserControl{
		...
		private void FillMainData() {
			...
			for(int i=0;i<_listProcTPsSelect.Count;i++) {
				...
				row.Appt=row.Appt=FillApptCellForRow(plannedAptNumCur,aptNumCur,apptStatus);
		...
		private void FillMainDisplay(bool hasSelectedTpChanged=false){
			...
			for(int i=0;i<listDisplayFields.Count;i++){
				...
				if(ListTools.In(listDisplayFields[i].InternalName,"Sub",DisplayFields.InternalNames.TreatmentPlanModule.Appt)) {
		...
		private void FillGridPrint() {
			...
			for(int i=0;i<listDisplayFields.Count;i++) {
				...
				if((ListTools.In(listDisplayFields[i].InternalName,"Pri Ins","Sec Ins","DPlan","Allowed")) && !checkShowIns.Checked) {
					continue;
				}
				if(listDisplayFields[i].InternalName=="Discount" && !checkShowDiscount.Checked) {
					continue;
					...
					switch(listDisplayFields[j].InternalName) {
						...
						case DisplayFields.InternalNames.TreatmentPlanModule.CatPercUCR:
							if(PrefC.GetBool(PrefName.TreatPlanItemized) 
								|| ListTools.In(_listTpRowsMain[i].Description.ToString(),Lan.g("TableTP","Total"),Lan.g("TableTP","Subtotal"))) 
							{
								row.Cells.Add(_listTpRowsMain[i].CatPercUCR.ToString("F"));
							}
							else {
		...
		private void gridMain_CellDoubleClick(object sender, OpenDental.UI.ODGridClickEventArgs e) {
			...
			{//current plan
				...
				_listClaimProcHistsLoops=new List<ClaimProcHist>();
			...
			if(formProcTPEdit.ProcCur!=null && formProcTPEdit.ProcCur.Discount>0) {
		...
		private void TrySendTreatPlan(){
			...
			MobileAppDevice mobileAppDevice=listMobileAppDevices.Where(x => x.PatNum==PatientCur.PatNum).OrderByDescending(x => x.LastCheckInActivity).FirstOrDefault();
		...
		private void SetPriority(Def defSelected,TreatPlan treatPlanSelected,List<ProcTP> listProcTPsSelected) {
			...
			if(_listTreatPlans.Count>0
				 && (treatPlanSelected.TPStatus==TreatPlanStatus.Active || treatPlanSelected.TPStatus==TreatPlanStatus.Inactive)) 
			{
				TreatPlanAttaches.SetPriorityForTreatPlanProcs(defSelected.DefNum,treatPlanSelected.TreatPlanNum,listProcNumsSelected);
			}
			else { //any Saved TP
				if(!Security.IsAuthorized(Permissions.TreatPlanEdit,treatPlanSelected.DateTP)) {
					return;
				}
				ProcTPs.SetPriorityForTreatPlanProcs(defSelected.DefNum,treatPlanSelected.TreatPlanNum,listProcNumsSelected);
			}
		...
		private void PrepImageForPrinting(){
			...
			_listToothInitials=ToothInitials.Refresh(PatientCur.PatNum);
		...
		private List<ProcTP> LoadActiveTP(ref TreatPlan treatPlan) {
			...
			List<Appointment> listAppointments=Appointments.GetAppointmentsForProcs(_listProcedures);
			if(listProceduresForTPs.Any(x => ProcedureCodes.GetWhereFromList(y => y.CodeNum==x.CodeNum).Count==0)) {
				MsgBox.Show(this,$"Missing codenum. Please run database maintenance method {nameof(DatabaseMaintenances.ProcedurelogCodeNumInvalid)}.");
			...
			if(PrefC.GetBool(PrefName.InsChecksFrequency)) {
				...
				for(int i=0;i<listProceduresForTPs.Count;i++) {
					listProceduresForTPs[i].ProcDate=dateTimeTP.Value;
					...
					Procedures.ComputeEstimates(listProceduresForTPs[i],PatientCur.PatNum,ref _listClaimProcs,false,_listInsPlans,_listPatPlans,_listBenefits,
						_listClaimProcHists,_listClaimProcHistsLoops,false,
				...
				for(int i=0;i<listProceduresForTPs.Count;i++) {
					...
					OrthoCases.FillOrthoCaseObjectsForProc(listProceduresForTPs[i].ProcNum,ref orthoProcLink,ref orthoCase,ref orthoSchedule,
						ref listOrthoProcLinksForOrthoCases,dictionaryOrthoProcLinksForProcList,dictionaryOrthoCases,dictionaryOrthoSchedules,listOrthoProcLinksAll);
					Procedures.ComputeEstimates(listProceduresForTPs[i],PatientCur.PatNum,ref _listClaimProcs,false,_listInsPlans,_listPatPlans,_listBenefits,_listClaimProcHists,_listClaimProcHistsLoops
			...
			if(_tpModuleData.DiscountPlanSub!=null && !listProceduresForTPs.IsNullOrEmpty()) {
				listDiscountPlanProcs=DiscountPlans.GetDiscountPlanProc(listProceduresForTPs,discountPlanSub:_tpModuleData.DiscountPlanSub,discountPlan:_tpModuleData.DiscountPlan);
			}
			...
			for(int i=0;i<listProceduresForTPs.Count;i++) {
				...
				row.Appt=FillApptCellForRow(listProceduresForTPs[i].PlannedAptNum,listProceduresForTPs[i].AptNum, apptStatus);
				...
				row.Tth=(Tooth.ToInternat(listProceduresForTPs[i].ToothNum));
				...
				if(listProceduresForTPs[i].ToothRange!="") {
					descript+=" #"+Tooth.FormatRangeForDisplay(listProceduresForTPs[i].ToothRange);
				}
				if(checkShowMaxDed.Checked) {
				...
				procTP.ToothNumTP=Tooth.ToInternat(procedure.ToothNum);
				...
				listProcTPsRetVals.Add(procTP);
				procTP.TagOD=procedure.ProcNumLab;//Used for selection logic. See gridMain_CellClick(...).
				row.Tag=procTP;
				_listTpRowsMain.Add(row);
				#region subtotal
				if(checkShowSubtotals.Checked &&
					 (i==listProceduresForTPs.Count-1 || listTreatPlanAttaches.FirstOrDefault(x => x.ProcNum==listProceduresForTPs[i+1].ProcNum).Priority!=procTP.Priority)) {
					row=new TpRow();
					row.Description=Lan.g("TableTP","Subtotal");
					row.Fee=subfee;
					row.PriIns=subpriIns;
			...
			#endregion Totals
			treatPlan.ListProcTPs=listProcTPsRetVals;
			return listProcTPsRetVals;
		}
		...
		private void ComputeProcListFiltered() {
			...
			for(int i=0;i<_listProcTPsSelect.Count;i++) {
				...
				if(Tooth.IsValidEntry(_listProcTPsSelect[i].ToothNumTP)) {
					procedureDummy.ToothNum=Tooth.FromInternat(_listProcTPsSelect[i].ToothNumTP);
				}
		...
		private void ToolBarMainUpdate_Click() {
			...
			OrthoCases.GetDataForListProcs(ref listOrthoProcLinksAll,ref dictOrthoProcLinksForProcList,ref dictOrthoCases,ref dictOrthoSchedules,_listProcedures);
			OrthoProcLink orthoProcLink=null;
			OrthoCase orthoCase=null;
			OrthoSchedule orthoSchedule=null;
			...
			for(int i=0;i<_listProceduresTP.Count;i++) {
				...
				OrthoCases.FillOrthoCaseObjectsForProc(procedure.ProcNum,ref orthoProcLink,ref orthoCase,ref orthoSchedule,ref listOrthoProcLinksForOrthoCase,
		...
		private void ToolBarMainCreate_Click(){//Save TP
			...
			ModuleSelected(PatientCur.PatNum);
			for(int i=0;i<_listTreatPlans.Count;i++){
				if(_listTreatPlans[i].TreatPlanNum==treatPlan.TreatPlanNum){
					gridPlans.SetSelected(i,true);
					FillMain();
				}
			}
		...
		private void ToolBarMainPreAuth_Click() {
			...
			if(formInsPlanSelect.SelectedPlan==null) {//Won't be null if there is only one PatPlan
			...
			claim.PlanNum=formInsPlanSelect.SelectedPlan.PlanNum;
			claim.InsSubNum=formInsPlanSelect.SelectedSub.InsSubNum;
			claim.ProvTreat=0;
			claim.ClaimForm=formInsPlanSelect.SelectedPlan.ClaimFormNum;
			claim.MedType=EnumClaimMedType.Dental;
			if(formInsPlanSelect.SelectedPlan.IsMedical) {
			...
			claim.PatRelat=formInsPlanSelect.PatRelat;
			...
			for(int i=0;i<listProceduresSelected.Count;i++) {
				claimProcExisting=ClaimProcs.GetEstimate(_listClaimProcs,listProceduresSelected[i].ProcNum,formInsPlanSelect.SelectedPlan.PlanNum,formInsPlanSelect.SelectedSub.InsSubNum);
				double insPayEst=0;
				...
				ClaimProcs.CreateEst(claimProc,listProceduresSelected[i],formInsPlanSelect.SelectedPlan,formInsPlanSelect.SelectedSub,0,insPayEst,false,true);//preauth est
				...
				if(formInsPlanSelect.SelectedPlan.UseAltCode && (procedureCode.AlternateCode1!="")){
					claimProc.CodeSent=procedureCode.AlternateCode1;
				}
				else if(formInsPlanSelect.SelectedPlan.IsMedical && listProceduresSelected[i].MedicalCode!=""){
		...
		private void butPlannedAppt_Click(object sender,EventArgs e) {
			...
			ChartModuleComponentsToLoad chartModuleComponentsToLoad=new ChartModuleComponentsToLoad(false);
