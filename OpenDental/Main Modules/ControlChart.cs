...
namespace OpenDental {
	...
	public partial class ControlChart:UserControl	{
		...
		private List<Mount> _listMounts;
		...
		private void menuItemViewMultiVisitGroup_Click(object sender,EventArgs e) {
			...
			foreach(KeyValuePair<long,List<DataRow>> keyValuePair in dictProceduresPerGroup) {
				FormMultiVisitGroup formMultiVisitGroup=new FormMultiVisitGroup {
					GridColumns=gridProg.Columns.ToList(),
      ...
      //Pen penBlue=new Pen(Color.FromArgb(97,136,173));
		...
		private void gridPlanned_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			if(Programs.UsingOrion) {
				if(formApptEdit.DialogResult==DialogResult.OK) {
					if(_patCur != null) { //module might have changed, such as when clicking GoTo on a task launched from the appointment edit window. _patCur is null here if that happens.
		...
		private void FillImages() {
			...
			for(int i=0;i<_listMounts.Count;i++) {
				if(tabControlImages.SelectedIndex>0) {//any category except 'all'
					if(_listMounts[i].DocCategory!=_listDefNumsVisImageCats[tabControlImages.SelectedIndex-1]){
						continue;//not in current category
					}
				}
				ImageInfo imageInfo=new ImageInfo();
				imageInfo.MountNum=_listMounts[i].MountNum;
				imageInfo.DateCreated=_listMounts[i].DateCreated;
				_listImageInfos.Add(imageInfo);
			}
			for(int i=0;i<_listDocuments.Count;i++) {
				if(_listDocuments[i].MountItemNum>0){
					continue;//skip images attached to mounts
				}
				if(!_listDefNumsVisImageCats.Contains(_listDocuments[i].DocCategory)){
					continue;//category doesn't show in chart
				}
				if(tabControlImages.SelectedIndex>0) {//any category except 'all'
					if(_listDocuments[i].DocCategory!=_listDefNumsVisImageCats[tabControlImages.SelectedIndex-1]){
						continue;//not in current category
					}
				}
				ImageInfo imageInfo=new ImageInfo();
				imageInfo.DocNum=_listDocuments[i].DocNum;
				_listImageInfos.Add(imageInfo);
			}
			...
			for(int i=0;i<_listImageInfos.Count;i++){
				if(_listImageInfos[i].DocNum>0){
					Document document=_listDocuments.Find(x=>x.DocNum==_listImageInfos[i].DocNum);
					...
					Mount mount=_listMounts.Find(x=>x.MountNum==_listImageInfos[i].MountNum);
		...
		private void MenuItemSetSelectedProcsStatus(ProcStat newProcStatus) {
			...
			foreach(DataRow row in listSelectedRows) {
				...
				if(procNew.AptNum!=0) {//if attached to an appointment
					...
					procNew.PlaceService=Clinics.GetPlaceService(_patCur.ClinicNum);
				}
...
//todo: track down where this is altered.  Optimize for eCW:
			...
			_listDocuments=LoadData.ListDocuments;
			_listMounts=LoadData.ListMounts;
			//StartXVWebThread();
...
//todo: optimize for Full mode:
		...
		private void Tool_CCD_Click() {
			...
			formContinuityOfCareDocument.PatCur=_patCur;
		...
		private void Tool_EHR_Click(bool onLoadShowOrders) {
			...
			formEHR.PatientNoteCur=_patientNoteCur;
			formEHR.FamilyCur=_famCur;
			formEHR.ShowDialog();
...
//todo: compare with: Bridges.ECW.AptNum, no need to generate PDF segment, pdfs only with eCW and this button not available with eCW integration
		...
		private bool AddProcHelper(Procedure ProcCur,List<Fee> listFees) {
			if(ProcedureCodes.AreAnyProcCodesHidden(ProcCur.CodeNum)) {
				MessageBox.Show(this,Lan.g(this,"Cannot add procedure because it is in a hidden category")+$": {ProcedureCodes.GetProcCode(ProcCur.CodeNum).ProcCode}");
				return false;
			}
