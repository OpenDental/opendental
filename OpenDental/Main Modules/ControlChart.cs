...
namespace OpenDental {
	...
	public partial class ControlChart:UserControl	{
		...
		private Dictionary<long,FormErx> _dictionaryFormErxSessions=new Dictionary<long,FormErx>();
		///<summary>Dictionary linking a TreatPlanNum key to the list of TpRows for that TP.</summary>
		private Dictionary<long,List<TpRow>> _dictionaryTpNumListTpRows;
		...
		private void ErxBrowserClosed(ODEventArgs e) {
			...
			_dictionaryFormErxSessions.Remove(patient.PatNum);
		...
		private void menuItemGroupSelected_Click(object sender,EventArgs e) {
			...
			if(Programs.UsingOrion) {
				...
				orionProc.ProcNum=procedureGroup.ProcNum;
		...
		private void FillTpProcData() {
			_dictionaryTpNumListTpRows=new Dictionary<long,List<TpRow>>();
			...
			for(int i=0;i<gridTreatPlans.SelectedIndices.Length;i++) {
				...
				_dictionaryTpNumListTpRows[_listTreatPlans[gridTreatPlans.SelectedIndices[i]].TreatPlanNum]=listTpRows;
			}
		}
		...
		private void FillTpProcDisplay() {
			...
			if(IsPatientNull() || _dictionaryTpNumListTpRows==null || gridTreatPlans.ListGridRows.Count==0) {
				gridTpProcs.EndUpdate();
				return;
			}
			GridRow row;
			foreach(KeyValuePair<long,List<TpRow>> kvPair in _dictionaryTpNumListTpRows) {
				row=new GridRow();
				if(_dictionaryTpNumListTpRows.Count>1) {
					row.Cells.Add("");
					row.Cells.Add("");
					row.Cells.Add("");
					row.Cells.Add("");
		...
		private void MenuItemSetSelectedProcsStatus(ProcStat procStatNew) {
			...
			for(int i=0;i<listProceduresSelected.Count();i++) {
				...
				OrthoProcLink orthoProcLink=OrthoProcLinks.TryLinkProcForActiveOrthoCase(orthoCaseProcLinkingData,procedureNew);
		...
		private void Tool_eRx_Click(bool isShowRefillsAndErrors=false) {
			...
			if(_dictionaryFormErxSessions.ContainsKey(Pd.PatNum) && _dictionaryFormErxSessions[Pd.PatNum]!=null) {
				_dictionaryFormErxSessions[Pd.PatNum].Restore();
				_dictionaryFormErxSessions[Pd.PatNum].BringToFront();
			...
			try {
				...
				//hook for additional authorization before prescription is saved
				bool[] boolArrayAuthorized=new bool[1] { false };
				if(Plugins.HookMethod(this,"ContrChart.Tool_eRx_Click_Authorize2",boolArrayAuthorized,provider)) {
					if(!boolArrayAuthorized[0]) {
...
//todo: compare with: Bridges.ECW.AptNum, no need to generate PDF segment, pdfs only with eCW and this button not available with eCW integration
		...
		private void UpdateErxAccess(string npi,string userId,long clinicNum,string clinicId,string clinicKey,ErxOption erxOption) {
			...
			else if(providerErx.IsEnabled!=ErxStatus.Enabled //If prov is not yet enabled, always check with OD HQ to see if the prov has been enabled yet.
				|| (erxOption==ErxOption.NewCrop && !PrefC.GetBool(PrefName.NewCropIsLegacy) && !providerErx.IsIdentifyProofed)//If new prov is not yet identity proofed, send to OD HQ.
				|| !providerErx.IsSentToHq//If prov has not been sent to OD HQ yet, always send to OD HQ so we can track our providers using eRx.
				|| dateLastAccessMonth<new DateTime(DateTime.Today.Year,DateTime.Today.Month,1))//If it has been over a month since sent to OD HQ, send.
			{
				...
				try {
					string result=service1Update.GetErxAccess(stringBuilder.ToString());//may throw error
					XmlDocument xmlDocument=new XmlDocument();
					xmlDocument.LoadXml(result);
		...
		private bool AddProcHelper(Procedure procedure,List<Fee> listFees) {
			...
			List<ClaimProcHist> listClaimProcHistsLoop=new List<ClaimProcHist>();
			if(!ChartModules.AddProcHelper(procedure,listFees,Pd,_procStatNew,_listProceduresCharted,_listTreatPlans,_listSubstitutionLinks,priorityNum,listTpNums,
		...
		private void DeleteRows() {
			...
			Dictionary<long,OrthoProcLink> dictOrthoProcLinks=OrthoProcLinks.GetManyForProcs(listDataRowsSelected.Where(x => PIn.Long(x["ProcNum"].ToString())!=0)
				.ToList().Select(y => PIn.Long(y["ProcNum"].ToString())).ToList()).ToDictionary(z => z.ProcNum,z => z);
			foreach(DataRow row in listDataRowsSelected) {
				long procNum=PIn.Long(row["ProcNum"].ToString());
				if(procNum!=0) {
					dictOrthoProcLinks.TryGetValue(procNum,out orthoProcLink);
				}
				EnumSkippedRow enumSkippedRow=CanDeleteRow(row,isCheckDb:true,orthoProcLink);
		...
		private void CheckForSearchMatch(string columnName,DataRow dataRow,ref List<string> listSearchInputs,bool isClinicDesc=false,bool isClinicAbbr=false) {
			for(int i=listSearchInputs.Count-1;i>=0;--i) {
				...
				else if((dataRow[columnName].ToString().ToLower()).Contains(listSearchInputs[i])) {
					listSearchInputs.RemoveAt(i);
				}
			}
		}
		...
		private void SearchProgNotes() {
			...
			if(_listDataRowsSearchResults!=null) {
				foreach(DataRow curRow in _listDataRowsSearchResults) {
					DataRow dataRow=Pd.TableProgNotes.NewRow();
					dataRow.ItemArray=curRow.ItemArray;
			...
			string currentSearchText) 
		{
			...
			foreach(DataRow rowCur in table.Rows) {
				...
				if(checkNotes.Checked) {
					CheckForSearchMatch("note",rowCur,ref listSearchInputs);
				}
				for(int f=0;f<listDisplayFields.Count;f++) {
					switch(listDisplayFields[f].InternalName) {
						case DisplayFields.InternalNames.ChartView.Date:
							CheckForSearchMatch("procDate",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.Time:
							CheckForSearchMatch("procTime",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.Th:
							CheckForSearchMatch("toothNum",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.Surf:
							CheckForSearchMatch("surf",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.Dx:
							CheckForSearchMatch("dx",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.Description:
							CheckForSearchMatch("description",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.Stat:
							CheckForSearchMatch("procStatus",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.Prov:
							CheckForSearchMatch("prov",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.Amount:
							CheckForSearchMatch("procFee",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.ProcCode:
							CheckForSearchMatch("ProcCode",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.User:
							CheckForSearchMatch("user",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.Signed:
							CheckForSearchMatch("signature",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.Priority:
							CheckForSearchMatch("priority",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.DateEntry:
							CheckForSearchMatch("dateEntryC",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.Prognosis:
							CheckForSearchMatch("prognosis",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.DateTP:
							CheckForSearchMatch("dateTP",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.EndTime:
							CheckForSearchMatch("procTimeEnd",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.Quadrant:
							CheckForSearchMatch("quadrant",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.ScheduleBy:
							CheckForSearchMatch("orionDateScheduleBy",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.StopClock:
							CheckForSearchMatch("orionDateStopClock",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.DPC:
							CheckForSearchMatch("orionDPC",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.EffectiveComm:
							CheckForSearchMatch("orionIsEffectiveComm",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.OnCall:
							CheckForSearchMatch("orionIsOnCall",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.Stat2:
							CheckForSearchMatch("orionStatus2",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.DPCpost:
							CheckForSearchMatch("orionDPCpost",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.Length:
							CheckForSearchMatch("length",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.Abbr: //abbreviation for procedures
							CheckForSearchMatch("AbbrDesc",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.Locked:
							CheckForSearchMatch("isLocked",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.HL7Sent:
							CheckForSearchMatch("hl7Sent",rowCur,ref listSearchInputs);
							break;
						case DisplayFields.InternalNames.ChartView.Clinic:
							CheckForSearchMatch("ClinicNum",rowCur,ref listSearchInputs,isClinicAbbr:true);
							break;
						case DisplayFields.InternalNames.ChartView.ClinicDesc:
							CheckForSearchMatch("ClinicNum",rowCur,ref listSearchInputs,isClinicDesc:true);
							break;
						default:
							break;
					}
					if(listSearchInputs.Count==0) {//All the passed in search terms match.
						listDataRowsSearchResult.Add(rowCur);
						break;
					}
				}
			}
