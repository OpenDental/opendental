...
namespace OpenDental{
	...
	public partial class ControlFamily : UserControl {
		...
		private void ToolButMove_Click() {
			...
			if(_patient.PatNum==_patient.Guarantor) {//if guarantor selected
				...
				if(_family.ListPats.Length==1) {//and no other family members
					...
					foreach(Patient pat in _family.ListPats) {
						if(pat.PatNum==_patient.PatNum) {
							continue;
						}
						List<PatientLink> listPatientLinks=PatientLinks.GetLinks(pat.PatNum,PatientLinkType.Merge);//If there is another family member, make sure it is merged.  
						if(listPatientLinks.Count==0 || !listPatientLinks.Exists(x => x.PatNumFrom==pat.PatNum)) {//If it's not merged, user can't move guarantor.
		...
		private void AddSuperGuarPriInsToFam(long guarNum) {
			...
			foreach(Patient pat in family.ListPats) {//possibly filter by PatStatus, i.e. .Where(x => x.PatStatus==PatientStatus.Patient)
				listPatPlansForPat=PatPlans.Refresh(pat.PatNum);
				insSub1=listInsSubsForFam.FirstOrDefault(x => x.Subscriber==pat.PatNum && x.PlanNum==insSub.PlanNum);
				if(insSub1!=null) {//InsSub already exists for this Patient and InsPlan
					...
					insSub1.SubscriberID=string.IsNullOrWhiteSpace(pat.MedicaidID)?"":pat.MedicaidID;
				...
				if(string.IsNullOrWhiteSpace(insSub1.SubscriberID)) {
					...
					List<Procedure> listProcedures=Procedures.Refresh(pat.PatNum);
					List<Benefit> listBenefits=Benefits.Refresh(listPatPlansForPat,listInsSubsForFam);
					Procedures.ComputeEstimatesForAll(pat.PatNum,listClaimProcs,listProcedures,listInsPlansForFam,listPatPlansForPat,listBenefits,pat.Age,listInsSubsForFam);
				}
		...
		private void FillInsData() {
			...
			gridIns.ListGridRows.Add(row);
			//Benefits-----------------------------------------------------------------------------------------------------
			List <Benefit> listBenefitsForPat=_loadData.ListBenefits;
			Benefit[,] benefitMatrix=Benefits.GetDisplayMatrix(listBenefitsForPat,_listPatPlans,_listInsSubs);
			string strDesc;
			string strVal;
			...
			for(int y=0;y<benefitMatrix.GetLength(1);y++) {//rows
				...
				for(int x=0;x<benefitMatrix.GetLength(0);x++) {//columns
					...
					else if(Benefits.IsExamFrequency(benefitMatrix[x,y]) && !gridIns.ListGridRows.Any(x => x.Cells[0].Text.Contains(Lan.g(this,"Exam frequency")+" "))) {
						strDesc+=Lan.g(this,"Exam frequency")+" ";
						hasSpecialFreqAdded=true;
					}
					else if(Benefits.IsBitewingFrequency(benefitMatrix[x,y]) 
						&& !gridIns.ListGridRows.Any(x => x.Cells[0].Text.Contains(Lan.g(this,"BW frequency")+" "))
						&& (benefitMatrix[x,y].CodeNum==ProcedureCodes.GetCodeNumForPref(PrefName.InsBenBWCodes) 
							|| listBenefitsForPat.FirstOrDefault(x => ProcedureCodes.GetCodeNumForPref(PrefName.InsBenBWCodes)==x.CodeNum)==null)) 
					{
						strDesc+=Lan.g(this,"BW frequency")+" ";
						hasSpecialFreqAdded=true;
					}
					else if(Benefits.IsPanoFrequency(benefitMatrix[x,y]) 
						&& !gridIns.ListGridRows.Any(x => x.Cells[0].Text.Contains(Lan.g(this,"Pano/FMX frequency")+" "))
						&& (benefitMatrix[x,y].CodeNum==ProcedureCodes.GetCodeNumForPref(PrefName.InsBenPanoCodes) 
							|| listBenefitsForPat.FirstOrDefault(x => ProcedureCodes.GetCodeNumForPref(PrefName.InsBenPanoCodes)==x.CodeNum)==null)) 
					{
						strDesc+=Lan.g(this,"Pano/FMX frequency")+" ";
						hasSpecialFreqAdded=true;
					}
					else if(Benefits.IsCancerScreeningFrequency(benefitMatrix[x,y]) 
						&& !gridIns.ListGridRows.Any(x => x.Cells[0].Text.Contains(Lan.g(this,"Cancer Screening frequency")+" "))
						&& (benefitMatrix[x,y].CodeNum==ProcedureCodes.GetCodeNumForPref(PrefName.InsBenCancerScreeningCodes) 
							|| listBenefitsForPat.FirstOrDefault(x => ProcedureCodes.GetCodeNumForPref(PrefName.InsBenCancerScreeningCodes)==x.CodeNum)==null)) 
					{
						strDesc+=Lan.g(this,"Cancer Screening frequency")+" ";
						hasSpecialFreqAdded=true;
					}
					else if(Benefits.IsProphyFrequency(benefitMatrix[x,y]) 
						&& !gridIns.ListGridRows.Any(x => x.Cells[0].Text.Contains(Lan.g(this,"Prophy frequency")+" "))
						&& (benefitMatrix[x,y].CodeNum==ProcedureCodes.GetCodeNumForPref(PrefName.InsBenProphyCodes) 
							|| listBenefitsForPat.FirstOrDefault(x => ProcedureCodes.GetCodeNumForPref(PrefName.InsBenProphyCodes)==x.CodeNum)==null)) 
					{
						strDesc+=Lan.g(this,"Prophy frequency")+" ";
						hasSpecialFreqAdded=true;
					}
					else if(Benefits.IsFlourideFrequency(benefitMatrix[x,y]) 
						&& !gridIns.ListGridRows.Any(x => x.Cells[0].Text.Contains(Lan.g(this,"Fluoride frequency")+" "))
						&& (benefitMatrix[x,y].CodeNum==ProcedureCodes.GetCodeNumForPref(PrefName.InsBenFlourideCodes) 
							|| listBenefitsForPat.FirstOrDefault(x => ProcedureCodes.GetCodeNumForPref(PrefName.InsBenFlourideCodes)==x.CodeNum)==null)) 
					{
						...
						&& !gridIns.ListGridRows.Any(x => x.Cells[0].Text.Contains(Lan.g(this,"Sealant frequency")+" "))
						&& (benefitMatrix[x,y].CodeNum==ProcedureCodes.GetCodeNumForPref(PrefName.InsBenSealantCodes) 
							|| listBenefitsForPat.FirstOrDefault(x => ProcedureCodes.GetCodeNumForPref(PrefName.InsBenSealantCodes)==x.CodeNum)==null)) 
					{
						...
						&& !gridIns.ListGridRows.Any(x => x.Cells[0].Text.Contains(Lan.g(this,"Crown frequency")+" "))
						&& (benefitMatrix[x,y].CodeNum==ProcedureCodes.GetCodeNumForPref(PrefName.InsBenCrownCodes) 
							|| listBenefitsForPat.FirstOrDefault(x => ProcedureCodes.GetCodeNumForPref(PrefName.InsBenCrownCodes)==x.CodeNum)==null)) 
					{
						strDesc+=Lan.g(this,"Crown frequency")+" ";
						hasSpecialFreqAdded=true;
					}
					else if(Benefits.IsSRPFrequency(benefitMatrix[x,y]) 
						&& !gridIns.ListGridRows.Any(x => x.Cells[0].Text.Contains(Lan.g(this,"SRP frequency")+" "))
						&& (benefitMatrix[x,y].CodeNum==ProcedureCodes.GetCodeNumForPref(PrefName.InsBenSRPCodes) 
							|| listBenefitsForPat.FirstOrDefault(x => ProcedureCodes.GetCodeNumForPref(PrefName.InsBenSRPCodes)==x.CodeNum)==null)) 
					{
						strDesc+=Lan.g(this,"SRP frequency")+" ";
						hasSpecialFreqAdded=true;
					}
					else if(Benefits.IsFullDebridementFrequency(benefitMatrix[x,y]) 
						&& !gridIns.ListGridRows.Any(x => x.Cells[0].Text.Contains(Lan.g(this,"Full Debridement frequency")+" "))
						&& (benefitMatrix[x,y].CodeNum==ProcedureCodes.GetCodeNumForPref(PrefName.InsBenFullDebridementCodes) 
							|| listBenefitsForPat.FirstOrDefault(x => ProcedureCodes.GetCodeNumForPref(PrefName.InsBenFullDebridementCodes)==x.CodeNum)==null)) 
					{
						strDesc+=Lan.g(this,"Full Debridement frequency")+" ";
						hasSpecialFreqAdded=true;
					}
					else if(Benefits.IsPerioMaintFrequency(benefitMatrix[x,y]) 
						&& !gridIns.ListGridRows.Any(x => x.Cells[0].Text.Contains(Lan.g(this,"Perio Maint frequency")+" "))
						&& (benefitMatrix[x,y].CodeNum==ProcedureCodes.GetCodeNumForPref(PrefName.InsBenPerioMaintCodes) 
							|| listBenefitsForPat.FirstOrDefault(x => ProcedureCodes.GetCodeNumForPref(PrefName.InsBenPerioMaintCodes)==x.CodeNum)==null)) 
					{
						strDesc+=Lan.g(this,"Perio Maint frequency")+" ";
						hasSpecialFreqAdded=true;
					}
					else if(Benefits.IsDenturesFrequency(benefitMatrix[x,y]) 
						&& !gridIns.ListGridRows.Any(x => x.Cells[0].Text.Contains(Lan.g(this,"Dentures frequency")+" "))
						&& (benefitMatrix[x,y].CodeNum==ProcedureCodes.GetCodeNumForPref(PrefName.InsBenDenturesCodes) 
							|| listBenefitsForPat.FirstOrDefault(x => ProcedureCodes.GetCodeNumForPref(PrefName.InsBenDenturesCodes)==x.CodeNum)==null)) 
					{
						strDesc+=Lan.g(this,"Dentures frequency")+" ";
						hasSpecialFreqAdded=true;
					}
					else if(Benefits.IsImplantFrequency(benefitMatrix[x,y]) 
						&& !gridIns.ListGridRows.Any(x => x.Cells[0].Text.Contains(Lan.g(this,"Implants frequency")+" "))
						&& (benefitMatrix[x,y].CodeNum==ProcedureCodes.GetCodeNumForPref(PrefName.InsBenImplantCodes) 
							|| listBenefitsForPat.FirstOrDefault(x => ProcedureCodes.GetCodeNumForPref(PrefName.InsBenImplantCodes)==x.CodeNum)==null)) 
					{
			...
			Dictionary<long,InsProcHist> dictInsProcHist=_listPatPlans.Select(x => x.InsSubNum).Distinct()
				.ToDictionary(x => x,x => new InsProcHist(Procedures.GetDictInsHistProcs(_patient.PatNum,x,out List<ClaimProc> listClaimProcs),listClaimProcs));
			foreach(PrefName prefName in Prefs.GetInsHistPrefNames()) {
				...
				foreach(PatPlan patPlan in _listPatPlans) {
					...
					if(dictInsProcHist.TryGetValue(patPlan.InsSubNum,out InsProcHist insProcHist)
						&& insProcHist.DictInsHistProcs.TryGetValue(prefName,out Procedure procedure)
						&& procedure!=null
						&& insProcHist.ListClaimProcs
							.Exists(x => x.InsSubNum==patPlan.InsSubNum && x.Status.In(ClaimProcStatus.InsHist,ClaimProcStatus.Received) && x.ProcNum==procedure.ProcNum))
					{
		...
		private void ToolButIns_Click() {
			...
			if(listInsSubs.Count==0) {
				...
				using FormInsSelectSubscr formInsSelectSubscr=new FormInsSelectSubscr(patientSubscriber.PatNum,_patient.PatNum);
		...
		private void FillGridPatientClones() {
			...
			foreach(KeyValuePair<Patient,Def> cloneAndSpecialty in _dictCloneSpecialty) {
				...
				if(_patient!=null && cloneAndSpecialty.Key.PatNum==_patient.PatNum) {
		...
		private void FillPatientPicture() {
			...
			try {
				...
				if(_loadData.HasPatPict==YN.Unknown) {
					pictureBoxPat.Image=Documents.GetPatPict(_patient.PatNum,ImageStore.GetPatientFolder(_patient,ImageStore.GetPreferredAtoZpath()));
				}
				else {
					pictureBoxPat.Image=Documents.GetPatPict(_patient.PatNum,ImageStore.GetPatientFolder(_patient,ImageStore.GetPreferredAtoZpath()),_loadData.PatPict);
				}
			}
			catch {
			}
		...
		private bool MovePats(Patient patientOld,Family family=null) {
			...
			if(family!=null) {//Move all family members marked as merged silently (The family should only contain guarantor and any merged pats at this point)
				foreach(Patient pat in family.ListPats) {
					if(pat.PatNum==patientOld.PatNum) {
						continue;//Don't move current pat yet
					}
					Patient patientOldFam=pat.Copy();
					pat.Guarantor=patientInNewFam.Guarantor;
					pat.SuperFamily=patientInNewFam.SuperFamily;
					Patients.Update(pat,patientOldFam);
				}
			}
			if(_patient.SuperFamily!=patientInNewFam.SuperFamily) {//If they are moving into or out of a superfamily
		...
		private void RefreshModuleScreen() {
			if(_patient!=null){//if there is a patient
				...
				bool showPanelForPatientClone=PrefC.GetBool(PrefName.ShowFeaturePatientClone) && _dictCloneSpecialty!=null && _dictCloneSpecialty.Count > 1;
		...
		private void UpdateConflictingOrdinals(PatPlan patPlan) {
			...
			foreach(PatPlan plan in listPatPlansConflictingOrdinal) {
				...
				plan.Ordinal=byteNonConflictingOrdinalNum;
