      ...
      return true;
		...
		private void FillGrid() {
			...
			foreach(PayPlanExtended payPlanCur in _listPayPlanExtended) {
				...
				if(PrefC.HasClinicsEnabled //Before checking the clinic combobox, make sure clinics are actually enabled.
					&& !comboClinics.IsAllSelected //If 'All' is not selected AND the selected clinic num(s) do not contain the clinic num of this plan, skip over this plan.
