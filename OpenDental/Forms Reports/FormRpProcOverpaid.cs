		...
		private void FillGrid() {
			...
			for(int i = 0;i<_myReport.ReportObjects.Count;i++) {
				...
				for(int j = 0;j<queryObj.ReportTable.Rows.Count;j++) {
					...
					if(checkExcludeOverpaidWithIns.Checked) {//Checkbox for filtering out procs with no insurance or writeoff amount
						double insPaid=PIn.Double(rowCur["insPaid"].ToString());
						double writeoff=PIn.Double(rowCur["wo"].ToString());
						if(CompareDouble.IsZero(insPaid) && CompareDouble.IsZero(writeoff)) {//Exclude from list when insurance and writeoff are 0
							continue;
						}
					}
					row=new GridRow();
