...
namespace OpenDental {
	public class WebFormL {
		...
		public static string RetrieveAndSaveData(string cultureName,List<long> listClinicNums) {
			string strMsg="";
			...
			for(int i=0;i<listSheetsFromCemtTool.Count;i++) {
				...
				if(ListTools.In(listSheetsFromCemtTool[i].SheetNum,listCemtSheetsToDelete)) {
					continue;
				}
				try {
					if(!DidImportSheet(null,listSheetsFromCemtTool[i],null,listSheetsFromCemtTool,System.Globalization.CultureInfo.CurrentCulture.Name,
			...
			try {
				...
				return Lan.g("FormWebForms","There was an error retrieving the webforms.")+"\r\n"+strMsg+"\r\n"+MiscUtils.GetExceptionText(e);
			}
			if(progressOD.IsCancelled) {
				return "";
			}
			if(listWebFormSheets.Count==0 && listSheetsFromCemtTool.Count==0) {
				return Lan.g("FormWebForms","No Patient forms retrieved from server.")+"\r\n"+strMsg;
			}
			List<long> listSheetIdsForDeletion=new List<long>();
			for(int i=0;i<listWebFormSheets.Count;i++) {
				if(listSheetIdsForDeletion.Contains(listWebFormSheets[i].SheetID)) {
				...
				try { //this try catch is put so that a defective downloaded sheet does not stop other sheets from being downloaded.
					if(!DidImportSheet(listWebFormSheets[i],null,listWebFormSheets,null,cultureName,ref listSheetIdsForDeletion,patientImportChoice)) {
						//user wants to cancel import.
						return strMsg;
					}
				}
