...
namespace OpenDental {
	public class SecurityL {
		...
		public static bool ChangePassword(bool isForcedLogOff,bool doRefreshSecurityCache=true) {
			...
			bool isPasswordStrong=FormU.IsPasswordStrong;
			try {
				Userods.UpdatePassword(Security.CurUser,FormU.PasswordContainer_,isPasswordStrong);
			}
			catch(Exception ex) {
				MessageBox.Show(ex.Message);
				return false;
			}
			Security.CurUser.PasswordIsStrong=FormU.IsPasswordStrong;
			Security.CurUser.LoginDetails=FormU.PasswordContainer_;
