...
namespace OpenDental {
	public class EtransL {
		...
		public static void ImportInsurancePlans(X834 x834,List<Patient> listPatients,bool doCreatePatient,bool doCreateEmployer,bool doDropExistingInsurance,
			...
			out int updatedPatPlanCount,out int createdEmployerCount,out StringBuilder stringBuilderErrorMessages,Action<int,Patient> showStatus=null) 
		{
			int rowIndex=1;
			createdPatsCount=0;
			updatedPatsCount=0;
			...
			stringBuilderErrorMessages=new StringBuilder();
			...
			for(int i=0;i<x834.ListTransactions.Count;i++) {
				Hx834_Tran hx834_trans=x834.ListTransactions[i];
				for(int j=0;j<hx834_trans.ListMembers.Count;j++) {
					Hx834_Member hx834_Member=hx834_trans.ListMembers[j];
					showStatus?.Invoke(rowIndex,hx834_Member.Pat);
					...
					if(hx834_Member.Pat.PatNum==0) {
						...
						List <Patient> listPatientsMatches=Patients.GetPatientsByNameAndBirthday(hx834_Member.Pat,listPatients);
						if(listPatientsMatches.Count==1) {
							hx834_Member.Pat.PatNum=listPatientsMatches[0].PatNum;
						}
						else if(listPatientsMatches.Count > 1) {
							isMultiMatch=true;
						}
					}
					if(isMultiMatch) {
						...
						Patient patientMemberOld=hx834_Member.Pat.Copy();
						...
						Patients.Update(hx834_Member.Pat,patientMemberOld);
						int patIdx=listPatients.BinarySearch(hx834_Member.Pat);//Preserve sort order by locating the index in which to insert the newly added patient.
						...
						listPatients.Insert(insertIdx,hx834_Member.Pat);
						...
						Patient patientDb=listPatients.FirstOrDefault(x => x.PatNum==hx834_Member.Pat.PatNum);//Locate by PatNum, in case user selected manually.
						if(patientDb==null) {
							listPatients=Patients.GetAllPatients();
						}
						listPatients.FirstOrDefault(x => x.PatNum==hx834_Member.Pat.PatNum);//If its still null, the patient really doesn't exist in the DB.
						if(patientDb!=null) {
							hx834_Member.MergePatientIntoDbPatient(patientDb);//Also updates the patient to the database and makes log entry.
							listPatients.Remove(patientDb);//Remove patient from list so we can add it back in the correct location (in case name or bday changed).
							int patIdx=listPatients.BinarySearch(patientDb);//Preserve sort order by locating the index in which to insert the newly added patient.
							...
							listPatients.Insert(insertIdx,patientDb);
							isMemberImported=true;
							updatedPatsCount++;
						}
					}
					if(isMemberImported) {
						...
						for(int k=0;k<hx834_Member.ListHealthCoverage.Count;k++) {
							...
							List<Carrier> listCarriers=Carriers.GetByNameAndTin(hx834_trans.Payer.Name,hx834_trans.Payer.IdentificationCode);							
							if(listCarriers.Count==0) {
								Carrier carrier=new Carrier();
								carrier.CarrierName=hx834_trans.Payer.Name;
								carrier.TIN=hx834_trans.Payer.IdentificationCode;
							...
							for(int p=0;p<listInsSubs.Count;p++) {
								...
								if(insSub.SubscriberID.Trim()!=hx834_Member.SubscriberId.Trim()
									|| carrier.CarrierName.Trim().ToLower()!=hx834_trans.Payer.Name.Trim().ToLower())
								{
									continue;
								}
								insSubMatch=insSub;
						...
						if(doDropExistingInsurance && !listPatPlansNumsToRemove.IsNullOrEmpty()) {
							...
							foreach(PatPlan plan in listPatPlansNumsToRemove) {
						...
						int endSegIndex=0;
						if(j<hx834_trans.ListMembers.Count-1) {
							endSegIndex=hx834_trans.ListMembers[j+1].MemberLevelDetail.SegmentIndex-1;
						}
						else {
		...
		///the patient is the first parameter to the action. The second is the current patient.</param>
		public static void ImportInsurancePlansUsingPatientLimited(X834 x834,List<PatientFor834Import> listPatientsFor834ImportsLimited,bool doCreatePatient,bool doCreateEmployer,bool doDropExistingInsurance,
			out int createdPatsCount,out int updatedPatsCount,out int skippedPatsCount,out int createdCarrierCount,out int createdInsPlanCount,
			out int updatedInsPlanCount,out int createdInsSubCount,out int updatedInsSubCount,out int createdPatPlanCount,out int droppedPatPlanCount,
			out int updatedPatPlanCount,out int createdEmployerCount,out StringBuilder stringBuilderErrorMessages,Action<int,Patient> actionShowStatus = null) {
			int rowIndex=1;
			createdPatsCount=0;
			updatedPatsCount=0;
			skippedPatsCount=0;
			...
			for(int i = 0;i<x834.ListTransactions.Count;i++) {
				...
				for(int j = 0;j<hx834_Tran.ListMembers.Count;j++) {
					...
					if(isMultiMatch) {
						...
						PatientFor834Import patientFor834ImportLimited = (PatientFor834Import)hx834_Member.Pat;
