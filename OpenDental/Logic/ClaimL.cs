...
namespace OpenDental {
	public class ClaimL {
		...
		public static void PromptForSecondaryClaim(List<ClaimProc> _listClaimProcsForClaim) {
			...
			listInputBoxParam.Add(new InputBoxParam(InputBoxType.RadioButton,msg,Lan.g("ContrAccount","Change the claim status to 'Waiting to send'"),Size.Empty));
			listInputBoxParam.Add(new InputBoxParam(InputBoxType.RadioButton,"",Lan.g("ContrAccount","Send secondary claim(s) now"),Size.Empty));
			if(!PrefC.GetBool(PrefName.ClaimPrimaryRecievedForceSecondaryStatus)) {
				listInputBoxParam.Add(new InputBoxParam(InputBoxType.RadioButton,"",Lan.g("ContrAccount","Do nothing"),Size.Empty));
			}
    ...
    public static bool IsClaimProcGreaterThanProcFee(List<ClaimProc> listClaimProcsHypothetical) {
      ...
      if(claimProcCreditsGreaterThanProcFee==ClaimProcCreditsGreaterThanProcFee.Allow) {
			...
			List<string> listProcDescripts=Claims.GetClaimProcGreaterThanProcFee(listClaimProcsHypothetical);
		...
		public static bool IsWriteOffGreaterThanProcFee(List<ClaimProc> listClaimProcsHypothetical) {
			...
			List<string> listProcDescripts=Claims.GetWriteOffGreaterThanProcFee(listClaimProcsHypothetical);
			if(!listProcDescripts.IsNullOrEmpty()) {
