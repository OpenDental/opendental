...
";
		...
		private string ConvertSignatureBoxWrapper(System.Windows.Forms.Control control,int indentLevel){
			...
			string s=GetIndents(indentLevel)+"<"+wName+" ";
			s+=SetBoundsAndAnchoring(control);
			s+="HorizontalAlignment=\"Left\" ";//this is not one of our custom controls, so we have to specify both alignments.
			s+="VerticalAlignment=\"Top\" ";
			_issues+="For "+control.Name+", add the following line to the xaml namespace area: xmlns:oduiodb=\"clr-namespace:OpenDental.UI;assembly=OpenDentBusiness\"\r\n";
			s+=">\r\n";
			s+=GetIndents(indentLevel+1)+"<oduiodb:SignatureBoxWrapper ";
			s+="x:Name=\""+control.Name+"\" ";
			s+="/>\r\n";
			s+=GetIndents(indentLevel)+"</"+wName+">\r\n";
			return s;
