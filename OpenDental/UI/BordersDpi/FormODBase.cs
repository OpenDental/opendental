...
namespace OpenDental {
	...
	public class FormODBase : Form {
		...
		///<summary>To use the UIManager, set this to true just before calling InitializeLayoutManager.</summary>
		...
		protected override void OnLoad(EventArgs e){
			...
			if(UsingUIManager) {
				...
				_uIManager.SetScaleMS((float)dpiScreen0/96f);//example 1.5
				if(!isMaximized){
					int widthNew = _uIManager.Scale(Width);//includes Zoom
					int heightNew = _uIManager.Scale(Height);
...
//todo: close when this form closes.
		...
		///<summary>This method or InitializeLayoutManager must be present on every form that derives from ODForm.  It should be immediately after InitializeComponent.  UIManager is responsible for drawing all controls and handling all events.</summary>
		protected void InitializeUIManager(bool isDpiSystem=false){
			UsingUIManager=true;
			//Can't do this on load because controls already have handles by that point.
			IsDpiSystem=isDpiSystem;
			if(ODBuild.IsDebug() && Environment.MachineName.ToLower()=="jordanhome"){
				//IsDpiSystem=true;//but this only works if we also set Dpi.SetUnaware(); in ProgramEntry, like in OD proper.
			}
			_uIManager=new UIManager(this,IsDpiSystem);
		}
		...
		private void PanelBorders_Paint(object sender, PaintEventArgs e){
			...
			Font font2=new Font("Microsoft Sans Serif",ScaleFontODZoom(8.25f));
			if(HasHelpButton){
				if(_isHotHelp){
					g.SmoothingMode=SmoothingMode.HighQuality;
				...
				g.DrawString("?",font2,brushText,_rectangleButHelp.X+Scale(7),_rectangleButHelp.Y+Scale(5));
			}
			...
			if(_fontTitle==null){
				float heightFont=ScaleMS(font2.Height);
				...
				using Font font=new Font("Microsoft Sans Serif",ScaleFontODZoom(13f));
				float heightFont=ScaleF(_fontTitle.Height);
		...
		private void PanelBorders_MouseMove(object sender, MouseEventArgs e){
			...
			if(rectangleNew!=Rectangle.Empty){
				...
				if(UsingUIManager){
					...
					if(rectangleNew.Height<LayoutManager.HeightTitleBar()+widthBorder){
						rectangleNew.Height=LayoutManager.HeightTitleBar()+widthBorder;
					}
				}
...
Application.DoEvents();//Without this, there are huge drag artifacts, especially dragging top downward
		...
		private int GetHeightTitleBar(){
			...
			return LayoutManager.HeightTitleBar();
		}
...
//todo: We shouldn't be changing the rectangle: https://learn.microsoft.com/en-us/windows/win32/hidpi/high-dpi-desktop-application-development-on-windows
			...
			if (m.Msg==WM_NCCALCSIZE){//client area calculation. We always fill the entire window.
				if(UsingUIManager) {
					...
					rectangle=new Rectangle(rectangle.X+8,rectangle.Y,rectangle.Width-16,rectangle.Height-8);
