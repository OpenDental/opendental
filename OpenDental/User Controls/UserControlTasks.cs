	...
	public partial class UserControlTasks:UserControl {
		...
		private void SetFiltersToDefault(TaskList taskListSelected=null) {
			...
			if(taskListSelected!=null) {//And if so, use its GlobalTaskFilterType override.
				_enumTaskFilterTypeForList=taskListSelected.GlobalTaskFilterType;
			}
		...
		///</summary>
		private void FillGrid(List<Signalod> listSignalods=null,bool isManualRefresh=false) {
			if(Security.CurUser==null 
			...
			for(int i=0;i<_listTasks.Count;i++) {
				...
				gridMain.ListGridRows.Add(gridRow);
				if(_taskClicked is Task && _listTasks[i].TaskNum==_taskClicked.TaskNum) {//_clickedTask can be a TaskList
		...
		private void FilterTaskList(ref List<TaskList> listTaskLists) {
		...
		private void Filter_Clicked() {
			...
			if(_taskClicked==null) {
				keyNum=0;
			}
			else {
				keyNum=_taskClicked.KeyNum;
				if(_taskClicked.ObjectType==TaskObjectType.Patient) {
					frmTaskFilter.PatientTask=Patients.GetPat(keyNum);
				}
				else if(_taskClicked.ObjectType==TaskObjectType.Appointment) {
		...
		private void Edit_Clicked() {
			if(_idxClicked < _listTaskLists.Count) {//is list
				using FormTaskListEdit formTaskListEdit=new FormTaskListEdit(_listTaskLists[_idxClicked]);
				formTaskListEdit.ShowDialog();
				long signalNum=Signalods.SetInvalid(InvalidType.TaskList,KeyType.Undefined,_listTaskLists[_idxClicked].Parent);//Signal for source parent tasklist.
				RefillLocalTaskGrids(_listTaskLists[_idxClicked],new List<long>() { signalNum });//No db call.
			}
			else {//task
				FormTaskEdit formTaskEdit=new FormTaskEdit(_taskClicked);//Handles signals for this task edit.
				formTaskEdit.Show();//non-modal
			}
		}
		...
		private void Cut_Clicked() {
			if(_idxClicked < _listTaskLists.Count) {//is list
				_taskListClip=_listTaskLists[_idxClicked].Copy();
				_taskClip=null;
			}
			else {//task
				_taskListClip=null;
				_taskClip=_taskClicked.Copy();
			}
			_wasCut=true;
		}
		...
		private void Delete_Clicked() {
			if(_idxClicked < _listTaskLists.Count) {//is list
				TaskList taskListToDelete=_listTaskLists[_idxClicked];
				//check to make sure the list is empty.  Do not filter tasks so we don't try to delete a list that still has tasks.
				List<Task> listTasks=Tasks.RefreshChildren(taskListToDelete.TaskListNum,true,DateTime.MinValue,Security.CurUser.UserNum,0,TaskType.All);
				List<TaskList> listTaskLists=TaskLists.RefreshChildren(taskListToDelete.TaskListNum,Security.CurUser.UserNum,0,TaskType.All);
				int countHiddenTasks=listTaskLists.Sum(x => x.NewTaskCount)+listTasks.Count-taskListToDelete.NewTaskCount;
				...
				if(_taskClicked.UserNum!=Security.CurUser.UserNum) {//current user didn't write this task, so block them.
					isTaskForCurUser=false;//Delete will only be enabled if the user has the TaskEdit and TaskNoteEdit permissions.
				}
				if(_taskClicked.TaskListNum!=Security.CurUser.TaskListInBox) {//the task is not in the logged-in user's inbox
					isTaskForCurUser=false;//Delete will only be enabled if the user has the TaskEdit and TaskNoteEdit permissions.
				}
				if(isTaskForCurUser) {
					List<TaskNote> listTaskNotes=TaskNotes.GetForTask(_taskClicked.TaskNum);//so we can check so see if other users have added notes
				...
				if(Tasks.GetOne(_taskClicked.TaskNum)==null) {
					MsgBox.Show(this,"Task already deleted.");
					return;
				}
				if(_taskClicked.TaskListNum==0) {
					Tasks.TaskEditCreateLog(Lan.g(this,"Deleted task"),_taskClicked);
				}
				else {
					string logText=Lan.g(this,"Deleted task from tasklist");
					TaskList taskList=TaskLists.GetOne(_taskClicked.TaskListNum);
					if(taskList!=null) {
					...
					logText+=".";
					Tasks.TaskEditCreateLog(logText,_taskClicked);
				}
				int countDocuments=TaskAttachments.GetCountDocumentForTaskNum(_taskClicked.TaskNum);
				Tasks.Delete(_taskClicked.TaskNum);//always do it this way to clean up all five tables (six if hq)
				if(countDocuments>0) { 
				...
				List<long> listSignalNums=new List<long>();
				listSignalNums.Add(Signalods.SetInvalid(InvalidType.TaskList,KeyType.Undefined,_taskClicked.TaskListNum));//Signal for source tasklist.
				listSignalNums.Add(Signalods.SetInvalid(InvalidType.Task,KeyType.Task,_taskClicked.TaskNum));//Signal for current task.
				RefillLocalTaskGrids(_taskClicked,_listTaskNotes.FindAll(x => x.TaskNum==_taskClicked.TaskNum),listSignalNums,false);
				TaskHist taskHist = new TaskHist(_taskClicked);
				taskHist.IsNoteChange=false;
				taskHist.UserNum=Security.CurUser.UserNum;
				TaskHists.Insert(taskHist);
				SecurityLogs.MakeLogEntry(EnumPermType.TaskDelete,0,"Task "+POut.Long(_taskClicked.TaskNum)+" deleted",0);
			}
		}
		...
		private void gridMain_CellClick(object sender,ODGridClickEventArgs e) {
			...
			if(_idxClicked < _listTaskLists.Count) {//is list
				_listTaskListTreeHistory.Add(_listTaskLists[_idxClicked]);
				_hasListSwitched=true;
				SetFiltersToDefault(_listTaskLists[_idxClicked]);//Fills Tree and Grid
				return;
			}
			_taskCollapsedState=-1;
			if(tabControl.SelectedTab==tabNew && !PrefC.GetBool(PrefName.TasksNewTrackedByUser)){//There's an extra column
				if(clickedCol==1) {
					TaskUnreads.SetRead(Security.CurUser.UserNum,_listTasks[_idxClicked-_listTaskLists.Count]);
					FillGrid();
				}
				if(clickedCol==3) {//Expand column
					if(_listTaskNumsExpanded.Contains(_listTasks[_idxClicked-_listTaskLists.Count].TaskNum)) {
						_listTaskNumsExpanded.Remove(_listTasks[_idxClicked-_listTaskLists.Count].TaskNum);
					}
					else { 
						_listTaskNumsExpanded.Add(_listTasks[_idxClicked-_listTaskLists.Count].TaskNum);
					}
					FillGrid();
				}
			...
			if(clickedCol==0){//check tasks off
				MarkRead(_listTasks[_idxClicked-_listTaskLists.Count]);
			}
		...
		private void SetClickedIAndTask(object e) {
			if(e is ODGridClickEventArgs) {
				_idxClicked=((ODGridClickEventArgs)e).Row;
			}
			else if(e is MouseEventArgs) {
				_idxClicked=gridMain.PointToRow(((MouseEventArgs)e).Y);
			}
			if(_idxClicked==-1){
				return;
			}
			if(_idxClicked>=gridMain.ListGridRows.Count) {//Grid refreshed mid-click and _clickedI is no longer valid.
				_idxClicked=-1;
				_taskClicked=null;
				SetMenusEnabled();
				return;
			}
			if(gridMain.ListGridRows[_idxClicked].Tag is Task) {
				_taskClicked=(Task)gridMain.ListGridRows[_idxClicked].Tag;//Task lists cause _clickedTask to be null
			}
			else {
				_taskClicked=null;
			}
		...
		private void SetMenusEnabled() {
			//Done----------------------------------
			if(gridMain.SelectedIndices.Length==0 || _idxClicked < _listTaskLists.Count) {//or a tasklist selected
			...
			if(gridMain.SelectedIndices.Length==0) {
				...
				if(_idxClicked < _listTaskLists.Count) {//Is a tasklist
			...
			else if(tabControl.SelectedTab==tabUser && _idxClicked<_listTaskLists.Count) {//user tab and is a list
				menuItemSubscribe.Enabled=false;
				menuItemUnsubscribe.Enabled=true;
			}
			else if(tabControl.SelectedTab==tabMain && _idxClicked < _listTaskLists.Count) {//main and tasklist
				menuItemSubscribe.Enabled=true;
				menuItemUnsubscribe.Enabled=false;
			}
			else if(tabControl.SelectedTab==tabReminders && _idxClicked < _listTaskLists.Count) {//reminders and tasklist
				menuItemSubscribe.Enabled=true;
				menuItemUnsubscribe.Enabled=false;
			}
			...
			if(gridMain.SelectedIndices.Length>0 && _idxClicked >= _listTaskLists.Count){//is task
				//The clicked task was removed from _listTasks, could happen between FillGrid(), mouse click, and now
				if(IsInvalidTaskRow(_idxClicked)) {
					IgnoreTaskClick();
					return;
				}
				Task task=_listTasks[_idxClicked-_listTaskLists.Count];
			...
			//Navigate to Job-------------------------------------------------------------
			if(gridMain.SelectedIndices.Length>0 && _idxClicked >= _listTaskLists.Count && PrefC.IsODHQ) {
				//The clicked task was removed from _listTasks, could happen between FillGrid(), mouse click, and now
				if(IsInvalidTaskRow(_idxClicked)) {
					IgnoreTaskClick();
					return;
			...
			if(_idxClicked > -1 && tabControl.SelectedTab==tabMain && _idxClicked < _listTaskLists.Count 
				&& _listTaskLists[_idxClicked].TaskListStatus==TaskListStatusEnum.Active)
			{
				menuArchive.Visible=true;
			}
			if(_idxClicked > -1 && tabControl.SelectedTab.In(tabUser,tabMain,tabReminders) && _idxClicked < _listTaskLists.Count
				&& _listTaskLists[_idxClicked].TaskListStatus==TaskListStatusEnum.Archived)
			{
				menuUnarchive.Visible=true;
			}
			if(_idxClicked<0) {//Not clicked on any row
				menuItemDone.Enabled=false;
				menuItemEdit.Enabled=false;
				menuItemCut.Enabled=false;
				menuItemCopy.Enabled=false;
		...
		private void IgnoreTaskClick() {
			gridMain.SetSelected(_idxClicked,false);//unselect problem row
			_idxClicked=-1;//since row is unselected		
			foreach(MenuItem menuItem in gridMain.ContextMenu.MenuItems) { //disable ContextMenu options
				menuItem.Enabled=false;
			}
			Signalods.SetInvalid(InvalidType.TaskList,KeyType.Undefined,_taskClicked.TaskListNum);
		...
		private void OnSubscribe_Click(){
			...
			if(TaskSubscriptions.TrySubscList(_listTaskLists[_idxClicked].TaskListNum,Security.CurUser.UserNum)) {
				lock(_listTaskListNumsSubscribed) {
					_listTaskListNumsSubscribed=GetSubscribedTaskLists(Security.CurUser.UserNum).Select(x => x.TaskListNum).ToList();
				}
			}
			...
			RefillLocalTaskGrids(_listTaskLists[_idxClicked],null);
		}

		private void OnUnsubscribe_Click() {
			TaskSubscriptions.UnsubscList(_listTaskLists[_idxClicked].TaskListNum,Security.CurUser.UserNum);
			lock(_listTaskListNumsSubscribed) {
				_listTaskListNumsSubscribed=GetSubscribedTaskLists(Security.CurUser.UserNum).Select(x => x.TaskListNum).ToList();
			};
			RefillLocalTaskGrids(_listTaskLists[_idxClicked],null);
		...
		private void menuDeleteTaken_Click(object sender,EventArgs e) {
			Cursor=Cursors.WaitCursor;
			TaskTakens.DeleteForTask(_taskClicked.TaskNum);
			Cursor=Cursors.Default;
		...
		private void menuArchive_Click(object sender,EventArgs e) {
			...
			TaskLists.Archive(_listTaskLists[_idxClicked]);
			long signalNum=Signalods.SetInvalid(InvalidType.TaskList,KeyType.Undefined,_listTaskLists[_idxClicked].Parent);//Signal for source parent tasklist.
			RefillLocalTaskGrids(_listTaskLists[_idxClicked],new List<long>() { signalNum });//No db call.
		}
		...
		private void menuUnarchive_Click(object sender,EventArgs e) {
			...
			TaskLists.Unarchive(_listTaskLists[_idxClicked]);
			long signalNum=Signalods.SetInvalid(InvalidType.TaskList,KeyType.Undefined,_listTaskLists[_idxClicked].Parent);//Signal for source parent tasklist.
			RefillLocalTaskGrids(_listTaskLists[_idxClicked],new List<long>() { signalNum });//No db call.
		}
