...
namespace OpenDental
{
	...
	public class ContrPerio : System.Windows.Forms.Control{
		...
		public AutoAdvanceDirection AutoAdvanceDirection_;
					...
					for(int toothI=1;toothI<PerioMeasures.List.GetLength(2);toothI++){//measurements
						...
						for(int surfI=0;surfI<Enum.GetNames(typeof(PerioSurf)).Length;surfI++){//surfaces(6or7)
							if(seqI==(int)PerioSequenceType.SkipTooth){
								...
								perioCellSelected=GetPerioCell(colRowSelected,false);
							...
							if(seqI==(int)PerioSequenceType.Probing){
								CalculateCAL(colRowSelected,false);
							}
						}//for surfI
					}//for toothI
				}//for seqI
		...
		public ColRow GetAutoAdvanceCustomColRow(int intTooth,PerioSurface perioSurface,PerioSequenceType perioSequenceType,bool isReverse) {
			int x=-1;
			int y=-1;
			bool isFacial=perioSurface==PerioSurface.Facial;
			if(isFacial) {
			...
			y=GetTableRow(IdxExamSelected,GetSection(intTooth,perioSurface),perioSequenceType);
		...
		public void SaveCurExam(PerioExam perioExam) {
			...
			//Bleeding as well.
			List<PerioMeasureItem> listPerioMeasureItemsBleeding=ListPerioMeasureItemsChanged.Where(x => x.SeqType==PerioSequenceType.Probing)
			...
			foreach(PerioMeasureItem measureItem in ListPerioMeasureItemsChanged) {
				PerioSequenceType perioSequenceType=measureItem.SeqType;
		...
		public int GetSection(int intTooth,PerioSurface perioSurface) {
			int section;
			if(intTooth<=16) {
		...
		protected override void OnKeyDown(KeyEventArgs e)
		{
			...
			if(e.KeyValue>=96 && e.KeyValue<=105){//keypad 0 through 9
				...
				if(ThreeAtATime){
					for(int i=0;i<3;i++){
						if(AutoAdvanceDirection_==AutoAdvanceDirection.Right) {
							AdvanceCell();
						}
						else if(AutoAdvanceDirection_==AutoAdvanceDirection.Custom) {
		...
		private void EnterValue(int keyValue){
			...
			SetHasChanged(ColRowSelected.Col,ColRowSelected.Row);
			if(_perioSequenceTypeArray[GetSection(ColRowSelected.Row)][GetSectionRow(ColRowSelected.Row)]==PerioSequenceType.Probing){ 
				CalculateCAL(ColRowSelected,true);
			}
				...
				CalculateCAL(new ColRow(ColRowSelected.Col,GetTableRow
					(_idxExamSelected,GetSection(ColRowSelected.Row),PerioSequenceType.Probing)),true);
			}
			AdvanceCell();
		}
		...
		private PerioCell GetPerioCell(ColRow colRow,bool setText) {
			...
			if(colRow.Row>=12 && colRow.Row<=30) {
				perioCell.Surface=PerioSurface.Lingual;
			}
			else {
				perioCell.Surface=PerioSurface.Facial;
			}
			if(colRow.Row<=20) {
			...
			if(perioCell.Surface==PerioSurface.Facial) {
		...
		private void AdvanceCell(bool isReverse){
			if(!TryFindNextCell(ColRowSelected,isReverse,out ColRow colRowToSet)) {
				return;
			}
		...
		public bool TryFindNextCell(ColRow colRow,bool isReverse,out ColRow colRowNext,bool setDirection=true) {
			colRowNext=colRow;
			...
			while(limit<400){//the 400 limit is just a last resort. Should always break manually.
				...
				if(AutoAdvanceDirection_==AutoAdvanceDirection.Custom) {//custom path
					return TryAutoAdvanceCustom(colRow,startedOnSkipped,isReverse,out colRowNext);
				}
				if((AutoAdvanceDirection_==AutoAdvanceDirection.Left && isReverse) || (AutoAdvanceDirection_==AutoAdvanceDirection.Right && !isReverse)) {//right
					if(!TryAutoAdvanceRight(colRowNext,section,setDirection,perioSequenceType,startedOnSkipped,ref count,out colRowNext)) {
						return false;
					}
				}
				//to the left
				else{//((DirectionIsRight && isBackspace) || !DirectionIsRight){
					if(!TryAutoAdvanceLeft(colRowNext,section,setDirection,perioSequenceType,out colRowNext)) {
						return false;
					}
				}
		...
		private bool TryAutoAdvanceRight(ColRow colRowSelected,int section,bool setDirection,PerioSequenceType perioSequenceType,bool startedOnSkipped,ref int count,
			out ColRow colRowNext) 
		{
			...
			if(colRowNext.Col==1 && !startedOnSkipped) {//if first column
				if(section==0 && _listSkippedTeeth.Contains(1) && intTooth==1) {
					...
					return false;//no jump.  This is the starting colrow.
				}
				else if(section==1) {
					newSection=3;
				...
				|| (section==3 && _listSkippedTeeth.Count==32)) //skipped all teeth and are on last row
			{
				...
				bool isMissingSection = true;
				for(int tooth = 17;tooth<=32;++tooth) {
					if(!_listSkippedTeeth.Contains(tooth)) {
						isMissingSection = false;
					}
				}
				if(section == 3 && isMissingSection) {
					return false;
				}
				else {
		...
		private bool TryAutoAdvanceLeft(ColRow colRowSelected,int section,bool setDirection,PerioSequenceType perioSequenceType,out ColRow colRowNext) {
			colRowNext=colRowSelected;
			int newRow=0;
			int newSection=section;
			if(colRowNext.Col==_perioCellArray.GetLength(0)-1) {//if last column
				if(section==0) {
					newSection=1;
					newRow=GetTableRow(_idxExamSelected,newSection,perioSequenceType);
					if(newRow!=-1 && setDirection) {
				...
				for(int toothI = 17;toothI<=32;++toothI) {
					if(!_listSkippedTeeth.Contains(toothI)) {
		...
		///<summary>Moves the current colrow to the next position.</summary>
		private bool TryAutoAdvanceCustom(ColRow colRowSelected,bool startedOnSkipped,bool isReverse,out ColRow colRowNext) {
			...
			int section=GetSection(colRowNext.Row);//used to test skipped teeth
			List<AutoAdvanceCustom> listAutoAdvanceCustomsPaths=AutoAdvanceCustom.GetCustomPaths();
			AutoAdvanceCustom autoAdvanceCustomPath=listAutoAdvanceCustomsPaths.FirstOrDefault(x => x.ToothNum==perioCellSelected.ToothNum && x.Surface==perioCellSelected.Surface);
			if(autoAdvanceCustomPath==null) {
			...
			AutoAdvanceCustom autoAdvanceCustomPathNext=GetAutoAdvanceCustomFromList(autoAdvanceCustomPath,getNext:true,perioSequenceType,listAutoAdvanceCustomsPaths);
			//Use the logical exclusive OR operator to figure out if we need the next colrow to be left or right.
			bool isAdvanceRight=isReverse ^ IsCustomAutoAdvanceRight(autoAdvanceCustomPath,autoAdvanceCustomPathNext,perioSequenceType);
			int countNotUsed=0;
			bool startedOnCellOne=colRowNext.Col==1;
			//Advance colrowNext to the next cell position.
			if(isAdvanceRight) {
				if(!TryAutoAdvanceLeft(colRowNext,section,false,perioSequenceType,out colRowNext) && !startedOnCellOne) {
					return false;
				}
			}
			else {
				if(!TryAutoAdvanceRight(colRowNext,section,false,perioSequenceType,startedOnSkipped,ref countNotUsed,out colRowNext) && !startedOnCellOne) {
					return false;
				}
			}
			...
			AutoAdvanceCustom autoAdvanceCustomPathPrev=GetAutoAdvanceCustomFromList(autoAdvanceCustomPath,getNext:false,perioSequenceType,listAutoAdvanceCustomsPaths);
			bool skipSameToothCheck=(startedOnCellOne && colRowNext.Col==1 && autoAdvanceCustomPath.Surface==PerioSurface.Lingual && autoAdvanceCustomPathPrev!=null)
				|| perioSequenceType==PerioSequenceType.Mobility;
			if(!skipSameToothCheck && perioCellSelected.ToothNum==perioCellNext.ToothNum && perioCellSelected.Surface==perioCellNext.Surface) {
				//Same tooth,different position. Return true. colorowNext was already set in TryAutoAdvanceLeft and TryAutoAdvanceRight
				return true;
			}
			//The colrow returned in TryAutoAdvanceLeft or TryAutoAdvanceRight is on a differnt tooth, get the next tooth from the listCustomPath.
			if(isReverse) {
				autoAdvanceCustomPathNext=autoAdvanceCustomPathPrev;//The next tooth should be the previous one. 
			}
			if(autoAdvanceCustomPathNext==null) {
			...
			colRowNext=GetAutoAdvanceCustomColRow(autoAdvanceCustomPathNext.ToothNum,autoAdvanceCustomPathNext.Surface,perioSequenceType,isReverse);
			return true;
		}
		...
		public AutoAdvanceCustom GetAutoAdvanceCustomFromList(AutoAdvanceCustom autoAdvanceCustomPath,bool getNext,PerioSequenceType perioSequenceType,
			List<AutoAdvanceCustom> listAutoAdvanceCustomsPaths) 
		{
			AutoAdvanceCustom autoAdvanceCustomPerioPath=getNext ? GetNext(autoAdvanceCustomPath,listAutoAdvanceCustomsPaths) : GetPrevious(autoAdvanceCustomPath,listAutoAdvanceCustomsPaths);
			if(autoAdvanceCustomPerioPath==null) {
				return null;//Most likely the last one
			}
			if((perioSequenceType==PerioSequenceType.MGJ && autoAdvanceCustomPerioPath.ToothNum.Between(1,16) || perioSequenceType==PerioSequenceType.Mobility) 
				&& autoAdvanceCustomPerioPath.Surface==PerioSurface.Lingual) 
			{
				//The perio chart does not have MGJ for Lingual teeth 1-16. Find the next available AutoAdvanceCustom from list.
				AutoAdvanceCustom autoAdvanceCustomPerioPathMGJ=autoAdvanceCustomPerioPath;
				return GetAutoAdvanceCustomFromList(autoAdvanceCustomPerioPathMGJ,getNext,perioSequenceType,listAutoAdvanceCustomsPaths);
			}
			//Check to make sure the perioPath is not marked as skipped.
			if(autoAdvanceCustomPerioPath!=null && _listSkippedTeeth.Contains(autoAdvanceCustomPerioPath.ToothNum)) {
				return GetAutoAdvanceCustomFromList(autoAdvanceCustomPerioPath,getNext,perioSequenceType,listAutoAdvanceCustomsPaths);
			}
			return autoAdvanceCustomPerioPath;
		}

		public static AutoAdvanceCustom GetNext(AutoAdvanceCustom autoAdvanceCustom,List<AutoAdvanceCustom> listAutoAdvanceCustomsPaths) {
			try {
				return listAutoAdvanceCustomsPaths.SkipWhile(x => !x.Equals(autoAdvanceCustom)).Skip(1).First();
			}
			catch {
				return null;
			}
		...
		public static AutoAdvanceCustom GetPrevious(AutoAdvanceCustom autoAdvanceCustom,List<AutoAdvanceCustom> listAutoAdvanceCustomsPaths) {
			try {
				return listAutoAdvanceCustomsPaths.TakeWhile(x => !x.Equals(autoAdvanceCustom)).Last();
			}
			catch {
				return null;
			}
		...
		public bool IsCustomAutoAdvanceRight(AutoAdvanceCustom autoAdvanceCustomPath,AutoAdvanceCustom autoAdvanceCustomPathNext,PerioSequenceType perioSequenceType) {
			//This method will need to change if we add other custom paths.
			if(autoAdvanceCustomPath==null) {
				return false;
			}
			int intTooth=autoAdvanceCustomPath.ToothNum;
			int toothNumNext=autoAdvanceCustomPathNext?.ToothNum??autoAdvanceCustomPath.ToothNum;
			PerioSurface perioSurface=autoAdvanceCustomPath.Surface;
			PerioSurface perioSurfaceNext=autoAdvanceCustomPathNext?.Surface??perioSurface;
			int section=GetSection(intTooth,perioSurface);
			int sectionNext=GetSection(toothNumNext,perioSurfaceNext);
			if(toothNumNext.Between(1,16)) {
				...
				if(perioSurfaceNext==PerioSurface.Facial || intTooth>=17 || section!=sectionNext) {
					return true;
				}
			}
				...
				if((perioSurfaceNext==PerioSurface.Lingual && perioSurface==perioSurfaceNext) || intTooth<=16) {
		...
		protected void OnDirectionChangedRight(){
			if(DirectionChangedRight != null && AutoAdvanceDirection_!=AutoAdvanceDirection.Custom){
				AutoAdvanceDirection_=AutoAdvanceDirection.Right;
		...
		protected void OnDirectionChangedLeft(){
			if(DirectionChangedLeft!=null && AutoAdvanceDirection_!=AutoAdvanceDirection.Custom) {
				AutoAdvanceDirection_=AutoAdvanceDirection.Left;
				EventArgs eArgs=new EventArgs();
	...
	public class AutoAdvanceCustom {
		...
		public AutoAdvanceCustom(int toothNum,PerioSurface perioSurface) {
			ToothNum=toothNum;
		...
		public static List<AutoAdvanceCustom> GetDefaultPath() {
			List<AutoAdvanceCustom> listAutoAdvanceCustomsPaths=new List<AutoAdvanceCustom>();
			//1-16F
			listAutoAdvanceCustomsPaths.AddRange(Enumerable.Range(1,16).Select(x => new AutoAdvanceCustom(x,PerioSurface.Facial)));
			//16-1L
			listAutoAdvanceCustomsPaths.AddRange(Enumerable.Range(1,16).OrderByDescending(x => x).Select(x => new AutoAdvanceCustom(x,PerioSurface.Lingual)));
			//32-17F
			listAutoAdvanceCustomsPaths.AddRange(Enumerable.Range(17,16).OrderByDescending(x => x).Select(x => new AutoAdvanceCustom(x,PerioSurface.Facial)));
			//17-32L
			listAutoAdvanceCustomsPaths.AddRange(Enumerable.Range(17,16).Select(x => new AutoAdvanceCustom(x,PerioSurface.Lingual)));
			return listAutoAdvanceCustomsPaths;
		}
		...
		public static List<AutoAdvanceCustom> GetCustomPaths() {
			...
			listAutoAdvanceCustomsPaths.AddRange(Enumerable.Range(1,16).Select(x => new AutoAdvanceCustom(x,PerioSurface.Facial)));
			//17-32F
			listAutoAdvanceCustomsPaths.AddRange(Enumerable.Range(17,16).Select(x => new AutoAdvanceCustom(x,PerioSurface.Facial)));
			//32-17L
			listAutoAdvanceCustomsPaths.AddRange(Enumerable.Range(17,16).OrderByDescending(x => x).Select(x => new AutoAdvanceCustom(x,PerioSurface.Lingual)));
			//16-1L
			listAutoAdvanceCustomsPaths.AddRange(Enumerable.Range(1,16).OrderByDescending(x => x).Select(x => new AutoAdvanceCustom(x,PerioSurface.Lingual)));
			return listAutoAdvanceCustomsPaths;
		}
