		...
		private void butProcess_Click(object sender,EventArgs e) {
			...
			foreach(NewCropCharge charge in _listNewCropCharges) {
				if(charge.repeatCharge==null) {//No such repeating charge exists yet for the given npi.
					...
					charge.repeatCharge.ChargeAmt=29;//$29/month is the cost for the basic plan. The user has to manually change the fee if it's supposed to be comprehensive.
					...
					try {
						procedure=RepeatCharges.AddProcForRepeatCharge(charge.repeatCharge,now,now,new OrthoCaseProcLinkingData(charge.repeatCharge.PatNum),isNewCropInitial:true);
					}
