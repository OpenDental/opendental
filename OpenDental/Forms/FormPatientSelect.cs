				...
				for(int f=0;f<_ListDisplayFields.Count;f++) {
					switch(_ListDisplayFields[f].InternalName) {
						...
						case "Clinic":
							...
							break;
						case "Birthdate":
							row.Cells.Add(Patients.DOBFormatHelper(PIn.Date(_DataTablePats.Rows[i]["Birthdate"].ToString())
		...
		private void PatSelected(){
			...
			if(PrefC.HasClinicsEnabled){
				...
				if(Security.IsAuthorized(Permissions.UnrestrictedSearch,true) 
					|| listUserClinicNums.Contains(patClinicNum)
					|| Appointments.GetAppointmentsForPat(patNumSelected).Select(x => x.ClinicNum).Any(x => listUserClinicNums.Contains(x))) 
				{
