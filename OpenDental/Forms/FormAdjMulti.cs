	...
	public partial class FormAdjMulti:FormODBase {
		...
		private void FormMultiAdj_Load(object sender,EventArgs e) {
			...
			for(int i=0;i<gridMain.ListGridRows.Count;i++) {
				...
				if(ListTools.In(((Procedure)gridMain.ListGridRows[i].Tag).ProcNum,_listProcNums)) {
		...
		private void FillGrid() {
			...
			for(int i=0;i<listAssignedProcAdjs.Count;i++) {
				...
				if(ListTools.In(creditCalcType,CreditCalcType.IncludeAll,CreditCalcType.AllocatedOnly)
					&& !ListTools.In(listAssignedProcAdjs[i].ProcNum,_listProcNums)
		...
		private List<ProcAdjs> GetProcAdjs() {
			...
			constructResults.ListAccountCharges.AddRange(_listAdjustments.Select(x => new AccountEntry(x)));
			...
			return constructResults.ListAccountCharges
				.FindAll(x => x.PatNum==_patient.PatNum
					&& (x.GetType()==typeof(Adjustment) && ListTools.In(x.Tag,_listAdjustments) || (x.GetType()==typeof(Procedure) && ((Procedure)x.Tag).ProcStatus==ProcStat.C)))
				.GroupBy(x => x.ProcNum)
		...
		private void butUpdate_Click(object sender,EventArgs e) {
			...
			for(int i=0;i<listAdjustments.Count;i++) {
				...
				if(procAdjs==null) {
					if(ListTools.In(GetAdjAmtType(),AdjAmtType.PercentOfFee,AdjAmtType.PercentOfRemBal)) {
		...
		private void butOK_Click(object sender,EventArgs e) {
			...
			bool hasNegAmt=constructResults.ListAccountCharges.Any(x => x.PatNum==_patient.PatNum
