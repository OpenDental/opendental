	...
	public partial class FormAdjMulti:FormODBase {
		...
		private void butUpdate_Click(object sender,EventArgs e) {
			...
			for(int i=0;i<listAdjustments.Count;i++) {
				...
				if(!Security.IsAuthorized(Permissions.AdjustmentCreate,dateAdjustment.Value,true) && listAdjustments[i].AdjAmt!=0) {
					hasSkipped=true;
					continue;
				}
		...
		private void butOK_Click(object sender,EventArgs e) {
			...
			if(!Security.IsAuthorized(Permissions.AdjustmentCreate,PIn.Date(dateAdjustment.Text),true) 
				||_listAdjustments.Any(x =>!Security.IsAuthorized(Permissions.AdjustmentCreate,x.AdjDate,true))) {//User does not have full edit permission.
