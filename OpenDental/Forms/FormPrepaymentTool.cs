...
namespace OpenDental {
	...
	public partial class FormPrepaymentTool:FormODBase {
		...
		private void CreateProcedureCharge(ProcedureCode procedureCode,int count,bool isNew=true) {
			...
			else if(listRepeatChargesForProc.Count<2) {
				return;
			}
			int remainder=count;
		...
		private void CreateProcedureLogs() {
			...
			for(int i=0;i<_listProcedureCodesPrePaySupport.Count();i++){
				...
				for(int j=0;j<listProcedureChargesMatching.Count;j++){
					listTransQtyAmt.Add(new AvaTax.TransQtyAmt(listProcedureChargesMatching[j].ProcCount,listProcedureChargesMatching[j].BaseFee));
				}
				dictTransactions.Add(_listProcedureCodesPrePaySupport[i],listTransQtyAmt);
			...
			try {
				AvaTax.CreatePrepaymentTransaction(dictTransactions,_patient,_listProceduresCompleted);
			}
			catch(Exception ex) {
		...
		private void butPreviousProc_Click(object sender,EventArgs e) {
			...
			for(int i=0;i<formProcSelect.ListSelectedProcs.Count();i++){
				//Use our list of ProcedureCodes so irrelevant codes result in a new ProcedureCode with blank values.
				ProcedureCode procedureCode=ProcedureCodes.GetProcCode(formProcSelect.ListSelectedProcs[i].CodeNum,_listProcedureCodesPrePaySupport);
				...
				ProcedureCharge procedureCharge=new ProcedureCharge(procedureCode,formProcSelect.ListSelectedProcs[i].ProcFee);
				Adjustment adjustmentTax=Adjustments.GetSalesTaxForProc(formProcSelect.ListSelectedProcs[i].ProcNum);
				...
				_listProceduresCompleted.Add(formProcSelect.ListSelectedProcs[i]);
			}
