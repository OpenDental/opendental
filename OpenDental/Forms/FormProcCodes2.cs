...
namespace OpenDental {
	...
	public partial class FormProcCodes:FormODBase {
		...
		private void FillComboBoxes() {
			...
			comboProvider1.Items.AddList(arrayProvAbbrs);
			comboProvider2.Items.AddList(arrayProvAbbrs);
			comboProvider3.Items.AddList(arrayProvAbbrs);
				...
				if(feeAmtNewStr=="") { //They want to delete the fee
					...
					{ 
						...
						if(isEditingGroup
							...
							&& selectedFeeSchedGroup.ListClinicNumsAll.Any(x => x!=clinicNum))
						{
							_listFees.RemoveAll(x => x.FeeSched==feeSched.FeeSchedNum && x.CodeNum==codeNum && selectedFeeSchedGroup.ListClinicNumsAll.Contains(x.ClinicNum) && x.ProvNum==provNum);
						}
					}
				}
		...
		private long GetClinicNumFromPicker(List<Clinic> listClinics) {
			...
			FormC.ShowDialog();
			return FormC.ClinicNumSelected;
		}
