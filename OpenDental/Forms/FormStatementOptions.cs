...
namespace OpenDental{
	...
	public partial class FormStatementOptions : FormODBase {
		...
		private bool SaveAsDocument(bool printSheet=false,string pdfFileName="",Sheet sheet=null,DataSet dataSet=null) {
			...
			StmtCur.DateSent=docc.DateCreated;
		...
		private void butDelete_Click(object sender,EventArgs e) {
			...
			try { 
				...
				List<Statement> listStatements=StmtList??new List<Statement>() {StmtCur};
		...
		private bool SaveToDb(){
			...
			if(StmtList==null){
				...
				if(checkBoxBillShowTransSinceZero.Checked) {
					...
					DataTable tableBals=Ledgers.GetDateBalanceBegan(patAges,checkSuperStatement.Checked);
				...
				if(checkBoxBillShowTransSinceZero.Checked) {
					...
					dictPatNumDateBalBegan=Ledgers.GetDateBalanceBegan(listPatAges,checkSuperStatement.Checked).Rows.OfType<DataRow>()
