	...
	public partial class FormInsVerificationList:FormODBase {
		...
		private void FillDisplayInfo(InsVerifyGridObject insVerifyGridObject) {
			...
			if(insVerifyGridObject.IsOnlyInsRow()) {
				textInsVerifyReadOnlyNote.Text=_gridRowSelected.InsVerifyPlan.Note;
			}
			else {//Both or Pat row
				textInsVerifyReadOnlyNote.Text=_gridRowSelected.InsVerifyPat.Note;
			}
		}
		...
		private List<InsVerifyGridRow> GetRowsForGrid(GridOD grid) {
			...
			progressOD.ActionMain=() => {
				List<InsVerifyGridObject> listInsVerifyGridObject=InsVerifies.GetVerifyGridList(dateStartStandard,dateEndStandard,dateLastPatEligibilityStandard,dateLastPlanBenefitsStandard,
				...
				foreach(InsVerifyGridObject gridObjCur in listInsVerifyGridObject) {
					//Filter rows that we don't want in whichever assign grid is showing.
					if((grid==gridAssignStandard && gridObjCur.IsForMedicaidPlan) || (grid==gridAssignMedicaid && !gridObjCur.IsForMedicaidPlan)) {
						continue;
					}
					listInsVerifyGridRows.Add(new InsVerifyGridRow(gridObjCur,_dictionaryDefsStatus,_listUserodsInRegion));
				}
			};
		...
		private void gridContextMenuStrip_Popup(object sender,EventArgs e) {
			...
			if(_gridRowSelected.InsVerifyPat!=null) {
		...
		private void OnOpenInsPlan() {
			...
			else if(_gridRowSelected.InsVerifyPlan!=null) {
				insPlan=InsPlans.GetPlan(_gridRowSelected.InsVerifyPlan.FKey,new List<InsPlan>());				
			}
			if(insPlan==null) {
		...
		private void butAssignUser_Click(object sender,EventArgs e) {
			...
			foreach(InsVerifyGridObject gridRowObject in listInsVerifyGridObjects) {
				if(gridRowObject.InsVerifyPat!=null) {
					gridRowObject.InsVerifyPat.UserNum=_assignUserNum;
					gridRowObject.InsVerifyPat.Note=GetInsVerifyNote();
				...
				if(gridRowObject.InsVerifyPlan!=null) {
					...
					gridRowObject.InsVerifyPlan.Note=GetInsVerifyNote();
		...
		private void SetStatus(long statusDefNum,bool isVerifyGrid) {
			...
			if(isVerifyGrid) {
				if(_gridRowSelected.InsVerifyPat!=null) {
					_gridRowSelected.InsVerifyPat.DefNum=statusDefNum;
					if(hasChanged) {
						_gridRowSelected.InsVerifyPat.Note=statusNote;
					}
					_gridRowSelected.InsVerifyPat.DateLastAssigned=DateTime.Today;
					InsVerifies.Update(_gridRowSelected.InsVerifyPat);
				}
				if(_gridRowSelected.InsVerifyPlan!=null) {
					_gridRowSelected.InsVerifyPlan.DefNum=statusDefNum;
					if(hasChanged) {
						_gridRowSelected.InsVerifyPlan.Note=statusNote;
						textInsVerifyReadOnlyNote.Text=statusNote;
					}
					_gridRowSelected.InsVerifyPlan.DateLastAssigned=DateTime.Today;
					InsVerifies.Update(_gridRowSelected.InsVerifyPlan);
				}
			}
			else {
				List<InsVerifyGridObject> listInsVerifyGridObjects=GetSelectedInsVerifyList();
				foreach(InsVerifyGridObject gridRowObject in listInsVerifyGridObjects) {
					if(gridRowObject.InsVerifyPat!=null) {
						gridRowObject.InsVerifyPat.DefNum=statusDefNum;
						if(hasChanged) {
							gridRowObject.InsVerifyPat.Note=statusNote;
						}
						gridRowObject.InsVerifyPat.DateLastAssigned=DateTime.Today;
						InsVerifies.Update(gridRowObject.InsVerifyPat);
					}
					if(gridRowObject.InsVerifyPlan!=null) {
						gridRowObject.InsVerifyPlan.DefNum=statusDefNum;
						if(hasChanged) {
							gridRowObject.InsVerifyPlan.Note=statusNote;
						}
						gridRowObject.InsVerifyPlan.DateLastAssigned=DateTime.Today;
						InsVerifies.Update(gridRowObject.InsVerifyPlan);
					}
		...
		private void menuItemVerifyStatusDropDown_Click(object sender, EventArgs e) {
			...
			else if(tabControl1.SelectedTab.In(tabAssignStandard,tabAssignMedicaid)) {
		...
		private void menuItemAssignUserDropDown_Click(object sender, EventArgs e) {
			...
			if(tabControl1.SelectedTab==tabVerify) {
				if(_gridRowSelected.InsVerifyPat!=null) {
					_gridRowSelected.InsVerifyPat.UserNum=userod.UserNum;
					_gridRowSelected.InsVerifyPat.DateLastAssigned=DateTime.Today;
					InsVerifies.Update(_gridRowSelected.InsVerifyPat);
				}
				if(_gridRowSelected.InsVerifyPlan!=null) {
					_gridRowSelected.InsVerifyPlan.UserNum=userod.UserNum;
					_gridRowSelected.InsVerifyPlan.DateLastAssigned=DateTime.Today;
					InsVerifies.Update(_gridRowSelected.InsVerifyPlan);
				}
			}
				...
				foreach(InsVerifyGridObject gridRowObject in listInsVerifyGridObjects) {
					if(gridRowObject.InsVerifyPat!=null) {
						gridRowObject.InsVerifyPat.UserNum=userod.UserNum;
						gridRowObject.InsVerifyPat.DateLastAssigned=DateTime.Today;
						InsVerifies.Update(gridRowObject.InsVerifyPat);
					}
					if(gridRowObject.InsVerifyPlan!=null) {
						gridRowObject.InsVerifyPlan.UserNum=userod.UserNum;
						gridRowObject.InsVerifyPlan.DateLastAssigned=DateTime.Today;
						InsVerifies.Update(gridRowObject.InsVerifyPlan);
					}
				}
		...
		private void tabControl1_Selected(object sender,TabControlEventArgs e) {
			if(e.TabPage.In(tabAssignStandard,tabAssignMedicaid)) {
				if(_verifyUserNum!=0) {
		...
		private class InsVerifyGridRow {
			...
			public InsVerifyGridRow(InsVerifyGridObject insVerifyGridObject,Dictionary<long,Def> dictionaryStatusDefs,List<Userod> listUserods) {
				...
				if(insVerifyGridObject.IsPatAndInsRow()) {//If showing a consolidated row, use the PatInsVerify information, since they are same anyways.
					...
					NextApptDate=insVerifyGridObject.InsVerifyPat.AppointmentDateTime;
					...
					NextApptDate=insVerifyGridObject.InsVerifyPlan.AppointmentDateTime;
