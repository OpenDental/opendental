...
namespace OpenDental {
	public partial class FormInsVerificationList:FormODBase {
		...
		public FormInsVerificationList() {
			...
			gridAssign.ContextMenu=_contextMenuRightClick;
			Lan.F(this);
		}
		...
		private void FormInsVerificationList_Load(object sender,EventArgs e) {
			...
			foreach(Def defCur in listDefsVerifyStatuses) {
				if(!_dictionaryDefsStatus.ContainsKey(defCur.DefNum)) {
					_dictionaryDefsStatus.Add(defCur.DefNum,defCur);
				}
			}
		...
		private void FillUsers() {
			...
			for(int i=0;i<_listUserodsInRegion.Count;i++) {
				if(_assignUserNum==_listUserodsInRegion[i].UserNum) {
					textAssignUser.Text=_listUserodsInRegion[i].UserName;
				}
			}
			if(_assignUserNum==0) {
		...
		private void PickUser(bool isAssigning) {
			...
			if(formUserPick.DialogResult==DialogResult.OK) {
				if(isAssigning) {//Setting the user
					_assignUserNum=formUserPick.SelectedUserNum;
				}
				else {//Filter by user
					_verifyUserNum=formUserPick.SelectedUserNum;
				}
				FillGrids();
			}
		}
		...
		private List<InsVerifyGridRow> GetRowsForGrid(bool isAssignGrid) {
			...
			progressOD.ActionMain=() => {
				List<InsVerifyGridObject> listInsVerifyGridObject=InsVerifies.GetVerifyGridList(dateTimeStart,dateTimeEnd,dateTimeLastPatEligibility,dateTimeLastPlanBenefits,
				...
				foreach(InsVerifyGridObject gridObjCur in listInsVerifyGridObject) {
					listInsVerifyGridRow.Add(new InsVerifyGridRow(gridObjCur,_dictionaryDefsStatus,_listUserodsInRegion,isAssignGrid));
				}
			};
		...
		private int CompareGridRows(InsVerifyGridRow insVerifyGridRowX,InsVerifyGridRow InsVerifyGridRowY) {
			if(insVerifyGridRowX.Type==InsVerifyGridRowY.Type && insVerifyGridRowX.Clinic==InsVerifyGridRowY.Clinic && insVerifyGridRowX.NextApptDate==InsVerifyGridRowY.NextApptDate) {
			...
			if(insVerifyGridRowX.Type==InsVerifyGridRowY.Type && insVerifyGridRowX.Clinic==InsVerifyGridRowY.Clinic) {
				return insVerifyGridRowX.NextApptDate.CompareTo(InsVerifyGridRowY.NextApptDate);
			}
		...
		private GridRow VerifyRowToODGridRow(InsVerifyGridRow insVerifyGridRow,bool isAssignGrid) {
			...
			row.Cells.Add(insVerifyGridRow.NextApptDate.ToString("g"));//"g" will exclude seconds from the DateTime.
		...
		private void FillGrid() {
			...
			this.Validate();
			List<InsVerifyGridRow> listInsVerifyGridRow=null;
			listInsVerifyGridRow=GetRowsForGrid(false);
			listInsVerifyGridRow.Sort(CompareGridRows);
			...
			for(int i=0;i<listInsVerifyGridRow.Count;i++) {
				grid.ListGridRows.Add(VerifyRowToODGridRow(listInsVerifyGridRow[i],false));
			}
			grid.EndUpdate();
		...
		private void OnVerify(PlanToVerify planToVerifyEnum) {
			...
			if((planToVerifyEnum==PlanToVerify.Both && !insVerifyGridObject.IsPatAndInsRow()) 
				|| (planToVerifyEnum==PlanToVerify.PatientEligibility && insVerifyGridObject.PatInsVerify==null) 
				|| (planToVerifyEnum==PlanToVerify.InsuranceBenefits && insVerifyGridObject.PlanInsVerify==null)) 
			{
			...
			if(planToVerifyEnum==PlanToVerify.Both || planToVerifyEnum==PlanToVerify.PatientEligibility) {
				insVerifyGridObject.PatInsVerify=InsVerifies.SetTimeAvailableForVerify(insVerifyGridObject.PatInsVerify,
				...
				insVerifyGridObject.PatInsVerify.DateLastVerified=DateTime.Today;
				InsVerifyHists.InsertFromInsVerify(insVerifyGridObject.PatInsVerify);
			}
			if(planToVerifyEnum==PlanToVerify.Both || planToVerifyEnum==PlanToVerify.InsuranceBenefits) {
				insVerifyGridObject.PlanInsVerify=InsVerifies.SetTimeAvailableForVerify(insVerifyGridObject.PlanInsVerify,
					PlanToVerify.InsuranceBenefits,PIn.Int(textAppointmentScheduledDays.Text),PIn.Int(textPatientEnrollmentDays.Text),
					PIn.Int(textInsBenefitEligibilityDays.Text));
				insVerifyGridObject.PlanInsVerify.DateLastVerified=DateTime.Today;
				InsVerifyHists.InsertFromInsVerify(insVerifyGridObject.PlanInsVerify);
			}
		...
		private void gridContextMenuStrip_Popup(object sender,EventArgs e) {
			...
			foreach(Userod user in _listUserodsInRegion) {
				if(user.IsHidden) {
					continue;
				}
				MenuItem menuItemAssignUserDropDown=new MenuItem(user.UserName);
				menuItemAssignUserDropDown.Tag=user;
				menuItemAssignUserDropDown.Click+=new EventHandler(assignUserToolItemDropDown_Click);
			...
			foreach(Def status in _listDefsVerifyStatuses) {
				MenuItem menuItemVerifyStatusDropDown=new MenuItem(status.ItemName);
				menuItemVerifyStatusDropDown.Tag=status;
				menuItemVerifyStatusDropDown.Click+=new EventHandler(verifyStatusToolItemDropDown_Click);
		...
		private void OnOpenInsPlan() {
			...
			if(_gridRowSelected.PatInsVerify!=null) {				
				...
				insPlan=InsPlans.GetPlan(_gridRowSelected.PlanInsVerify.FKey,new List<InsPlan>());				
			}
			if(insPlan==null) {
		...
		private void FillGridAssign() {
			...
			this.Validate();
			List<InsVerifyGridRow> listInsVerifyGridRow=null;
			listInsVerifyGridRow=GetRowsForGrid(isAssignGrid:true);
			listInsVerifyGridRow.Sort(CompareGridRows);
			...
			for(int i=0;i<listInsVerifyGridRow.Count;i++) {
				gridAssign.ListGridRows.Add(VerifyRowToODGridRow(listInsVerifyGridRow[i],true));
			}
		...
		private List<InsVerifyGridObject> GetSelectedInsVerifyList() {
			List<InsVerifyGridObject> listInsVerifyGridObject=new List<InsVerifyGridObject>();
			for(int i=0;i<gridAssign.SelectedIndices.Length;i++) {
				listInsVerifyGridObject.Add(((InsVerifyGridObject)gridAssign.ListGridRows[gridAssign.SelectedIndices[i]].Tag));
			}
			return listInsVerifyGridObject;
		}
		#endregion
		...
		private void SetStatus(long statusDefNum,bool isVerifyGrid) {
			...
			if(!isVerifyGrid) {
				inputBox.textResult.Text=textInsVerifyNote.Text;
			}
			inputBox.ShowDialog();
		...
		private void verifyStatusToolItemDropDown_Click(object sender, EventArgs e) {
			...
			if(tabControl1.SelectedTab==tabVerify) {
				SetStatus(defStatus.DefNum,true);
			}
			if(tabControl1.SelectedTab==tabAssign) {
				SetStatus(defStatus.DefNum,false);
			}
			FillGrids();
		}
		...
		private void assignUserToolItemDropDown_Click(object sender, EventArgs e) {
			...
			if(tabControl1.SelectedTab==tabVerify) {
				if(_gridRowSelected.PatInsVerify!=null) {
					_gridRowSelected.PatInsVerify.UserNum=userod.UserNum;
					_gridRowSelected.PatInsVerify.DateLastAssigned=DateTime.Today;
					InsVerifies.Update(_gridRowSelected.PatInsVerify);
				}
				if(_gridRowSelected.PlanInsVerify!=null) {
					_gridRowSelected.PlanInsVerify.UserNum=userod.UserNum;
					_gridRowSelected.PlanInsVerify.DateLastAssigned=DateTime.Today;
					InsVerifies.Update(_gridRowSelected.PlanInsVerify);
				}
			}
			if(tabControl1.SelectedTab==tabAssign) {
				...
				foreach(InsVerifyGridObject gridRowObject in listInsVerifyGridObject) {
					if(gridRowObject.PatInsVerify!=null) {
						gridRowObject.PatInsVerify.UserNum=userod.UserNum;
						gridRowObject.PatInsVerify.DateLastAssigned=DateTime.Today;
						InsVerifies.Update(gridRowObject.PatInsVerify);
					}
					if(gridRowObject.PlanInsVerify!=null) {
						gridRowObject.PlanInsVerify.UserNum=userod.UserNum;
						gridRowObject.PlanInsVerify.DateLastAssigned=DateTime.Today;
						InsVerifies.Update(gridRowObject.PlanInsVerify);
					}
				}
		...
		private class InsVerifyGridRow {
			...
			public InsVerifyGridRow(InsVerifyGridObject insVerifyGridObject,Dictionary<long,Def> dictionaryStatusDefs,List<Userod> listUserods,bool isAssignGrid) {
				if(insVerifyGridObject==null) {
					return;
				}
				Type="";
				...
				if(insVerifyGridObject.IsPatAndInsRow()) {//If showing a consolidated row, use the PatInsVerify information, since they are same anyways.
					Type="Pat/Ins";
					Clinic=insVerifyGridObject.PatInsVerify.ClinicName;
					PatientName=insVerifyGridObject.PatInsVerify.PatientName;
					NextApptDate=insVerifyGridObject.PatInsVerify.AppointmentDateTime;
					CarrierName=insVerifyGridObject.PatInsVerify.CarrierName;
					//Get the oldest DateLastVerified
					DateLastVerified=(insVerifyGridObject.PatInsVerify.DateLastVerified<=insVerifyGridObject.PlanInsVerify.DateLastVerified ? 
						insVerifyGridObject.PatInsVerify.DateLastVerified : 
						insVerifyGridObject.PlanInsVerify.DateLastVerified);
					if(dictionaryStatusDefs.ContainsKey(insVerifyGridObject.PatInsVerify.DefNum)) {
						VerifyStatus=dictionaryStatusDefs[insVerifyGridObject.PatInsVerify.DefNum].ItemName;
					}
					bool isPatLastAssignedNewer=insVerifyGridObject.PatInsVerify.DateLastAssigned>=insVerifyGridObject.PlanInsVerify.DateLastAssigned;
					//Get the most recent DateLastAssigned
					DateLastAssigned=(isPatLastAssignedNewer ? 
						insVerifyGridObject.PatInsVerify.DateLastAssigned : 
						insVerifyGridObject.PlanInsVerify.DateLastAssigned);
					SetAssignedTo(listUserods,insVerifyGridObject,isPatRow:true);
					Tag=insVerifyGridObject;
				}
				else if(insVerifyGridObject.IsOnlyPatRow()) {
					Type="Pat";
					Clinic=insVerifyGridObject.PatInsVerify.ClinicName;
					PatientName=insVerifyGridObject.PatInsVerify.PatientName;
					NextApptDate=insVerifyGridObject.PatInsVerify.AppointmentDateTime;
					CarrierName=insVerifyGridObject.PatInsVerify.CarrierName;
					DateLastVerified=insVerifyGridObject.PatInsVerify.DateLastVerified;
					if(dictionaryStatusDefs.ContainsKey(insVerifyGridObject.PatInsVerify.DefNum)) {
						VerifyStatus=dictionaryStatusDefs[insVerifyGridObject.PatInsVerify.DefNum].ItemName;
					}
					DateLastAssigned=insVerifyGridObject.PatInsVerify.DateLastAssigned;
					SetAssignedTo(listUserods,insVerifyGridObject,isPatRow:true);
					Tag=insVerifyGridObject;
				}
				else if(insVerifyGridObject.IsOnlyInsRow()) {
					Type="Ins";
					Clinic=insVerifyGridObject.PlanInsVerify.ClinicName;
					PatientName=insVerifyGridObject.PlanInsVerify.PatientName;
					NextApptDate=insVerifyGridObject.PlanInsVerify.AppointmentDateTime;
					CarrierName=insVerifyGridObject.PlanInsVerify.CarrierName;
					DateLastVerified=insVerifyGridObject.PlanInsVerify.DateLastVerified;
					if(dictionaryStatusDefs.ContainsKey(insVerifyGridObject.PlanInsVerify.DefNum)) {
						VerifyStatus=dictionaryStatusDefs[insVerifyGridObject.PlanInsVerify.DefNum].ItemName;
					}
					DateLastAssigned=insVerifyGridObject.PlanInsVerify.DateLastAssigned;
			...
			private void SetAssignedTo(List<Userod> listUserods,InsVerifyGridObject insVerifyGridObject,bool isPatRow=false,bool isInsPlanRow=false) {
				...
				if(isPatRow || (isPatRow && isInsPlanRow)) {//Pat only row or pat/ins plan row
					userod=listUserods.FirstOrDefault(x => x.UserNum==insVerifyGridObject.PatInsVerify.UserNum);
				}
				if(isInsPlanRow) {//plan only row
					userod=listUserods.FirstOrDefault(x => x.UserNum==insVerifyGridObject.PlanInsVerify.UserNum);
				}
				if(userod!=null) {
