	...
	public partial class FormPaySplitEdit : FormODBase {
		...
		private void FillPatient(){
			...
			if(PaySplitCur.PatNum==0){
				...
				PaySplitCur.PatNum=_famCur.ListPats[0].PatNum;//Patient of new split will default to the first patient in the family, usually guarantor
				for(int i=0;i<_famCur.ListPats.Length;i++) {//This is the same order added to listPatient.
					if(FormOpenDental.PatNumCur==_famCur.ListPats[i].PatNum) {
						listPatient.SelectedIndex=i;
						PaySplitCur.PatNum=_famCur.ListPats[i].PatNum;
						break;
					}
				}
			}
		...
		private void checkPatOtherFam_Click(object sender, System.EventArgs e) {
			...
			if(checkPatOtherFam.Checked){
				...
				PaySplitCur.PatNum=FormPS.PatNumSelected;
			}
		...
		private void FillProcedure(){
			...
			textProcTooth.Text=Tooth.Display(ProcCur.ToothNum);
			textProcDescription.Text="";
		...
		private void butAttachAdjust_Click(object sender,EventArgs e) {
			...
			_adjCur=FormAS.AdjustmentSelected;
		...
		private void checkPayPlan_Click(object sender, System.EventArgs e) {
			...
			PayPlan payPlanSelect=listPayPlans.First(x => x.PayPlanNum==formPayPlanSelect.SelectedPayPlanNum);
