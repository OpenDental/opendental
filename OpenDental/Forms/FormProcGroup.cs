				...
				switch(fields[f].InternalName){
					...
					case "PatFields":
						...
						for(int i=0;i<_listPatFieldDefs.Count;i++) {
							if(FieldDefLinks.GetExists(x => x.FieldDefNum==_listPatFieldDefs[i].PatFieldDefNum && x.FieldDefType==FieldDefTypes.Patient 
								&& x.FieldLocation==FieldLocations.GroupNote))
							{
								continue;
							}
							if(i>0){
