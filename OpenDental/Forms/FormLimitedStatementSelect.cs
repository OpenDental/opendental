...
namespace OpenDental {
	public partial class FormLimitedStatementSelect:FormODBase {
		...
		private void ConstructListFromTable() {
			foreach(DataRow tableRow in _tableAccount.Rows) {
				...
				if(tableRow["AdjNum"].ToString()!="0") {
					...
					row.PrimaryKey=PIn.Long(tableRow["PayPlanChargeNum"].ToString());
					row.Type=AccountEntryType.PayPlanCharge;
				}
				else {
		...
		private void butOK_Click(object sender,EventArgs e) {
			...
			ListSelectedPayPlanChargeNums=listSelectedRows.FindAll(x => x.Type==AccountEntryType.PayPlanCharge).Select(x => x.PrimaryKey).Distinct().ToList();
