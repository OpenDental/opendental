...
namespace OpenDental{
	...
	public partial class FormMountDefEdit : FormODBase{
		...
		private void FormMountDefEdit_Load(object sender, System.EventArgs e) {
			...
			textUnits.Text=MountDefCur.GetScaleUnits();
			textScale.Value=MountDefCur.GetScale();
			textDecimals.Value=MountDefCur.GetDecimals();
			FillItems();
			ShowWarning();
			if(!MountDefCur.IsNew){
				return;
		...
		private void butOK_Click(object sender, System.EventArgs e) {
			...
			MountDefCur.SetScale((float)textScale.Value,textDecimals.Value,textUnits.Text);
