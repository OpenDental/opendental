	...
	public partial class FormPatientEdit : FormODBase {
		...
		private void FormPatientEdit_Load(object sender, System.EventArgs e) {
				...
				&& Patient.PatNum==Patient.SuperFamily) //Has to be the Super Head.
			{
				...
				checkAddressSameForSuperFam.Checked=Patients.SuperFamHasSameAddrPhone(Patient,isAuthArchivedEdit);
			}
...
;						break;
		...
		private void GetEmployerNum(){
			if(Patient.EmployerNum==0){//no employer was previously entered.
				if(textEmployer.Text==""){
					...
					Patient.EmployerNum=Employers.GetEmployerNum(textEmployer.Text);
				}
				return;
			}
			//an employer was previously entered
			if(textEmployer.Text==""){
				Patient.EmployerNum=0;
			}
			//if text has changed
			else if(Employers.GetName(Patient.EmployerNum)!=textEmployer.Text){
				Patient.EmployerNum=Employers.GetEmployerNum(textEmployer.Text);
			}
		}
		...
		private void FillReferrals() {
			...
			_listRefAttaches=RefAttaches.Refresh(Patient.PatNum);
		...
		private void butSave_Click(object sender ,System.EventArgs e) {
			...
			PatientRaces.Reconcile(Patient.PatNum,_listPatientRaces);//Insert, Update, Delete if needed.
