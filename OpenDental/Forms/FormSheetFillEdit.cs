...
namespace OpenDental {
	...
	public partial class FormSheetFillEdit:FormODBase {
		...
		private string LayoutFields() {
			...
			imgDraw=new Bitmap(pictureBoxMain.Width,pictureBoxMain.Height);
			pictureBoxMain.Location=new Point(0,0);
			pictureBoxMain.Image=(Image)imgDraw.Clone();
			...
			foreach(SheetField field in SheetCur.SheetFields) {
				...
				if(Security.CurUser!=null
					...
					&& field.CanElectronicallySign //If the field allows for electronic signature
		...
		private void pictDraw_MouseMove(object sender,MouseEventArgs e) {
			...
			if(checkErase.Checked){
				...
				PointF eraserPt=new PointF(e.X+pictureBoxMain.Left+8.49f,e.Y+pictureBoxMain.Top+8.49f);
				foreach(SheetField field in SheetCur.SheetFields){
		...
		private void pictDraw_MouseUp(object sender,MouseEventArgs e) {
			...
			for(int i=0;i<PointList.Count;i++){
				...
				field.FieldValue+=(PointList[i].X+pictureBoxMain.Left)+","+(PointList[i].Y+pictureBoxMain.Top);
			}
			field.FontName="";
			...
			for(int f=0;f<SheetCur.SheetFields.Count;f++){
				if(SheetCur.SheetFields[f].FieldType==SheetFieldType.Drawing){
					...
					for(int i=1;i<points.Count;i++){
						g.DrawLine(pen,points[i-1].X-pictureBoxMain.Left,
							points[i-1].Y-pictureBoxMain.Top,
							points[i].X-pictureBoxMain.Left,
							points[i].Y-pictureBoxMain.Top);
					}
				}
				...
				switch(SheetCur.SheetType.ToString()+"."+field.FieldName) {
					case "TreatmentPlan.Note":
						g.DrawRectangle(Pens.DarkGray,
							field.XPos-pictureBoxMain.Left-1,
							field.YPos-pictureBoxMain.Top-1,
							field.Width+2,
							field.Height+2);
						break;
				}
			...
			foreach(SheetField field in SheetCur.SheetFields.Where(x => x.FieldType.In(SheetFieldType.SigBox,SheetFieldType.SigBoxPractice))) {//rectangles around specific output fields
				switch(SheetCur.SheetType) {
					case SheetTypeEnum.TreatmentPlan:
						g.DrawRectangle(Pens.Black,field.XPos-pictureBoxMain.Left-1,field.YPos-pictureBoxMain.Top-1,field.Width+2,field.Height+2);
						break;
				}
			}
		...
		private void butPrint_Click(object sender,EventArgs e) {
			...
			if((SheetCur.SheetType==SheetTypeEnum.ReferralSlip || SheetCur.SheetType==SheetTypeEnum.ReferralLetter) && FormS.Email2) {
				...
				pdfFile=EmailSheet(FormS.Email2Address,Lan.g(this,"RE: ")+Patients.GetLim(SheetCur.PatNum).GetNameLF());
			}
      ...
      string pdfFile=EmailSheet(emailAddress,subject);
		...
		private void butChangePat_Click(object sender,EventArgs e) {
			...
			if(FormPS.ShowDialog()==DialogResult.OK) {
				SecurityLogs.MakeLogEntry(Permissions.SheetEdit,SheetCur.PatNum,Lan.g(this,"Sheet with ID")+" "+SheetCur.SheetNum+" "+Lan.g(this,"moved to PatNum")+" "+FormPS.PatNumSelected);
				SecurityLogs.MakeLogEntry(Permissions.SheetEdit,FormPS.PatNumSelected,Lan.g(this,"Sheet with ID")+" "+SheetCur.SheetNum+" "+Lan.g(this,"moved from PatNum")+" "+SheetCur.PatNum);
				SheetCur.PatNum=FormPS.PatNumSelected;
				if(SheetCur.PatNum>0) {
