	...
	public partial class FormPayPlanDynamic : FormODBase {
		...
		private void FormPayPlanDynamic_Load(object sender,System.EventArgs e) {
			...
			if(_payPlanCur.Signature!="" && _payPlanCur.Signature!=null) {
				...
				signatureBoxWrapper.FillSignature(_payPlanCur.SigIsTopaz,PayPlans.GetKeyDataForSignature(_payPlanCur),_payPlanCur.Signature);//fill signature
			}
		...
		private void LoadPayDataFromDB() {
			...
			_listSplitsForCharges=_listPaySplits.Where(x => _listPayPlanChargesDb.Select(y => y.PayPlanChargeNum).ToList().Contains(x.PayPlanChargeNum)).ToList();
		}
		...
		private bool TryGetTermsFromUI(out PayPlanTerms terms,bool isSilent=false) {
			terms=new PayPlanTerms();
			try {
				if(!ValidateTerms(isSilent)) {//saveData relies on this, if removed from this method, needs to be addded to SaveData()
					return false;
				}
		...
		private void AddProcedures(List<Procedure> listProcs) {
			...
			foreach(Procedure proc in listProcs) {
				...
				AccountEntry accountEntry=constructResults.ListAccountEntries.FirstOrDefault(x => x.GetType()==typeof(Procedure) && x.PriKey==proc.ProcNum);
		...
		private void ButDeleteProduction_Click(object sender,EventArgs e) {
			...
			foreach(PayPlanProductionEntry entry in listSelectedProduction) {
				...
				_listPayPlanChargesDb.RemoveAll(x => listPayPlanChargeNumsDeleting.Contains(x.PayPlanChargeNum));
			}
			if(countInvalid>0) {
		...
		private void butChangeGuar_Click(object sender,System.EventArgs e) {
			...
			_payPlanCur.Guarantor=formPatientSelect.PatNumSelected;
			textGuarantor.Text=Patients.GetLim(_payPlanCur.Guarantor).GetNameLF();
		...
		private bool CreateSchedule() {
			if(ValidateTerms(doCheckAPR:false)) {//Don't need to validate APR and full lock because we will auto-lock when APR is set (only for creating schedule).
				...
				signatureBoxWrapper.FillSignature(_payPlanCur.SigIsTopaz,PayPlans.GetKeyDataForSignature(_payPlanCur),_payPlanCur.Signature);
				return true;
			}
			return false;
		}
		...
		private bool ValidateTerms(bool isSilent=false,bool doCheckAPR=true) {
			...
			if(PIn.Double(textPeriodPayment.Text)==0 && PIn.Long(textPaymentCount.Text)==0) {
				stringBuilderErrors.AppendLine(Lan.g(this,"Please enter a term or payment amount first."));
			}
			if(PIn.Date(textDate.Text).Date > DateTime.Today.Date && !PrefC.GetBool(PrefName.FutureTransDatesAllowed)) {
				stringBuilderErrors.AppendLine(Lan.g(this,"Payment plan date cannot be set for the future."));
			}
			if(textDateFirstPay.Text=="" || (PIn.Date(textDateFirstPay.Text).Date < DateTime.Today && _payPlanCur.IsNew)) {
				stringBuilderErrors.AppendLine(Lan.g(this,"Please enter a date on or after today for the date of the first payment."));
			}
			if(gridLinkedProduction.ListGridRows.Count!=0 && textPaymentCount.Text=="" && PIn.Double(textPeriodPayment.Text)==0) {
				stringBuilderErrors.AppendLine(Lan.g(this,"Payment cannot be 0."));
			}
			if(textPaymentCount.Text!="" && textPeriodPayment.Text!="") {
				stringBuilderErrors.AppendLine(Lan.g(this,"Please choose either Number of Payments or Payment Amt."));
			}
			if(textPeriodPayment.Text=="" && PIn.Long(textPaymentCount.Text) < 1) {
				stringBuilderErrors.AppendLine(Lan.g(this,"Term cannot be less than 1."));
			}
			if(PIn.Double(textTotalPrincipal.Text)-PIn.Double(textDownPayment.Text) < 0) {
				stringBuilderErrors.AppendLine(Lan.g(this,"Down payment must be less than or equal to total amount."));
			}
			if(doCheckAPR && !CompareDouble.IsZero(PIn.Double(textAPR.Text)) && checkProductionLock.Checked==false) {
				stringBuilderErrors.AppendLine(Lan.g(this,"Payment plans with APR must be locked. Remove the APR or check the box for Full Lock."));
			}
			if(!stringBuilderErrors.ToString().IsNullOrEmpty()) {
				if(!isSilent){
		...
		private void butSignPrint_Click(object sender,EventArgs e) {
			...
			Sheet sheetPP=null;
			sheetPP=PayPlanToSheet(_payPlanCur);
			string keyData=PayPlans.GetKeyDataForSignature(_payPlanCur);
		...
		private void butCloseOut_Click(object sender,EventArgs e) {
			...
			List<PayPlanCharge> listChargesClosing=PayPlanEdit.GetCloseoutChargesForDynamicPaymentPlan(_payPlanCur,
				_famCur,
				ref _listPayPlanChargesDb,
				ref _listPayPlanLinks,
				ref _listPayPlanProductionEntries);
		...
		private void butSendToDevice_Click(object sender,EventArgs e) {
			...
			else if(!string.IsNullOrEmpty(_payPlanCur.Signature)) {
		...
		private void PushSelectedPayPlanToEclipboard(MobileAppDevice mobileAppDevice){
			...
			using(PdfDocument document=GetPayPlanPDF(_payPlanCur)) {//Cant be null due to above check.
				if(PushNotificationUtils.CI_SendPaymentPlan(document,_payPlanCur,mobileAppDevice.MobileAppDeviceNum
					,out string errorMsg,out long mobileDataByeNum)) 
				{
					...
					PayPlans.UpdateMobileAppDeviceNum(_payPlanCur,0);
					Signalods.SetInvalid(InvalidType.AccModule,KeyType.PatNum,_payPlanCur.PatNum);
		...
		private void OpenUnlockCodeForPayPlan(){
			MobileDataByte funcInsertDataForUnlockCode(string unlockCode) {
				using(PdfDocument doc=GetPayPlanPDF(_payPlanCur)) {
					List<string> listTagValues=new List<string>() { _payPlanCur.PayPlanNum.ToString(),_payPlanCur.PayPlanDate.Ticks.ToString(),
						_patCur.GetNameFirstOrPrefL() };
					if(MobileDataBytes.TryInsertPDF(doc,_patCur.PatNum,unlockCode,eActionType.PaymentPlan,out long mobileDataByteNum,out string errorMsg,
						listTagValues))
					{
