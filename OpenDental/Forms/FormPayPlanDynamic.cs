	...
	public partial class FormPayPlanDynamic : FormODBase {
		...
		private void gridCharges_RightClickEdit(object sender, System.EventArgs e) { 
			...
			if(listPayPlanCharges.Count>1) {
				using FormPayPlanChargeSelection formPayPlanChargeSelection=new FormPayPlanChargeSelection(listPayPlanCharges,_dynamicPaymentPlanData.PayPlan);
				formPayPlanChargeSelection.ShowDialog();
			}
			else {
				using FormPayPlanChargeEdit formPayPlanChargeEdit=new FormPayPlanChargeEdit(listPayPlanCharges[0],_dynamicPaymentPlanData.PayPlan);
