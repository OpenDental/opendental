...
namespace OpenDental {
	...
	public partial class FormInsPlansOverrides:FormODBase {
		...
		private void FormInsTemplates_Load(object sender, System.EventArgs e) {
			...
			if(prog!=null && prog.Enabled) { //If trojan bridge exists and is enabled, allow user to filter upon TrojanID.
		...
		private void FillGrid(bool isGetAll=false){
			...
			_dataTable=InsPlans.GetBigList(radioOrderEmp.Checked,textEmployer.Text,textCarrier.Text,
			...
			for(int i=0;i<_dataTable.Rows.Count;i++) {
				listPlanNums.Add(PIn.Long(_dataTable.Rows[i]["PlanNum"].ToString()));
			}
			...
			for(int i=0;i<_dataTable.Rows.Count;i++) {
				row=new GridRow();
				row.Cells.Add(_dataTable.Rows[i]["EmpName"].ToString());
				row.Cells.Add(_dataTable.Rows[i]["CarrierName"].ToString());
				row.Cells.Add(_dataTable.Rows[i]["GroupNum"].ToString());
				row.Cells.Add(_dataTable.Rows[i]["GroupName"].ToString());
				row.Cells.Add(_dataTable.Rows[i]["subscribers"].ToString());
				if(CultureInfo.CurrentCulture.Name.EndsWith("CA")) {//Canadian. en-CA or fr-CA
					row.Cells.Add((_dataTable.Rows[i]["IsCDA"].ToString()=="0")?"":"X");
				}
				if(_isTrojanEnabled) { // If trojan bridge exists and is enabled, show TrojanID column in grid
					row.Cells.Add(_dataTable.Rows[i]["TrojanID"].ToString());
				}
				string noBillInsOverride="";
				InsPlanPreference insPlanPreference=listInsPlanPreferencesNoBillIns.Find(x => x.PlanNum==PIn.Long(_dataTable.Rows[i]["PlanNum"].ToString()));
				...
				row.Tag=PIn.Long(_dataTable.Rows[i]["PlanNum"].ToString());//Tag the row with the PlanNum to easily get list of selected PlanNums later on
