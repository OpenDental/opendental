...
namespace OpenDental {
	...
	public partial class FormProcEdit : FormODBase {
		...
		private void butAddExistAdj_Click(object sender,EventArgs e) {
			...
			if(AvaTax.IsEnabled() && 
				(FormAP.AdjustmentSelected.AdjType==AvaTax.SalesTaxAdjType || FormAP.AdjustmentSelected.AdjType==AvaTax.SalesTaxReturnAdjType) && 
		...
		private void textSurfaces_Validating(object sender, System.ComponentModel.CancelEventArgs e) {
			if(Tooth.IsValidEntry(textTooth.Text)){
				textSurfaces.Text=Tooth.SurfTidyForDisplay(textSurfaces.Text,Tooth.Parse(textTooth.Text));
			}
			else{
		...
		private bool EntriesAreValid(){
			...
			if(_sigChanged && !signatureBoxWrapper.SigIsBlank && hasAutoNotePrompt && PrefC.GetBool(PrefName.ProcNoteSigsBlockedAutoNoteIncomplete)) {
			...
			if(_procedureOld.ProcStatus!=ProcStat.C && _procedure.ProcStatus==ProcStat.C){//if status was changed to complete
				...
				if(_procedureOld.ProcStatus.In(ProcStat.C,ProcStat.EO,ProcStat.EC)) {//that was already complete
