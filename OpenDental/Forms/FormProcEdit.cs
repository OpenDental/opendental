...
namespace OpenDental {
	...
	public partial class FormProcEdit : FormODBase {
		...
		private void butPickOrderProvReferral_Click(object sender,EventArgs e) {
			...
			SetOrderingReferral(form.ReferralSelected);
		}
		...
		private void PromptForAutoNotes() {
			...
			foreach(Match match in listMatches) {
				...
				if(FormA.DialogResult==DialogResult.OK) {
					...
					loc=matchloc+FormA.StrCompletedNote.Replace("\r\n","\n").Length;
					string resultstr=textNotes.Text.Substring(0,matchloc)+FormA.StrCompletedNote;
		...
		private void SetControlsUpperLeft(){
				...
				|| _procedureCode.AreaAlsoToothRange)
			{
				...
				if(_listPatToothInitials==null) {
					_listPatToothInitials=ToothInitials.GetPatientData(_patient.PatNum);
				}
		...
		private void butAddEstimate_Click(object sender, System.EventArgs e) {
			...
			if(FormIS.InsPlanSelected==null) {
				FormIS.ShowDialog();
				if(FormIS.DialogResult==DialogResult.Cancel){
					return;
				}
			}
			InsPlan plan=FormIS.InsPlanSelected;
			InsSub sub=FormIS.InsSubSelected;
			_listClaimProcsForProc=ClaimProcs.RefreshForProc(_procedure.ProcNum);
			...
			using FormClaimProc formClaimProc=new FormClaimProc(cp,_procedure,_family,_patient,_listInsPlans,ListClaimProcHists,ref ListClaimProcHistsLoop,_listPatPlans,true,_listInsSubs);
			...
			if(formClaimProc.DialogResult==DialogResult.Cancel && cp.IsNew){
		...
		private void gridPay_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			FormP.PaySplitNumInitial=((PaySplit)_paySplitsForProc[e.Row]).SplitNum;
			FormP.ShowDialog();
			FillPayments();
		}
		...
		private void butAddExistAdj_Click(object sender,EventArgs e) {
			...
			if(!Security.IsAuthorized(Permissions.AdjustmentEdit,FormAP.AdjustmentSelected.AdjDate)) {
				return;
			}
			if(AvaTax.IsEnabled() && 
				(FormAP.AdjustmentSelected.AdjType==AvaTax.GetSalesTaxAdjType() || FormAP.AdjustmentSelected.AdjType==AvaTax.GetSalesTaxReturnAdjType()) && 
			...
			FormAP.AdjustmentSelected.ProcNum=_procedure.ProcNum;
			FormAP.AdjustmentSelected.ProcDate=_procedure.ProcDate;
			Adjustments.Update(FormAP.AdjustmentSelected);
			_loadData.ArrAdjustments=Adjustments.GetForProcs(new List<long>() { _procedure.ProcNum }).ToArray();
			FillAdj();
		}
		...
		private void buttonUseAutoNote_Click(object sender,EventArgs e) {
			...
			FormA.ShowDialog();
			if(FormA.DialogResult==DialogResult.OK) {
				textNotes.AppendText(FormA.StrCompletedNote);
				bool hasAutoNotePrompt=Regex.IsMatch(textNotes.Text,Procedures.AutoNotePromptRegex);
				butEditAutoNote.Visible=hasAutoNotePrompt;
			}
		}
		...
		private void butEditAutoNote_Click(object sender,EventArgs e) {
			if(Regex.IsMatch(textNotes.Text,Procedures.AutoNotePromptRegex)) {
				using FormAutoNoteCompose FormA=new FormAutoNoteCompose();
				FormA.StrMainTextNote=textNotes.Text;
				FormA.ShowDialog();
				if(FormA.DialogResult==DialogResult.OK) {
					textNotes.Text=FormA.StrCompletedNote;
					bool hasAutoNotePrompt=Regex.IsMatch(textNotes.Text,Procedures.AutoNotePromptRegex);
					butEditAutoNote.Visible=hasAutoNotePrompt;
				}
			}
		...
		private void PickDiagnosisCode(TextBox textBoxDiagnosisCode) {
			if(checkIcdVersion.Checked) {//ICD-10
				...
				if(formI.ShowDialog()==DialogResult.OK) {
					textBoxDiagnosisCode.Text=formI.Icd10Selected.Icd10Code;
				}
			}
			else {//ICD-9
				...
				formI.IsSelectionMode=true;
				if(formI.ShowDialog()==DialogResult.OK) {
					textBoxDiagnosisCode.Text=formI.ICD9Selected.ICD9Code;
				}
			}
		}
		...
		private bool EntriesAreValid() {
			...
			Func<string,bool> actionYesNoPrompte=(msg) => {
			...
			bool isSigChangedAndNotBlank=_sigChanged && !signatureBoxWrapper.SigIsBlank;
			int unityQty=PIn.Int(textUnitQty.Text, false);
			bool isTextDateOriginalProsthBlank=textDateOriginalProsth.Text=="";
			DateTime procDateTime=PIn.Date(textDate.Text);
			bool result=Procedures.EntriesAreValid(textNotes.Text,isSigChangedAndNotBlank,textTimeStart.Text,textTimeEnd.Text,unityQty,comboProv.GetSelectedProvNum(),textMedicalCode.Text,isTextDrugNdcNotBlank,textDrugQty.Text,
				ref procDateTime,IsNew,PIn.Double(textProcFee.Text),_isQuickAdd,isCheckTypeCodeNonXChecked,checkTypeCodeX.Checked,listProsth.SelectedIndex,isQuadrantSelected,
				...
				actionOnFailure,actionYesNoPrompte,actionOnProcedureCodeFailure,
				ref _listClaimProcsForProc
			);
			textDate.Text=procDateTime.ToShortDateString();//The value can be changed in EntriesAreValid(...)
			return result;
		}
		...
		private void SaveAndClose() {
			...
			if(!Procedures.VerifyProviderChange(_procedure,_procedureOld,_adjustmentsForProc.Cast<Adjustment>().ToList(),out bool hasSplitProvChanged,out bool hasAdjProvChanged,
				translationSource,funcYesNoPrompt,actionOnFailure))
			{
				return;
			}
			if(!Procedures.VerifyCompletedProcStatusChange(_paySplitsForProc.Cast<PaySplit>().ToList(),_procedure,_procedureOld,translationSource,funcYesNoPrompt,actionOnFailure)) {
				return;
			}
			...
			if(_procedureCode.TreatArea==TreatmentArea.Surf){
				_procedure.ToothNum=Tooth.Parse(textTooth.Text);
				...
				|| _procedureCode.AreaAlsoToothRange)
			{
				...
				if(listBoxTeeth.SelectedItems.Count<1 && listBoxTeeth2.SelectedItems.Count<1) {
					actionOnFailure.Invoke(Lan.g(this,"Must pick at least 1 tooth"));
					return;
				}
			...
			try {
				...
				actionOnFailure.Invoke(Lan.g(this,"Error saving signature.")+"\r\n"+ex.Message);
			...
			if(CultureInfo.CurrentCulture.Name.EndsWith("CA")) {//Canadian. en-CA or fr-CA
				...
				if(canadaLabFee1==0) {
					...
					if(_listCanadaLabFees.Count>0) { //Retain the old lab code if present.
						...
						CanadaLabProcEditHelper(canadaLabFee1);
					}
				}
				...
				if(canadaLabFee2==0) {
					...
					if(_listCanadaLabFees.Count>1) { //Retain the old lab code if present.
						...
						CanadaLabProcEditHelper(canadaLabFee2);
					}
				}
			}
			else {
			...
			if(!_procedureOld.ProcStatus.In(ProcStat.C,ProcStat.EO,ProcStat.EC)
				...
				if(AutoCodeItems.ShouldPromptForCodeChange(_procedure,_procedureCode,_patient,isMandibular,_listClaimProcsForProc,out verifyCode)) {
					...
					_procedure=FormACLI.ProcedureCur;
		...
		private void CanadaLabProcEditHelper(double labFeeAmt) {
			...
			labFee.PatNum=_procedure.PatNum;
			labFee.ProcDate=_procedure.ProcDate;
			labFee.ProcFee=labFeeAmt;
			labFee.ProcStatus=_procedure.ProcStatus;
			labFee.DateEntryC=DateTime.Now;
			labFee.ClinicNum=_procedure.ClinicNum;
			labFee.ProcNumLab=_procedure.ProcNum;
			...
			labFee.PlaceService=Clinics.GetPlaceService(_procedure.ClinicNum);
			...
			labFee.ProvNum=_procedure.ProvNum;
		...
		private void butChangeUser_Click(object sender,EventArgs e) {
			...
			if(FormChangeUser.DialogResult==DialogResult.OK) { //if successful
				_curUser=FormChangeUser.UserodSimpleSwitch; //assign temp user
