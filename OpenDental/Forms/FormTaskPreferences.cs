...
namespace OpenDental {
	public partial class FormTaskPreferences:FormODBase {
		...
		private void FillComboImageCategoryFolders() {
			long curImageCategoryDefNum=PrefC.GetLong(PrefName.TaskAttachmentCategory);
			...
			comboImageCategoryFolders.SetSelectedDefNum(curImageCategoryDefNum);
		}
		...
		private void butOK_Click(object sender,EventArgs e) {
			...
			if(Prefs.UpdateBool(PrefName.TaskListAlwaysShowsAtBottom,checkTaskListAlwaysShow.Checked)
				...
				| Prefs.UpdateLong(PrefName.TaskAttachmentCategory,comboImageCategoryFolders.GetSelected<Def>()?.DefNum??0)//If 'none' is selected, def will be null so set the pref to 0
				) {
				changed=true;
			}
