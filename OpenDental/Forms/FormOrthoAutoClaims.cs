		...
		private void butGenerateClaims_Click(object sender,EventArgs e) {
			...
			for(int i = 0;i < gridMain.SelectedIndices.Count();i++) {
				try {
					...
					string procCode=ProcedureCodes.GetProcCode(codeNumCur).ProcCode;
					if(!ListTools.In(procCode,listHiddenProcCodes) && ProcedureCodes.AreAnyProcCodesHidden(codeNumCur)) {
