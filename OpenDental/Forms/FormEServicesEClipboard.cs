...
namespace OpenDental {
	public partial class FormEServicesEClipboard:FormODBase {
		...
		private ClinicPrefHelper _clinicPrefHelperEClipboard=new ClinicPrefHelper(
			...
			PrefName.EClipboardShowPerioGraphical);
		...
		private void GridEClipboardSheetsInUse_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			FormEClipboardSheetRules formESR=new FormEClipboardSheetRules((EClipboardSheetDef)gridEClipboardSheetsInUse.ListGridRows[e.Row].Tag,gridEClipboardSheetsInUse.ListGridRows.Select(x=>(EClipboardSheetDef)x.Tag).ToList());
			formESR.ShowDialog();
			FillGridEClipboardSheetInUse();
		}

		private void gridMobileAppDevices_CellClick(object sender,ODGridClickEventArgs e) {
			int indexOfEnabledColumn = gridMobileAppDevices.Columns.GetIndex(_enabledColumnText);
		...
		private void CheckAllowPaymentCheckIn_Click(object sender,EventArgs e) {
			...
			_clinicPrefHelperEClipboard.ValChangedByUser(PrefName.EClipboardAllowPaymentOnCheckin,clinicPickerEClipboard.SelectedClinicNum,strAllowPayments);
		...
		private void checkEClipboardShowPerioGraphical_Click(object sender,EventArgs e) {
			string strShowPerioGraphical=POut.Bool(checkEClipboardShowPerioGraphical.Checked);
			_clinicPrefHelperEClipboard.ValChangedByUser(PrefName.EClipboardShowPerioGraphical,clinicPickerEClipboard.SelectedClinicNum,strShowPerioGraphical);
			UpdateEClipboardDefaultsIfNeeded(PrefName.EClipboardShowPerioGraphical,strShowPerioGraphical);
		}

		private void checkDisplayIndividually_Click(object sender, EventArgs e) {
			string boolString=POut.Bool(checkDisplayIndividually.Checked);
			_clinicPrefHelperEClipboard.ValChangedByUser(PrefName.EClipboardHasMultiPageCheckIn,clinicPickerEClipboard.SelectedClinicNum,boolString);
			UpdateEClipboardDefaultsIfNeeded(PrefName.EClipboardHasMultiPageCheckIn,boolString);
		}
		...
		private void EClipboardSetControlsToPrefDefaults(){
			...
			checkEClipboardAllowPaymentCheckIn.Checked=_clinicPrefHelperEClipboard.GetDefaultBoolVal(PrefName.EClipboardAllowPaymentOnCheckin);
			...
			checkDisplayIndividually.Checked=_clinicPrefHelperEClipboard.GetDefaultBoolVal(PrefName.EClipboardHasMultiPageCheckIn);
			...
			checkEClipboardShowPerioGraphical.Checked=_clinicPrefHelperEClipboard.GetDefaultBoolVal(PrefName.EClipboardShowPerioGraphical);
			...
			_clinicPrefHelperEClipboard.ValChangedByUser(PrefName.EClipboardAllowPaymentOnCheckin,clinicPickerEClipboard.SelectedClinicNum,
			...
			_clinicPrefHelperEClipboard.ValChangedByUser(PrefName.EClipboardHasMultiPageCheckIn,clinicPickerEClipboard.SelectedClinicNum,POut.Bool(checkDisplayIndividually.Checked));
			_clinicPrefHelperEClipboard.ValChangedByUser(PrefName.EClipboardShowPerioGraphical,clinicPickerEClipboard.SelectedClinicNum,
				POut.Bool(checkEClipboardShowPerioGraphical.Checked));
		}
		...
		private void EClipboardSetControlsForClinicPrefs(){
			...
			if(checkEClipboardUseDefaults.Checked) {
				...
				checkEClipboardAllowPaymentCheckIn.Checked=_clinicPrefHelperEClipboard.GetBoolVal(PrefName.EClipboardAllowPaymentOnCheckin,clinicNumSelected);
				...
				checkDisplayIndividually.Checked=_clinicPrefHelperEClipboard.GetBoolVal(PrefName.EClipboardHasMultiPageCheckIn,clinicNumSelected);
				...
				checkEClipboardShowPerioGraphical.Checked=_clinicPrefHelperEClipboard.GetBoolVal(PrefName.EClipboardShowPerioGraphical,clinicNumSelected);
			}
		}
		...
		private void FillGridEClipboardSheetInUse() {
			...
			gridEClipboardSheetsInUse.Columns.Add(gridColumn);
		...
		private void FillGridMobileAppDevices() {
			...
			gridMobileAppDevices.Columns.Add(gridColumn);
			...
			gridColumn=new GridColumn("Device State",100) {IsWidthDynamic=true };
			gridMobileAppDevices.Columns.Add(gridColumn);
			gridColumn=new GridColumn(_enabledColumnText,50,HorizontalAlignment.Center);
			gridMobileAppDevices.Columns.Add(gridColumn);
			...
			for(int i=0;i<listMobileAppDevicesToShow.Count;i++) {
				...
				row.Cells.Add(listMobileAppDevicesToShow[i].DevicePage.GetDescription());
