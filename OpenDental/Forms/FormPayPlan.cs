...
namespace OpenDental{
	...
	public partial class FormPayPlan : FormODBase {
		...
		private void butChangeGuar_Click(object sender,System.EventArgs e) {
			...
			_payPlanCur.Guarantor=FormPS.PatNumSelected;
			textGuarantor.Text=Patients.GetLim(_payPlanCur.Guarantor).GetNameLF();
		}
		...
		private void butChangePlan_Click(object sender,System.EventArgs e) {
			...
			_payPlanCur.PlanNum=FormI.InsPlanSelected.PlanNum;
			_payPlanCur.InsSubNum=FormI.InsSubSelected.InsSubNum;
      ...
      if(_listPayPlanCharges.Count(x => x.ChargeType==PayPlanChargeType.Debit)==0//amortization schedule does not exist
		...
		private void butSendToDevice_Click(object sender,EventArgs e) {
			...
			if(MobileAppDevices.ShouldSendPush(PatCur.PatNum,out MobileAppDevice device)) {
			...
			MobileDataByte funcInsertDataForUnlockCode(string unlockCode) {
				using(PdfDocument doc=GetPayPlanPDF(_payPlanCur)) {
					List<string> listTagValues=new List<string>() { _payPlanCur.PayPlanNum.ToString(),_payPlanCur.PayPlanDate.Ticks.ToString(),
						PatCur.GetNameFirstOrPrefL() };
					if(MobileDataBytes.TryInsertPDF(doc,PatCur.PatNum,unlockCode,eActionType.PaymentPlan,out long mobileDataByteNum,out string errorMsg,
						listTagValues))
					{
