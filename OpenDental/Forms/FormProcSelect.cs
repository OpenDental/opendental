	...
	public partial class FormProcSelect : FormODBase {
		...
		private void FillGrid(){
			...
			foreach(AccountEntry entry in _listAccountCharges) {
				if((!entry.GetType().In(typeof(ProcExtended),typeof(Adjustment)) || Math.Round(entry.AmountEnd,3) == 0) && credCalc!=CreditCalcType.ExcludeAll) {
					continue;
				}
				string dateStr;
				string procCodeStr="";
				...
				if(entry.GetType()==typeof(Adjustment)) {
					...
					toothNumStr=procCur.ToothNum=="" ? Tooth.SurfTidyFromDbToDisplay(procCur.Surf,procCur.ToothNum) : Tooth.Display(procCur.ToothNum);
