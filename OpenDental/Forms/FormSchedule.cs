...
namespace OpenDental{
	...
	public partial class FormSchedule:FormODBase {
		...
		private void PasteSelected() {
			...
			List<Schedule> listScheduledHolidays=GetHolidaySchedules(dateSelectedStart,dateSelectedEnd,isWeekCopied);
			if(checkReplace.Checked) {
				...
				listSchedsToDelete=FilterScheduleList(listSchedsToDelete,true);
			...
			for(int i=0;i<listSchedulesToCopy.Count;i++){
				...
				if(listScheduledHolidays.Exists(x => x.SchedDate==sched.SchedDate)){
					continue;//Don't add schedules to a day that's a holiday.
				}
				listSchedulesToInsert.Add(sched);
			}
			if(listScheduledHolidays.Count>0){
				MsgBox.Show(this,listScheduledHolidays.Count+" holidays exist in the destination date range. Holidays will not be replaced and must be done manually.");
			}
		...
		private void butRepeat_Click(object sender,EventArgs e) {
			...
			int dayCount=0;//this is needed when repeat pasting days in order to calculate skipping weekends.
			...
			List<Schedule> listScheduledHolidays=GetHolidaySchedules(dateSelectedStart,dateSelectedEnd,isWeekCopied,repeatCount);
			...
			for(int r=0;r<repeatCount;r++){//for example, user wants to repeat 3 times.
				if(checkReplace.Checked) {
					...
					if(isWeekCopied){
						...
						Logger.LogToPath("!isWeek.Schedules.Clear",LogPath.Signals,LogPhase.Start);
					  listSchedsToDelete=Schedules.GetSchedulesToDelete(dateSelectedStart.AddDays(dayCount),dateSelectedEnd.AddDays(dayCount),
				...
				for(int i=0;i<listSchedulesToCopy.Count;i++) {//For example, if 3 weeks for one provider, then about 30 loops.
					...
					if(isWeekCopied) {
						...
						sched.SchedDate=dateSelectedStart.AddDays(dayCount);
					}
					if(listScheduledHolidays.Exists(x => x.SchedDate==sched.SchedDate)){
		...
		private int CalculateNextDay(DateTime dateTimeCur) {
			if(_weekendFilter==ScheduleWeekendFilter.WorkWeek && dateTimeCur.DayOfWeek==DayOfWeek.Friday) {
				return 3;//Skip to the following monday
			}
			else if(_weekendFilter==ScheduleWeekendFilter.Weekend && dateTimeCur.DayOfWeek==DayOfWeek.Sunday) {
		...
		private List<Schedule> GetHolidaySchedules(DateTime dateTimeStart,DateTime dateTimeEnd,bool isWeekCopied,int repeatCount=0) {
			...
			List<Schedule> listScheduleHolidays=Schedules.GetAllHolidaysForDateRange(dateTimeStart,dateTimeEnd.AddDays(totalDays),listClinicNums);
			listScheduleHolidays=FilterScheduleList(listScheduleHolidays,false);
			return listScheduleHolidays;
		}
		...
		private List<Schedule> FilterScheduleList(List<Schedule> listSchedule,bool removeHolidays){
			if(removeHolidays) {
				List<Schedule> listScheduleHolidays=listSchedule.FindAll(x => x.Status==SchedStatus.Holiday);
				for(int i=0;i<listScheduleHolidays.Count;i++) {
					listSchedule.RemoveAll(x => x.SchedDate.Date==listScheduleHolidays[i].SchedDate);//Filter all schedules on the holiday to not be changed
				}
			}
			if(_weekendFilter==ScheduleWeekendFilter.FullWeek) {
				return listSchedule;
			}
			for(int i=listSchedule.Count-1;i>=0;i--) {
				if(_weekendFilter==ScheduleWeekendFilter.Weekend 
					&& listSchedule[i].SchedDate.DayOfWeek!=DayOfWeek.Saturday
					&& listSchedule[i].SchedDate.DayOfWeek!=DayOfWeek.Sunday)
				{
					listSchedule.Remove(listSchedule[i]);//Remove weekdays on the weekend view
				}
				else if(_weekendFilter==ScheduleWeekendFilter.WorkWeek
					&& (listSchedule[i].SchedDate.DayOfWeek==DayOfWeek.Saturday || listSchedule[i].SchedDate.DayOfWeek==DayOfWeek.Sunday)) {
					listSchedule.Remove(listSchedule[i]);//Remove weekend days on the work week view
				}
			}
			return listSchedule;
		}
