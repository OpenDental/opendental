      ...
      FormC.ShowDialog(); 
		...
		private void pdCards_PrintPage(object sender, PrintPageEventArgs ev){
			...
			while(yPos<ev.PageBounds.Height-bottomPageMargin && patientsPrinted<AddrTable.Rows.Count){
				...
				//Body text, single card-------------------------------------------------------------------
					...
					str=PatComm.BuildConfirmMessage(ContactMethod.Mail,pat,aptDateTime,dateTimeAskedToArrive);
				}
			...
			for(int i=0;i<gridMain.SelectedIndices.Length;i++){
				if(checkGroupFamilies.Checked) {//build the list of appointments a family has scheduled
					if(i==gridMain.SelectedIndices.Length-1 || _table.Rows[gridMain.SelectedIndices[i]]["Guarantor"].ToString()!=_table.Rows[gridMain.SelectedIndices[i+1]]["Guarantor"].ToString()) {
						if(familyApptList!=""){
							familyApptList+="\r\n"+_table.Rows[gridMain.SelectedIndices[i]]["nameF"].ToString()+":  "
								+PIn.Date(_table.Rows[gridMain.SelectedIndices[i]]["AptDateTime"].ToString()).ToShortDateString();
				...
				Patient pat=new Patient {
					FName=PIn.String(_table.Rows[gridMain.SelectedIndices[i]][checkGroupFamilies.Checked?"guarNameF":"nameF"].ToString()),
				};
				...
				if(checkGroupFamilies.Checked && familyApptList!="") {
					...
					//See B29252, only working on postcards so don't change functionality here by adding dateTimeAskedToArrive.
					//Setting dateTimeAskedToarrive to DateTime.MinValue ensures that apptDateTime is always used.
					str=PatComm.BuildConfirmMessage(ContactMethod.Email,pat,apptDateTime,dateTimeAskedToArrive:DateTime.MinValue);
				}
		...
		private void butText_Click(object sender,EventArgs e) {
			...
			for(int i=0;i<gridMain.SelectedIndices.Length;i++){
				...
				txtMsgOk=((YN)PIn.Int(_table.Rows[gridMain.SelectedIndices[i]][checkGroupFamilies.Checked?"guarTxtMsgOK":"TxtMsgOk"].ToString()));
				Patient pat=new Patient {
					FName=PIn.String(_table.Rows[gridMain.SelectedIndices[i]]["nameF"].ToString()),
				};
				DateTime apptDateTime=PIn.DateT(_table.Rows[gridMain.SelectedIndices[i]]["AptDateTime"].ToString());
				if(checkGroupFamilies.Checked && familyApptList!="") {
					...
					//See B29252, only working on postcards so don't change functionality here by adding dateTimeAskedToArrive.
					//Setting dateTimeAskedToarrive to DateTime.MinValue ensures that apptDateTime is always used.
					message=PatComm.BuildConfirmMessage(ContactMethod.TextMessage,pat,apptDateTime,dateTimeAskedToArrive:DateTime.MinValue);
				}
