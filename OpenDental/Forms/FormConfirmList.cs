...
///<summary></summary>
	public partial class FormConfirmList : FormODBase {
		...
		private void FillComboEmail() {
			...
			for(int i=0;i<listClinics.Count;i++) {//Exclude any email addresses that are associated to a clinic.
				_listEmailAddresses.RemoveAll(x => x.EmailAddressNum==listClinics[i].EmailAddressNum);
			}
			...
			EmailAddress emailAddress=EmailAddresses.GetForUser(Security.CurUser.UserNum);//can be null
			if(emailAddress!=null) {
				_listEmailAddresses.Insert(0,emailAddress);
				comboEmailFrom.Items.Add(Lan.g(this,"Me")+" <"+emailAddress.EmailUsername+">");//Just below Practice/Clinic
			}
			...
			for(int i=0;i<_listEmailAddresses.Count;i++) {
				if(emailAddress==null || _listEmailAddresses[i]!=emailAddress) {
