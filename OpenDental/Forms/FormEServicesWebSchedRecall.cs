	...
	public partial class FormEServicesWebSchedRecall:FormODBase {
		...
		private void butWebSchedPickProv_Click(object sender,EventArgs e) {
			...
			if(comboWebSchedProviders.SelectedIndex>0) {
				formProviderPick.SelectedProvNum=_provNum;
			}
			formProviderPick.ShowDialog();
			if(formProviderPick.DialogResult!=DialogResult.OK) {
				return;
			}
			comboWebSchedProviders.SelectedIndex=_listProvidersAll.FindIndex(x => x.ProvNum==formProviderPick.SelectedProvNum)+1;//+1 for 'All'
