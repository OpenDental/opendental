...
///<summary></summary>
	public partial class FormQueryFavorites:FormODBase {
		...
		public IsValidQuery IsQueryAllowed = (query) => true;//By default all queries are allowed.
		...
		private void FillGrid(bool refreshList=true,bool isScrollToSelection=true) {
			...
			foreach(UserQuery queryCur in listDisplayQueries) {
				...
				if(!IsQueryAllowed(queryCur)) {
					//Filter out queries that the caller of this form doesn't want included.
					continue;
				}
		...
		public delegate bool IsValidQuery(UserQuery query);
	}
}
