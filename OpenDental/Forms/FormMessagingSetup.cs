...
namespace OpenDental {
	...
	public partial class FormMessagingSetup : FormODBase {
		...
		private void butAdd_Click(object sender,EventArgs e) {
			...
			formSigElementDefEdit.ElementCur=new SigElementDef();
			formSigElementDefEdit.ElementCur.LightColor=Color.White;
			//default is user
			if(listExtras.SelectedIndex!=-1){
				formSigElementDefEdit.ElementCur.SigElementType=SignalElementType.Extra;
			}
			if(listMessages.SelectedIndex!=-1) {
				formSigElementDefEdit.ElementCur.SigElementType=SignalElementType.Message;
			}
			...
			//set the order
			SigElementDef sigElementDef=formSigElementDefEdit.ElementCur.Copy();
		...
		private void listToFrom_DoubleClick(object sender,EventArgs e) {
			...
			using FormSigElementDefEdit formSigElementDefEdit=new FormSigElementDefEdit();
			formSigElementDefEdit.ElementCur=listToFrom.GetSelected<SigElementDef>();
			formSigElementDefEdit.ShowDialog();
			if(formSigElementDefEdit.DialogResult!=DialogResult.OK) {
				return;
			}
		...
		private void listExtras_DoubleClick(object sender,EventArgs e) {
			...
			using FormSigElementDefEdit formSigElementDefEdit=new FormSigElementDefEdit();
			formSigElementDefEdit.ElementCur=listExtras.GetSelected<SigElementDef>();
			formSigElementDefEdit.ShowDialog();
			if(formSigElementDefEdit.DialogResult!=DialogResult.OK) {
				return;
			}
		...
		private void listMessages_DoubleClick(object sender,EventArgs e) {
			...
			using FormSigElementDefEdit formSigElementDefEdit=new FormSigElementDefEdit();
			formSigElementDefEdit.ElementCur=listMessages.GetSelected<SigElementDef>();
			formSigElementDefEdit.ShowDialog();
			if(formSigElementDefEdit.DialogResult!=DialogResult.OK) {
				return;
			}
