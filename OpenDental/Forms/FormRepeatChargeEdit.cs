...
namespace OpenDental{
	...
	public partial class FormRepeatChargeEdit :FormODBase{
		...
		public FormRepeatChargeEdit(RepeatCharge repeatCur)
		{
			...
			_isMassEmail=_repeatCur.ProcCode==ProcedureCodes.GetProcCodeForEService(eServiceCode.EmailMassUsage);
		}
			...
			if(PrefC.IsODHQ && EServiceCodeLink.IsProcCodeAnEService(_repeatCur.ProcCode,out _eService)) {
				...
				if(_isForZipwhip || _isMassEmail) {
					...
					if(CompareDouble.IsGreaterThan(_repeatCur.ChargeAmtAlt,-1)) {
						textZipwhipChargeAmount.Text=Currency.Round(_repeatCur.ChargeAmtAlt).ToString();
					}
				}
		...
		private void butManual_Click(object sender,EventArgs e) {
			...
			Procedure proc;
			try {
				proc=RepeatCharges.AddProcForRepeatCharge(chargeManual,DateTime.Today,DateTime.Today,orthoCaseProcLinkingData:new OrthoCaseProcLinkingData(_repeatCur.PatNum));
			}
		...
		private bool UpdateRepeatCharge(RepeatCharge repeatCharge) {
			...
			if((_isForZipwhip || _isMassEmail) && CompareDouble.IsGreaterThan(_repeatCur.ChargeAmtAlt,-1) && textZipwhipChargeAmount.Text.Trim()=="") {
				if(_isMassEmail) {
			...
			if((_isForZipwhip || _isMassEmail) && textZipwhipChargeAmount.Text.Trim()!="") {
