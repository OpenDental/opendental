	...
	public partial class FormProvAdditional:FormODBase {
		...
		private void FillGrid() {
			...
			if(PrefC.HasClinicsEnabled) {
				foreach(Clinic clinicCur in Clinics.GetForUserod(Security.CurUser)) {
					...
					//This must be checked right after adding CareCreditMerchantId cell.
					if(CareCredit.IsMerchantNumClosed(provClinic.CareCreditMerchantId)) {
						row.Cells.Last().ColorText=Color.Red;
						labelMerchantClosedDescription.Visible=true;
					}
					row.Tag=provClinic;
		...
		private void gridProvProperties_CellLeave(object sender,ODGridClickEventArgs e) {
			...
			if(e.Col==1) {
				...
				selectedRow.Cells[5].ColorText=Color.Black;
				if(CareCredit.IsMerchantNumClosed(provClin.CareCreditMerchantId)) {
					selectedRow.Cells[5].ColorText=Color.Red;
				}
			}
			labelMerchantClosedDescription.Visible=false;
		...
		private bool AreMerchantNumbersValid(List<ProviderClinic> listProvClinics) {
			List<ProviderClinic> listInvalidMerchantNums=listProvClinics.FindAll(x => IsProvClinicModified(x)
				...
				&& !CareCredit.IsMerchantNumValid(x.CareCreditMerchantId,allowClosedMerchant:true));//allowClosedMerchant so user can leave closed MID.
