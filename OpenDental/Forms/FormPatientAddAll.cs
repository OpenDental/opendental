...
namespace OpenDental {
	public partial class FormPatientAddAll:FormODBase {
			...
			for(int i=0;i<listConditions.Count;i++) {
				...
				switch(listConditions[i].ConditionType) {
					...
					case RequiredFieldName.PrimaryProvider:
						...
						List<ComboBoxOD> listProviderCombos=new List<ComboBoxOD>();
		...
		///<summary>Puts an asterisk next to the label if the field is required and the conditions are met. If the disallowedIdx is also selected, 
		///highlights the combobox background.</summary>
		private void SetRequiredComboBoxOD(Label labelCur,ComboBoxOD comboBoxOD,bool areConditionsMet,int disallowedIdx,string errorMsg,
			ErrorIconAlignment errorIconAlignment=ErrorIconAlignment.MiddleRight)
		{
			if(areConditionsMet) {
				labelCur.Text=labelCur.Text.Replace("*","")+"*";
				if(comboBoxOD.SelectedIndex==disallowedIdx) {
					_isMissingRequiredFields=true;
					if(_isValidating) {
						_errorProv.SetError(comboBoxOD,errorMsg);
						_errorProv.SetIconAlignment(comboBoxOD,errorIconAlignment);
					}
				}
				else {
					_errorProv.SetError(comboBoxOD,"");
				}
			}
			else {
				labelCur.Text=labelCur.Text.Replace("*","");
		...
		private void SetRequiredListOrComboControlNonGuarantor(Label labelCur,TextBox textBoxFName,TextBox textBoxLName,Control controlRequired,
		...
		{
			if(string.IsNullOrWhiteSpace(textBoxFName.Text) && string.IsNullOrWhiteSpace(textBoxLName.Text)) {
				_errorProv.SetError(controlRequired,"");
				return;
			}
			//Either ListBox or ComboBoxOD.
			if(controlRequired is ListBox) {
				SetRequiredListControl(labelCur,(ListControl)controlRequired,areConditionsMet,disallowedIdx,errorMsg,errorIconAlignment);
			}
			else {
				SetRequiredComboBoxOD(labelCur,(ComboBoxOD)controlRequired,areConditionsMet,disallowedIdx,errorMsg,errorIconAlignment);
			}
		}
		...
		private void butPick1_Click(object sender,EventArgs e) {
			...
			selectedPlan1=FormIP.InsPlanSelected.Copy();
			//Non-synched fields:
		...
		private void butPick2_Click(object sender,EventArgs e) {
			...
			selectedPlan2=FormIP.InsPlanSelected.Copy();
			//Non-synched fields:
			...
			for(int i=0;i<arrayPatsInFam.Length;i++) {
				...
				switch(i) {
					case 0://guarantor
						...
						pat.Gender=listGender1.GetSelected<PatientGender>();
					...
					case 1://patient 2
						...
						pat.Gender=listGender2.GetSelected<PatientGender>();
					...
					case 2://patient 3
						...
						pat.Gender=listGender3.GetSelected<PatientGender>();
					...
					case 3://patient 4
						...
						pat.Gender=listGender4.GetSelected<PatientGender>();
						pat.Position=PatientPosition.Child;
					...
					case 4://patient 5
						...
						pat.Gender=listGender5.GetSelected<PatientGender>();
						pat.Position=PatientPosition.Child;
