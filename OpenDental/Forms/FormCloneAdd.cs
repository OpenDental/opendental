	...
	public partial class FormCloneAdd:FormODBase {
		...
		private void FillClinicComboBoxes() {
			...
			if(listDefLinksPatsCur.Count>0) {
				listDefLinksClinics.RemoveAll(x => ListTools.In(x.DefNum,listDefLinksPatsCur.Select(y => y.DefNum).ToList()));
			}
			...
			if(listDefsSpecialties.Count > 0) {
				listClinicsNoSpecialties.RemoveAll(x => ListTools.In(x.ClinicNum,listDefLinksClinics.Select(y => y.FKey).ToList()));
			}
			//Remove all clinics that do not have any specialties from the original list of clinics for the user.
			listClinicsForUsers.RemoveAll(x => !ListTools.In(x.ClinicNum,listDefLinksClinics.Select(y => y.FKey).ToList()));
			//Filter out any specialties that are not associated to any available clinics for this user.
			listDefsSpecialties.RemoveAll(x => !ListTools.In(x.DefNum,listDefLinksClinics.Select(y => y.DefNum).ToList()));
			...
			for(int i=0;i<listDefsSpecialties.Count;i++) {
				...
				_listSpecialtyClinics.Add(new SpecialtyClinics() { 
					...
					ListClinics=listClinicsForUsers.FindAll(x => ListTools.In(x.ClinicNum,listDefLinks.Select(y => y.FKey).ToList()))
				});
			}
		...
		private void FillComboSpecialtyNoClinics() {
			...
			//Filter out any specialties that are currently in use by clones of this patient.
			if(listDefLinksPatsCur.Count>0) {
				listDefsSpecialties.RemoveAll(x => ListTools.In(x.DefNum,listDefLinksPatsCur.Select(y => y.DefNum).ToList()));
			}
			comboSpecialty.Items.Clear();
		...
		private void butPickPrimary_Click(object sender,EventArgs e) {
			...
			if(comboPriProv.SelectedIndex > -1) {//Initial FormP selection if selected prov is not hidden.
				formProviderPick.SelectedProvNum=_provNumSelected;
			}
			formProviderPick.ShowDialog();
			if(formProviderPick.DialogResult!=DialogResult.OK) {
				return;
			}
			comboPriProv.SelectedIndex=_listProviders.FindIndex(x => x.ProvNum==formProviderPick.SelectedProvNum);
			_provNumSelected=formProviderPick.SelectedProvNum;
		}
