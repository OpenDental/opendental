...
namespace OpenDental{
	...
	public partial class FormClaimEdit : FormODBase {
		...
		private void FormClaimEdit_Load(object sender, System.EventArgs e) {
			...
			else if(ListTools.In(_claim.ClaimStatus,ClaimStatus.Sent.GetDescription(true),ClaimStatus.Received.GetDescription(true))){//sent or received
      ...
      _listClaimProcs=_loadData.ListClaimProcs;
		...
		private void SetFormReadOnly(System.Windows.Forms.Control controlsInput) {
			for(int i=0;i<controlsInput.Controls.Count;i++) {
				...
				try {
					...
					if(controlsInput.Controls[i]==butCancel || controlsInput.Controls[i]==tabMain) {
						continue;
					}
		...
		private void comboClaimStatus_SelectionChangeCommitted(object sender,EventArgs e) {
			ClaimStatus claimStatus=_listClaimStatuses.Where(x=>x.GetDescription(true)==_claimOld.ClaimStatus).FirstOrDefault();
		...
		private void FillGrids(bool doRefreshData=true){
			...
			gridProc.Columns.Add(new GridColumn(Lan.g("TableClaimProc","WriteOff"),62,HorizontalAlignment.Right));
			...
			if(CultureInfo.CurrentCulture.Name.EndsWith("CA")) {
				...
				for(int i=0;i<listProceduresLab.Count;i++) {
					//introduce list of ClaimProcs
					if(_dictCanadianLabClaimProcs.ContainsKey(listProceduresLab[i].ProcNumLab)) {	
						_dictCanadianLabClaimProcs[listProceduresLab[i].ProcNumLab].AddRange(_listClaimProcs.FindAll(x => x.ProcNum==listProceduresLab[i].ProcNum && x.ClaimNum==_claim.ClaimNum));
						continue;
					}
					_dictCanadianLabClaimProcs.Add(listProceduresLab[i].ProcNumLab,_listClaimProcs.FindAll(x => x.ProcNum==listProceduresLab[i].ProcNum && x.ClaimNum==_claim.ClaimNum));
				}
			}
			for(int i=0;i<_listClaimProcsForClaim.Count;i++){
				row=new GridRow();
				...
				if(CultureInfo.CurrentCulture.Name.EndsWith("CA") && _dictCanadianLabClaimProcs.ContainsKey(_listClaimProcsForClaim[i].ProcNum)) {//This proc has one or more labs attached.
					List<ClaimProc> listClaimProcsForProcNum=_dictCanadianLabClaimProcs[_listClaimProcsForClaim[i].ProcNum];
					for(int j=0;j<listClaimProcsForProcNum.Count;j++) {
						...
						if(_listClaimProcsForClaim[i].Status!=listClaimProcsForProcNum[j].Status//Needed for supplemental payments. Original estimates will show together and all supplementals will show together.
							|| (_listClaimProcsForClaim[i].Status==ClaimProcStatus.Supplemental && _listClaimProcsForClaim[i].DateCP!=listClaimProcsForProcNum[j].DateCP))//Needed when there are multiple supplemental payments.
						{
						...
						listClaimProcsIntermingled.Add(listClaimProcsForProcNum[j]);
						ClaimProcRowHelper(labRow,listClaimProcsForProcNum[j]);
						labRow.Tag=listClaimProcsForProcNum[j];
		...
		private void gridProc_CellClick(object sender,ODGridClickEventArgs e) {
			if(ListTools.In(_claim.ClaimStatus,ClaimStatus.Sent.GetDescription(true),ClaimStatus.Received.GetDescription(true))) {//sent or received
		...
		private void SelectLabProcs() {
			...
			for(int i=0;i<gridProc.SelectedIndices.Count();i++) {
				...
				if(procedure.ProcNumLab==0 && _dictCanadianLabClaimProcs.ContainsKey(procedure.ProcNum)) {//This proc has one or two labs attached.
					...
					if(GetListLabsForProc(procedure.ProcNumLab,_listClaimProcsForClaim[index]).Count>1) {//User already clicked the 1 lab and it is already selected when 1.
		...
		private void butOtherCovChange_Click(object sender, System.EventArgs e) {
			...
			_claim.PlanNum2=formInsPlanSelect.SelectedPlan.PlanNum;
			_claim.InsSubNum2=formInsPlanSelect.SelectedSub.InsSubNum;
			textPlan2.Text=InsPlans.GetDescript(_claim.PlanNum2,_family,_listInsPlans,_claim.InsSubNum2,_listInsSubs);
			if(textPlan2.Text==""){
				comboPatRelat2.Visible=false;
				label10.Visible=false;
		...
		private void butPayProc_Click(object sender, System.EventArgs e) {
			...
			if(listClaimProcs.Count > 0) {
				if(_claim.ClaimType=="PreAuth") {
					...
					formClaimPayPreAuth.ClaimProcsToEdit=listClaimProcs;
					...
					formClaimPayTotal.ClaimProcsToEdit=listClaimProcs.ToArray();
					formClaimPayTotal.ShowDialog();
					if(formClaimPayTotal.DialogResult!=DialogResult.OK){
						return;
					}
		...
		private void MakeSuppPayment() {
			...
			using FormClaimPayTotal formClaimPayTotal=new FormClaimPayTotal(_patient,_family,_listInsPlans,_listPatPlans,_listInsSubs,GetBlueBookEstimateData());
			formClaimPayTotal.ClaimProcsToEdit=listClaimProcsSupplemental.ToArray();
			formClaimPayTotal.ShowDialog();
			if(formClaimPayTotal.DialogResult!=DialogResult.OK) {
...
//fix: need to debug the recalculation feature to take this status into account.
		...
		private void butReferralSelect_Click(object sender,EventArgs e) {
			...
			_claim.ReferringProv=formReferralSelect.SelectedReferral.ReferralNum;
		...
		private void butPreview_Click(object sender, System.EventArgs e) {
			...
			formClaimPrint.PatNumCur=_claim.PatNum;
			formClaimPrint.ClaimNumCur=_claim.ClaimNum;
		...
		private void SendClaim() {
			...
			if(claimSendQueueItemsArrayCA[0].MissingData!=""){
				...
				try {
					ClaimConnect.ValidateClaimResponse validateClaimResponse=ClaimConnect.ValidateClaim(_claim,true);
		...
		private void butReverse_Click(object sender,EventArgs e) {
			...
			_isCanadianClaimSending=true;
			try {
				long etransNumAck=CanadianOutput.SendClaimReversal(clearinghouseClin,_claim,insPlan,insSub,false,FormCCDPrint.PrintCCD);
		...
		public static bool AddClaimCustomTracking(Claim claim,string noteText="") {
			...
			return ((formClaimCustomTrackingUpdate.ListClaimTrackingsNew.FirstOrDefault()?.TrackingErrorDefNum??0)!=0);
		}
		...
		private ClaimEdit.UpdateData UpdateClaim(){
			...
			bool wasSentOrReceived=ListTools.In(_claim.ClaimStatus,ClaimStatus.Sent.GetDescription(true),ClaimStatus.Received.GetDescription(true));
				...
				|| _claim.ClaimType=="Cap") //all cap claimprocs are just duplicates
			{
				...
				for(int i=0;i<_listClaimProcsForClaim.Count;i++) {
					...
					if(_dictCanadianLabClaimProcs.ContainsKey(_listClaimProcsForClaim[i].ProcNum)) {//If proc "i" has labs.
						foreach(ClaimProc claimProcLab in _dictCanadianLabClaimProcs[_listClaimProcsForClaim[i].ProcNum]) {//Foreach lab FIX IT
