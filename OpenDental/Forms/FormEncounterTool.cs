	...
	public partial class FormEncounterTool:FormODBase {
		...
		private void butEncSnomed_Click(object sender,EventArgs e) {
			...
			for(int i=1;i<comboEncCodes.Items.Count;i++) {
				if(formSnomeds.SelectedSnomed.SnomedCode==comboEncCodes.Items[i].ToString()) {//if they selected one of the recommended codes, select in list
					comboEncCodes.SelectedIndex=i;
					textEncCodeValue.Clear();
					textEncCodeDescript.Text=formSnomeds.SelectedSnomed.Description;
					labelEncWarning.Visible=false;
					return;
				}
			}
			comboEncCodes.SelectedIndex=-1;
			textEncCodeValue.Text=formSnomeds.SelectedSnomed.SnomedCode;
			textEncCodeDescript.Text=formSnomeds.SelectedSnomed.Description;
