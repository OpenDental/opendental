	...
	public partial class FormBilling : FormODBase {
			...
			ref PdfDocument outputDocument,ref int skippedDeleted,ref int texted) 
		{
			...
			while(numOfBatchesSent<=numOfBatchesTotal) {
				//somewhere in here is where you would "batch". Or use a for loop instead of the while loop
			...
			foreach(Statement statement in listStatements) {
				...
				if(statement.Mode_==StatementMode.Email) {
					...
					try {
						...
						if(Enum.TryParse(ClinicPrefs.GetPrefValue(PrefName.EmailDefaultSendPlatform,clinicNumPat),out EmailPlatform emailPlatform)
							&& emailPlatform==EmailPlatform.Secure 
							&& Clinics.IsSecureEmailEnabled(clinicNumPat))
						{
							message.SentOrReceived=EmailSentOrReceived.SecureEmailSent;
