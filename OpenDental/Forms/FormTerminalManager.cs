	...
	public partial class FormTerminalManager:FormODBase {
		...
		private void FillGrid() {
			...
			foreach(SheetDevice device in listDevices) {
				...
				if(device.MobileDevice!=null) {
					row.Cells.Add(new GridCell(device.MobileDevice.DevicePage.GetDescription()));
				}
				else if(device.ComputerKiosk!=null) {
					row.Cells.Add(new GridCell(device.ComputerKiosk.TerminalStatus.GetDescription()));
				}
				else {
				...
				void CellClick(object sender,EventArgs e) {
					...
					if(device.PatNum==0) { //we are trying to load the patient
						...
						device.SetPatNum(FormOpenDental.PatNumCur);
					}
		...
		private void butPatForms_Click(object sender,EventArgs e) {
			...
			SheetDevice device=(SheetDevice)gridMain.ListGridRows[gridMain.GetSelectedIndex()].Tag;
			formPF.PatNum=device.PatNum==0?FormOpenDental.PatNumCur:device.PatNum;
			formPF.ShowDialog();
			FillPat();
		}		
		...
		private class SheetDevice {
			...
			public TerminalActive ComputerKiosk;
			...
			public string Name {
				get {
					return IsKiosk?ComputerKiosk.ComputerName:MobileDevice.DeviceName;
				}
			}
			...
			public string SessionName {
				get {
					return IsKiosk?ComputerKiosk.SessionName:"eClipboard";
				}
			}
			...
			public long PatNum {
				get {
					return IsKiosk?ComputerKiosk.PatNum:MobileDevice.PatNum;
				}
			}
			...
			public string PatName {
				get {
					long patNum=IsKiosk?ComputerKiosk.PatNum:MobileDevice.PatNum;
					if(!IsKiosk && patNum==-1) {
			...
			public string ClinicDesc {
				get {
					return IsKiosk?"Unassigned":Clinics.GetDesc(MobileDevice.ClinicNum);
				}
			}
			...
			public bool Matches(SheetDevice device) {
				if(IsKiosk && device.IsKiosk) {
					return this.ComputerKiosk.TerminalActiveNum==device.ComputerKiosk.TerminalActiveNum;
				}
				else if(!IsKiosk && !device.IsKiosk) {
					return this.MobileDevice.MobileAppDeviceNum==device.MobileDevice.MobileAppDeviceNum;
				}
			...
			public void Delete() {
				if(IsKiosk) { 
					TerminalActives.DeleteForCmptrSessionAndId(ComputerKiosk.ComputerName,ComputerKiosk.SessionId,processId:ComputerKiosk.ProcessId);
				}
				else {
					MobileAppDevices.Delete(MobileDevice.MobileAppDeviceNum);
				}
			}
			...
			public void SetPatNum(long patNum) {
				if(IsKiosk) {
					TerminalActives.SetPatNum(ComputerKiosk.TerminalActiveNum,patNum);
					...
					MobileAppDevices.SetPatNum(MobileDevice.MobileAppDeviceNum,patNum);
					if(patNum>0) {
						OpenDentBusiness.WebTypes.PushNotificationUtils.CI_CheckinPatient(patNum,MobileDevice.MobileAppDeviceNum);
					}
					else {
						OpenDentBusiness.WebTypes.PushNotificationUtils.CI_GoToCheckin(MobileDevice.MobileAppDeviceNum);
					}
				}
			}
		}
