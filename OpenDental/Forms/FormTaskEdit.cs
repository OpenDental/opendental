	...
	public partial class FormTaskEdit:FormODBase {
		...
		private void butAutoNote_Click(object sender,EventArgs e) {
			...
			if(FormA.DialogResult==DialogResult.OK) {
				textDescript.AppendText(FormA.StrCompletedNote);
				butEditAutoNote.Visible=GetHasAutoNotePrompt();
			}
		}
		...
		private void listCategory_SelectionChangeCommitted(object sender,EventArgs e) {
			Def selectedCategory=listCategory.GetSelected<Def>();
			if(selectedCategory==null) {
				MsgBox.Show(this,"Invalid Triage Category. Please select a valid Triage Category.");
				return;
			}
			_taskCur.TriageCategory=selectedCategory.DefNum;
		}
		...
		private void butChange_Click(object sender,System.EventArgs e) {
			...
			if(_taskCur.ObjectType==TaskObjectType.Patient) {
				_taskCur.KeyNum=FormPS.PatNumSelected;
			}
			if(_taskCur.ObjectType==TaskObjectType.Appointment) {
				using FormApptsOther FormA=new FormApptsOther(FormPS.PatNumSelected,null);//Select only, can't create new appt so don't need pinboard appointments.
		...
		private void butChangeUser_Click(object sender,EventArgs e) {
			...
			if(FormChangeUser.DialogResult==DialogResult.OK) {
				_taskCur.UserNum=FormChangeUser.UserodSimpleSwitch.UserNum; //assign task new UserNum
		...
		private bool SaveCur() {
			...
			_taskCur.IsReadOnly=checkIsReadOnly.Checked;
			//Original task was read only, updated  task is not.
			if(!_taskCur.IsReadOnly && _taskOld!=null && _taskOld.IsReadOnly) {
				bool result=MsgBox.Show(MsgBoxButtons.YesNo,"Original task was read only. Set updated task to read only?");
				_taskCur.IsReadOnly=result;
			}
		...
		private void butDelete_Click(object sender,EventArgs e) {
			...
			int countDocuments=TaskAttachments.GetCountDocumentForTaskNum(_taskCur.TaskNum);
			Tasks.Delete(_taskCur.TaskNum);//always do it this way to clean up all five tables (six if hq)
			if(countDocuments>0) { 
			...
			SecurityLogs.MakeLogEntry(Permissions.TaskDelete,0,"Task "+POut.Long(_taskCur.TaskNum)+" deleted",0);
			DialogResult=DialogResult.OK;
			Close();
		}
		...
		private void SetFormReadOnly(bool doSetReadOnly) {
			...
			else if(checkIsReadOnly.Checked) {
		...
		private void butCancel_Click(object sender,System.EventArgs e) {
			...
			if(IsNew) { 
				List<TaskAttachment> listTaskAttachments=TaskAttachments.GetManyByTaskNum(_taskCur.TaskNum);
		...
		private void ButEditAutoNote_Click(object sender,EventArgs e) {
			if(GetHasAutoNotePrompt()) {
				using FormAutoNoteCompose FormA=new FormAutoNoteCompose();
				FormA.StrMainTextNote=textDescript.Text;
				FormA.ShowDialog();
				if(FormA.DialogResult==DialogResult.OK) {
					textDescript.Text=FormA.StrCompletedNote;
					butEditAutoNote.Visible=GetHasAutoNotePrompt();
				}
			}
			else {
