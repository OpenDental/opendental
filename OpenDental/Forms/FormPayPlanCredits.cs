...
namespace OpenDental {
	...
	public partial class FormPayPlanCredits:FormODBase {
		...
		private List<PayPlanEdit.PayPlanEntry> CreatePayPlanEntries(bool showImplicitlyPaidOffProcs) {
			...
			listPayPlanEntries.AddRange(PayPlanEdit.CreatePayPlanEntriesForAccountCharges(_constructResults.ListAccountEntries,_patCur,_payPlanCur));
			ListPayPlanCreditsCur=ListPayPlanCreditsCur.OrderBy(x => x.ChargeDate).ToList();
			listPayPlanEntries.AddRange(PayPlanEdit.CreatePayPlanEntriesForPayPlanCredits(_constructResults.ListAccountEntries,ListPayPlanCreditsCur));
		...
		private void butAddOrUpdate_Click(object sender,EventArgs e) {
			...
			List<long> listDynamicPayPlanProcNums=_constructResults.ListAccountEntries.FindAll(x => x.GetType()==typeof(FauxAccountEntry)
