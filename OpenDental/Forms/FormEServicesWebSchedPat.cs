...
namespace OpenDental {
	public partial class FormEServicesWebSchedPat:FormODBase {
		...
		private void FillBlockoutTypes() {
			...
			List<Def> listDefsAllowedBlockouts=Defs.GetDefs(DefCat.BlockoutTypes,listAllowedBlockoutTypes.FindAll(x => !ListTools.In(x,listDefNumsRestrictedTo)));
			...
			for(int i=0;i<listDefsRestrictedToBlockouts.Count;i++) {
				...
				listItemNamesAssociatedReason=listDefsForApptTypes.Where(x => ListTools.In(x.DefNum,listDefNumsAssociatedReason)).Select(x => x.ItemName).ToList();
