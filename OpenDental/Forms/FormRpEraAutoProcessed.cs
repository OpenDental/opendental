	...
	public partial class FormRpEraAutoProcessed:FormODBase {
		...
		private void FillGridEras() {
			...
			for(int i=0;i<eraData.ListEtrans.Count;i++) {
				...
				List<Hx835_ShortClaimProc> listShortClaimProcsForRow=listShortClaimProcs.FindAll(x => ListTools.In(x.ClaimNum,listClaimNumsForRow));
		...
		private void MenuItemProcessSecondaryClaims_Click(object sender,EventArgs e) {
			...
			if(!shortClaimsUnsent.Any(x => x.ClaimType=="S" && ListTools.In(x.ClaimStatus,"U","H"))) {//S=Secondary, U=Unsent, H=Hold Until Pri Recieved.
				MsgBox.Show(this,"There are unsent claim(s) for one or more procedures on the selected claim, " 
					+"but none of them are for secondary claims with a status of 'Unsent' or 'Hold Until Pri Received'. "
