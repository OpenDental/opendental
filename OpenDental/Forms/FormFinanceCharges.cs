	...
	public partial class FormFinanceCharges : FormODBase {
		...
		private void FormFinanceCharges_Shown(object sender,EventArgs e) {
			if(PrefC.GetBool(PrefName.AgingIsEnterprise)) {
				...
				DateTime dateAsOf=DateTime.Today;
		...
		private void butOK_Click(object sender,System.EventArgs e) {
			...
			ProgressOD progressODAging=new ProgressOD();
			progressODAging.ActionMain=() => Ledgers.RunAging();
			progressODAging.StartingMessage=Lan.g(this,"Calculating aging for all patients as of")+" "+dateAsOf.ToShortDateString()+"...";
			progressODAging.MessageCancel=Lan.g(this,"You should not Cancel because this might leave some or all patient balances set to 0. If you do cancel, make sure to run Aging again. Cancel anyway?");
			try{
				progressODAging.ShowDialogProgress();
			}
			catch(Exception ex){
				Ledgers.AgingExceptionHandler(ex,this);
				return;
			}
			if(progressODAging.IsCancelled){
				return;
			}
			SecurityLogs.MakeLogEntry(Permissions.AgingRan,0,"Aging complete - Finance Charges window");
			DialogResult = DialogResult.OK;
