...
namespace OpenDental {
	public partial class FormTaskSearch:FormODBase {
		...
		private List<Userod> _listUsersNotHidden;
		...
		private void FormTaskSearch_Load(object sender,EventArgs e) {
			...
			long userNum=0;
			_listUsersAll=Userods.GetDeepCopy();
			_listUsersNotHidden=_listUsersAll.Where(x => !x.IsHidden).ToList();
			FillComboUsers();
		...
		private void FillComboUsers() {
			...
			if(checkShowHiddenUsers.Checked) {
				comboUsers.Items.AddList(_listUsersAll,x => x.UserName);
				return;
			}
			comboUsers.Items.AddList(_listUsersNotHidden,x => x.UserName);
		}
		...
		private void butPatPicker_Click(object sender,EventArgs e) {
			...
			if(FormPS.ShowDialog()==DialogResult.OK) {
				long patNum=FormPS.PatNumSelected;
		...
		private void butUserPicker_Click(object sender,EventArgs e) {
			...
			FormUP.ListUserodsFiltered=_listUsersAll;
			FormUP.IsSelectionmode=true;
			if(!checkShowHiddenUsers.Checked) {
				FormUP.ListUserodsFiltered=_listUsersNotHidden;
			}
			if(FormUP.ShowDialog()==DialogResult.OK) {
				comboUsers.SetSelectedKey<Userod>(FormUP.SelectedUserNum,x => x.UserNum);
			}
		}
