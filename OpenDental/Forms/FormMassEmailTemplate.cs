...
namespace OpenDental {
	public partial class FormMassEmailTemplate:FormODBase {
		...
		private bool _isImportOrCopy {
			get {
				return _emailHostingTemplate.IsNew && !string.IsNullOrEmpty(_emailHostingTemplate.BodyHTML);
			}
		}
		...
		private void FormMassEmailTemplate_Load(object sender,EventArgs e) {
			if(_isImportOrCopy && string.IsNullOrEmpty(_emailHostingTemplate.BodyPlainText)) {
		...
		private void butDeleteTemplate_Click(object sender,EventArgs e) {
			...
			if(_emailHostingTemplate.TemplateId!=0) {
				...
				try {
					iAccountApi.DeleteTemplate(new DeleteTemplateRequest {
						TemplateNum=_emailHostingTemplate.TemplateId,
					});
				}
				catch(Exception ex) {
		...
		private bool Save() {
			...
			if(_emailHostingTemplate.IsNew || _emailHostingTemplate.TemplateId==0) {//templates inserted during convert didn't have their Id's set, we need to create them.
				try {
					CreateTemplateResponse createTemplateResponse=iAccountApi.CreateTemplate(new CreateTemplateRequest { 
				...
				try {
					iAccountApi.UpdateTemplate(new UpdateTemplateRequest {//is back end expecting that template name can be changed now?
						TemplateNum=_emailHostingTemplate.TemplateId,
						Template=new Template {
							TemplateName=textTemplateName.Text,
							TemplateBodyHtml=xhtml,
