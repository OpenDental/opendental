...
namespace OpenDental{
	...
	public partial class FormClockEventEdit : FormODBase {
		...
		private void FillInitialControlsHelper() {
			...
			//Rate 3 Time -----------------------------
			textRate3Auto.Text=ClockEvents.Format(ClockEventCur.Rate3Auto);
			if(ClockEventCur.Rate3Hours==TimeSpan.FromHours(-1)) {
				textRate3Hours.Text="";
			}
			else if(ClockEventCur.Rate3Hours==TimeSpan.Zero) {
				textRate3Hours.Text="0";
			}
			else {
				textRate3Hours.Text=ClockEvents.Format(ClockEventCur.Rate3Hours);
			}
			//notes ------------------------------------------------------------------------------------
		...
		private void textRate2Hours_TextChanged(object sender,EventArgs e) {
			try {
				if(textRate2Hours.Text=="") {
					...
					if(ClockEventCur.Rate2Hours!=TimeSpan.Zero) {
						ClockEventCur.Rate3Hours=TimeSpan.Zero;
						textRate3Hours.Text="0";
					}
				}
			}
		...
		private void textRate3Hours_TextChanged(object sender,EventArgs e) {
			try {
				if(textRate3Hours.Text=="") {
					ClockEventCur.Rate3Hours=TimeSpan.FromHours(-1);
				}
				else {
					ClockEventCur.Rate3Hours=TimeSpan.FromHours(Double.Parse(textRate3Hours.Text));
					//No ClockEvent can have both Rate2 and Rate3 hours
					if(ClockEventCur.Rate3Hours!=TimeSpan.Zero) {
