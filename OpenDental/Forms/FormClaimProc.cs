...
namespace OpenDental {
	...
	public partial class FormClaimProc : FormODBase {
		...
		public void Initialize() {
			...
			else if((ListTools.In(_claimProc.Status,ClaimProcStatus.CapComplete,ClaimProcStatus.Received,ClaimProcStatus.Supplemental,ClaimProcStatus.InsHist))
			...
			if(_claimProc.ClaimNum>0) {//attached to claim
				...
				if(ListTools.In(_claimProc.Status,ClaimProcStatus.Received,ClaimProcStatus.NotReceived,ClaimProcStatus.CapClaim) 
			...
			else if(_claimProc.PlanNum>0 && ListTools.In(_claimProc.Status,ClaimProcStatus.CapEstimate,ClaimProcStatus.CapComplete)) { //not attached to a claim
			...
			if(_insPlan.PlanType!="c" && ListTools.In(_claimProcOld.Status,ClaimProcStatus.Estimate,ClaimProcStatus.InsHist)) {
				comboStatus.Enabled=false;
			}
		...
		private void FillAllowed(BlueBookEstimateData blueBookEstimateData=null){
			if(IsProc){
				decimal allowed=InsPlans.GetAllowedForProc(_procedure,_claimProc,_listInsPlans,GetListSubLinks(),null,blueBookEstimateData);
				if(allowed==-1){
		...
		private void butPickProv_Click(object sender,EventArgs e) {
			...
			if(comboProvider.SelectedIndex > -1) {
				formProviderPick.SelectedProvNum=_listProviders[comboProvider.SelectedIndex].ProvNum;
			}
			...
			comboProvider.SetSelectedKey<Provider>(formProviderPick.SelectedProvNum, x => x.ProvNum);
		}

		private void comboStatus_SelectionChangeCommitted(object sender,EventArgs e) {
			//new selected index will already be set
			if(!ListTools.In(_claimProcOld.Status,ClaimProcStatus.Estimate,ClaimProcStatus.InsHist)//not an estimate or inshist
		...
		private void butOK_Click(object sender, System.EventArgs e) {
			...
			if(PIn.Date(textDateCP.Text).Date > DateTime.Today.Date
				...
				&& ListTools.In(_claimProc.Status,ClaimProcStatus.Received,ClaimProcStatus.Supplemental,ClaimProcStatus.CapClaim,ClaimProcStatus.CapComplete)) 
			{ 
