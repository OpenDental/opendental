...
namespace OpenDental{
	...
	public partial class FormInsBenefits : FormODBase {
		...
		private void FillSimple(){
			...
			for(int i=0;i<_listBenefitsAll.Count;i++){
				...
				//BWs group
					...
					&& ProcedureCodes.GetStringProcCode(benefit.CodeNum)==ProcedureCodes.GetProcCode(PrefName.InsBenBWCodes)
				...
				//Pano
					...
					&& ProcedureCodes.GetStringProcCode(benefit.CodeNum)==ProcedureCodes.GetProcCode(PrefName.InsBenPanoCodes) 
					&& textPano.Text=="") 
				{
		...
		private bool ConvertFormToBenefits(bool isSilent=false){
			...
			//Flo
			if(textFlo.Text !=""){
				benefit=Benefits.CreateAgeLimitationBenefit(ProcedureCodes.GetCodeNumForPref(PrefName.InsBenFlourideCodes),PIn.Byte(textFlo.Text),_planNum);
				_listBenefitsAll.Add(benefit);
			}
			if(textSealantAge.Text!="") {
				benefit=Benefits.CreateAgeLimitationBenefit(ProcedureCodes.GetCodeNumForPref(PrefName.InsBenSealantCodes),PIn.Byte(textSealantAge.Text),_planNum);
				_listBenefitsAll.Add(benefit);
			}
			//Frequency BW group
			if(textBW.Text !="") {
				benefit=new Benefit();
				benefit.CodeNum=ProcedureCodes.GetCodeNumForPref(PrefName.InsBenBWCodes);
				benefit.BenefitType=InsBenefitType.Limitations;
				benefit.CovCatNum=0;
				benefit.PlanNum=_planNum;
			...
			if(textPano.Text !="") {
				benefit=new Benefit();
				benefit.CodeNum=ProcedureCodes.GetCodeNumForPref(PrefName.InsBenPanoCodes);
				benefit.BenefitType=InsBenefitType.Limitations;
				benefit.CovCatNum=0;
				benefit.PlanNum=_planNum;
