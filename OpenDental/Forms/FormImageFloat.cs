		...
		*/
		...
		public void formVideo_BitmapCaptured(object sender, VideoEventArgs e){
			...
			if(!IsMountShowing()){//single
				...
				EventSelectTreeNode?.Invoke(this,new NodeTypeAndKey(EnumImageNodeType.Document,doc.DocNum));
				return;
			}
		...
		public List<UnmountedObject> GetUmountedObjects(){
			...
			for(int i=0;i<_listMountItems.Count;i++){
				...
				unmountedObject.Document=_arrayDocumentsShowing[i];//could be null
				unmountedObject.Bitmap=_arrayBitmapsShowing[i];//could be null
		...
		public void InvalidateSettingsColor() {
			...
			if(IsMountItemSelected()){
				...
				if(_bitmapRaw!=null){
					_arrayBitmapsShowing[_idxSelectedInMount]=ImageHelper.CopyWithBrightContrast(_arrayDocumentsShowing[_idxSelectedInMount],_bitmapRaw);
				}
				if(_bitmapDicomRaw!=null){
		...
		public void SelectTreeNode(NodeTypeAndKey nodeTypeAndKey,string localPathImportedCloud="") {
			...
			if(nodeTypeAndKey.NodeType==EnumImageNodeType.Document){
				...
				if(document.ToothNumbers!=""){
					...
					Text+=Tooth.FormatRangeForDisplay(document.ToothNumbers);
				}
				...
				if(_bitmapRaw==null) {
					...
					else if(Path.GetExtension(document.FileName).ToLower()==".pdf") {//Adobe acrobat file.
		...
		public void ToolBarCopy_Click(){
			...
			if(IsMountItemSelected()){
				if(ImageHelper.HasImageExtension(GetDocumentShowing(_idxSelectedInMount).FileName) || GetDocumentShowing(_idxSelectedInMount).FileName.EndsWith(".dcm"))
				{
					...
					bitmapCopy=ImageHelper.CopyWithCropRotate(_arrayDocumentsShowing[_idxSelectedInMount],GetBitmapShowing(_idxSelectedInMount));
		...
		public void ToolBarRotateL_Click(){
			...
			if(IsMountItemSelected()) {
				...
				if(_arrayDocumentsShowing[_idxSelectedInMount].CropW>0
					&& _arrayDocumentsShowing[_idxSelectedInMount].CropX!=0 && _arrayDocumentsShowing[_idxSelectedInMount].CropY!=0)
				{
					MsgBox.Show(this,"Remove crop before attempting to change rotation.");
					return;
				}
				...
				if(_arrayDocumentsShowing[_idxSelectedInMount].CropW != 0){
					...
					int w=_arrayDocumentsShowing[_idxSelectedInMount].CropH;
					_arrayDocumentsShowing[_idxSelectedInMount].CropH=_arrayDocumentsShowing[_idxSelectedInMount].CropW;
					_arrayDocumentsShowing[_idxSelectedInMount].CropW=w;
				}
				_arrayDocumentsShowing[_idxSelectedInMount].DegreesRotated-=90;
		...
		public void ToolBarSize_Click(){
			...
			if(IsMountItemSelected()){
				...
				formDocumentSizeMount.SizeRaw=_arrayBitmapsShowing[_idxSelectedInMount].Size;
		...
		private void panelMain_MouseUp(object sender, MouseEventArgs e){
			...
			if(GetSelectedType()==EnumImageNodeType.Mount && !isClick && _cropPanAdj==EnumCropPanAdj.Adj && _isDraggingMount){
				...
				if(_arrayDocumentsShowing[_idxSelectedInMount].CropW==0 || _arrayDocumentsShowing[_idxSelectedInMount].CropH==0){
					Size sizeCrop=ImageHelper.CalcSizeFit(
						widthImage:_arrayBitmapsShowing[_idxSelectedInMount].Width,
						heightImage:_arrayBitmapsShowing[_idxSelectedInMount].Height,
						degreesRotated:_arrayDocumentsShowing[_idxSelectedInMount].DegreesRotated,
						sizeCanvas:new Size(_listMountItems[_idxSelectedInMount].Width,_listMountItems[_idxSelectedInMount].Height));
					_arrayDocumentsShowing[_idxSelectedInMount].CropW=sizeCrop.Width;
					_arrayDocumentsShowing[_idxSelectedInMount].CropH=sizeCrop.Height;
				}
				//convert the drag to image coords
				float scaleImage;
				if(_arrayDocumentsShowing[_idxSelectedInMount].CropW > 0 && _arrayDocumentsShowing[_idxSelectedInMount].CropH > 0){
					scaleImage=(float)_listMountItems[_idxSelectedInMount].Width/_arrayDocumentsShowing[_idxSelectedInMount].CropW;//example 100/200=.5 because image is smaller
				}
				else{
					...
					scaleImage=ImageTools.CalcScaleFit(
						...
						sizeImage:_arrayBitmapsShowing[_idxSelectedInMount].Size,
						degreesRotated:_arrayDocumentsShowing[_idxSelectedInMount].DegreesRotated);
				}
				...
				_arrayDocumentsShowing[_idxSelectedInMount].CropX+=xDrag;
				_arrayDocumentsShowing[_idxSelectedInMount].CropY+=yDrag;
				...
				Rectangle rectangleBitmap=new Rectangle(-_arrayBitmapsShowing[_idxSelectedInMount].Width/2,//left
					-_arrayBitmapsShowing[_idxSelectedInMount].Height/2,//up
					_arrayBitmapsShowing[_idxSelectedInMount].Width,
					_arrayBitmapsShowing[_idxSelectedInMount].Height);
				Region regionBitmap=new Region(rectangleBitmap);
				Matrix matrixRotate=new Matrix();
				matrixRotate.Rotate(_arrayDocumentsShowing[_idxSelectedInMount].DegreesRotated);
				regionBitmap.Transform(matrixRotate);
				Rectangle rectangleCrop=new Rectangle(
					_arrayDocumentsShowing[_idxSelectedInMount].CropX-_arrayDocumentsShowing[_idxSelectedInMount].CropW/2,
					-_arrayDocumentsShowing[_idxSelectedInMount].CropY-_arrayDocumentsShowing[_idxSelectedInMount].CropH/2,
					_arrayDocumentsShowing[_idxSelectedInMount].CropW,
					_arrayDocumentsShowing[_idxSelectedInMount].CropH);
			...
			if(_cropPanAdj==EnumCropPanAdj.Crop){
				...
				PointF[] pointFs={pointFCrop1,pointFCrop2};
				matrix.TransformPoints(pointFs);
				//I can't find the matrix command for flip vertically, so we'll do that part manually later.
				pointFCrop1=new PointF(pointFs[0].X,pointFs[0].Y);
				pointFCrop2=new PointF(pointFs[1].X,pointFs[1].Y);
		...
		private void ChangeColorAt(PointF pointF){
			...
			for(int i=0;i<_listImageDraws.Count;i++) {
				...
				if(_listImageDraws[i].DrawType==ImageDrawType.Text){
					...
					_listImageDraws[i].ColorDraw=ColorFore;//just change foreground. We can't change background here.
					ImageDraws.Update(_listImageDraws[i]);
					panelMain.Invalidate();
					return;
				}
		...
		private void DrawDrawings(Graphics g,float rotateText){
			for(int i=0;i<_listImageDraws.Count;i++){
				if(_listImageDraws[i].DrawType==ImageDrawType.Text){
					...
					g.DrawString(s,font,solidBrush,0,0);
		...
		private void DrawMountOne(Graphics g,int i,bool translateToItemPos=true){
			...
			if(_arrayDocumentsShowing[i].CropW > 0 && _arrayDocumentsShowing[i].CropH > 0){
				scale=(float)_listMountItems[i].Width/_arrayDocumentsShowing[i].CropW;//example 100/200=.5 because image needs to be smaller
			}
			else{
				//We don't have a real crop, so this handles the math:
				scale=ImageTools.CalcScaleFit(
					...
					sizeImage:_arrayBitmapsShowing[i].Size,
					degreesRotated:_arrayDocumentsShowing[i].DegreesRotated);
			}
			g.ScaleTransform(scale,scale);
			//We are in bitmap coords from here down.
			//Translate from center of crop area to center of image
			//This step is not intuitive because our crop coordinates are positive to UR instead of the LR that we are using here.
			g.TranslateTransform(-_arrayDocumentsShowing[i].CropX,_arrayDocumentsShowing[i].CropY);
			//Rotate and flip
			...
			//Translate from center of image to UL corner of image
			g.TranslateTransform(-_arrayBitmapsShowing[i].Width/2f,-_arrayBitmapsShowing[i].Height/2f);
		...
		private int HitTestText(Point pointPanel){
			for(int i=0;i<_listImageDraws.Count;i++){
				...
				PointF[] pointFs={pointTextInImage };
				matrix.TransformPoints(pointFs);
				Point pointTextInPanel=Point.Round(pointFs[0]);
		...
		private void LoadBitmap(int idx,EnumLoadBitmapType loadBitmapType){
			...
			if(loadBitmapType==EnumLoadBitmapType.OnlyIdx || loadBitmapType==EnumLoadBitmapType.IdxAndRaw){
				if(_arrayDocumentsShowing[idx].FileName.EndsWith(".dcm")){
					...
					_arrayBitmapsShowing[idx]=new Bitmap(bitmapTemp);
				}
				else{
					_arrayBitmapsShowing[idx]=new Bitmap(bitmapTemp);
					ImageHelper.ApplyColorSettings(_arrayBitmapsShowing[idx],_arrayDocumentsShowing[idx].WindowingMin,_arrayDocumentsShowing[idx].WindowingMax);
		...
		private void ShowScale(){
			FormImageScale formImageScale=new FormImageScale();
