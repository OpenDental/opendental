	...
	public partial class FormImageFloat:FormODBase {
		...
		*/
		...
		public void ClearPDFBrowser() {
			if(ODBuild.IsWeb()) {
				//ODCloud does not use _webBrowser
				_cloudIframe.HideIframe();
		...
		public void DocumentAcquiredForMount(Document document,Bitmap bitmap,MountItem mountItem){
		...
		public void formVideo_BitmapCaptured(object sender, VideoEventArgs e){
			...
			try {
				...
				document=ImageStore.Import(e.Bitmap,GetCurrentCategory(),ImageType.Photo,PatientCur);
			}
			catch(Exception ex) {
		...
		public bool HideWebBrowser(){
			if(ODBuild.IsWeb()) {
				//ODCloud uses _cloudIframe instead of _webBrowser
				_cloudIframe.HideIframe();
			}
			else {
				if(_webBrowser==null) {
		...
		public void ToolBarExport_Click(){
			...
			if(IsMountItemSelected()){
				...
				saveFileDialog.DefaultExt=Path.GetExtension(_documentArrayShowing[_idxSelectedInMount].FileName);
				if(saveFileDialog.ShowDialog()!=DialogResult.OK) {
					return;
				}
				if(ImageHelper.HasImageExtension(GetDocumentShowing(_idxSelectedInMount).FileName) || GetDocumentShowing(_idxSelectedInMount).FileName.EndsWith(".dcm"))
			...
			try {
				...
				if(!File.Exists(_webBrowserFilePath)) {
					...
					if(ODBuild.IsWeb()) {
						...
						_cloudIframe.DisplayFile(_webBrowserFilePath);
						_isExportable=true;
