	...
	public partial class FormDentalSchoolSetup:FormODBase {
		...
		private void butStudentPicker_Click(object sender,EventArgs e) {
			...
			if(dialogResult==DialogResult.Yes) {
				try {
					Userods.UpdateUserGroupsForDentalSchools(formUserGroupPicker.UserGroupSelected,false);
				}
				catch {
			...
			Prefs.UpdateLong(PrefName.SecurityGroupForStudents,formUserGroupPicker.UserGroupSelected.UserGroupNum);
			textStudents.Text=formUserGroupPicker.UserGroupSelected.Description;
		...
		private void butInstructorPicker_Click(object sender,EventArgs e) {
			...
			if(dialogResult==DialogResult.Yes) {
				try {
					Userods.UpdateUserGroupsForDentalSchools(formUserGroupPicker.UserGroupSelected,true);
				}
				catch {
			...
			//For now, only one user group can be defined as the default security group for students/instructors.
			Prefs.UpdateLong(PrefName.SecurityGroupForInstructors,formUserGroupPicker.UserGroupSelected.UserGroupNum);
			textInstructors.Text=formUserGroupPicker.UserGroupSelected.Description;
			DataValid.SetInvalid(InvalidType.Prefs);
		}
