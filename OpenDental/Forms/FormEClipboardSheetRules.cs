...
namespace OpenDental {
	public partial class FormEClipboardSheetRules:FormODBase {
		...
		private void FormEClipboardSheetRules_Load(object sender,EventArgs e) {
			...
			_listSheetDefs=SheetDefs.GetWhere(x=>ListTools.In(x.SheetDefNum,listSheetDefNumsIgnored));
		...
		private void butSelectSheetsToIgnore_Click(object sender,EventArgs e) {
			...
			formSheetPicker.ListSheetDefsExclude.Add(_eClipboardSheetDef.SheetDefNum);
			//Sets the list of sheets the sheet picker window will display.
			formSheetPicker.ListSheets=SheetDefs.GetCustomForType(sheetDef.SheetType);
			formSheetPicker.SelectedSheetDefs=formSheetPicker.ListSheets.Where(x=>_listSheetDefs.Select(y=>y.SheetDefNum).Contains(x.SheetDefNum)).ToList();
			...
			_listSheetDefs=formSheetPicker.SelectedSheetDefs;
			listSheetsToIgnore.Items.Clear();
			listSheetsToIgnore.Items.AddStrings(_listSheetDefs.Select(x=>x.Description).ToList());
		}
