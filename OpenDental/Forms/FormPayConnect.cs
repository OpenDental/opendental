...
namespace OpenDental {
	public partial class FormPayConnect:FormODBase {
		...
		private PayConnectResponse _pcResponse;
		...
		public PayConnectResponse Response {
			get {
				if(_response != null) {
					return PayConnectREST.ToPayConnectResponse(_response,_request);
				}
				if(_pcResponse != null) {
					return _pcResponse;
				}
		...
		private bool ProcessPaymentTerminal() {
			if(ODBuild.IsWeb()) {
				...
				if(radioSale.Checked) {
					_pcResponse=ODCloudClient.ProcessPaymentTerminal("SALE",PIn.Decimal(textAmount.Text),checkForceDuplicate.Checked);
				}
				else if(radioAuthorization.Checked) {
					_pcResponse=ODCloudClient.ProcessPaymentTerminal("AUTH",PIn.Decimal(textAmount.Text),checkForceDuplicate.Checked);
				}
				else if(radioVoid.Checked) {
					_pcResponse=ODCloudClient.ProcessPaymentTerminal("VOID",PIn.Decimal(textAmount.Text),checkForceDuplicate.Checked,textRefNumber.Text);
				}
				else if(radioReturn.Checked) {
					_pcResponse=ODCloudClient.ProcessPaymentTerminal("RETURN",PIn.Decimal(textAmount.Text),checkForceDuplicate.Checked,textRefNumber.Text);
				}
				...
				textCardNumber.Text=_pcResponse.CardNumber;
				textAmount.Text=_pcResponse.Amount.ToString("f");
				_receiptStr=PayConnectTerminal.BuildReceiptString(_pcResponse,null,_clinicNum);
				PrintReceipt(_receiptStr);
				return true;
			}//end of IsWeb()
			...
			progressOD.ActionMain=() => _pcResponse=PayConnectTerminal.ToPayConnectResponse(DpsPos.ProcessCreditCard(posRequest));
			...
			if(_pcResponse.StatusCode!="0") {//"0" indicates success. May need to check the AuthCode field too to determine if this was a success.
				MessageBox.Show(Lan.g(this,"Error message from Pay Connect:")+"\r\n"+_pcResponse.Description);
				return false;
			}
			PayConnectService.signatureResponse sigResponse=null;
			try {
				Cursor=Cursors.WaitCursor;
				sigResponse=SendSignature(_pcResponse.RefNumber.ToString());
				Cursor=Cursors.Default;
			}
			catch(Exception ex) {
				Cursor=Cursors.Default;
			...
			textCardNumber.Text=_pcResponse.CardNumber;
			textAmount.Text=_pcResponse.Amount.ToString("f");
			_receiptStr=PayConnectTerminal.BuildReceiptString(_pcResponse,sigResponse,_clinicNum);
			PrintReceipt(_receiptStr);
			return true;
		}
