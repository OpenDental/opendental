	...
	public partial class FormTransactionEdit : FormODBase {
		...
		private void butAdd_Click(object sender,EventArgs e) {
			...
			if(FormJ.DialogResult==DialogResult.OK) {
				JournalList.Add(FormJ.JournalEntryCur);
				if(checkMemoSame.Checked) {
					for(int i=0;i<JournalList.Count;i++) {
						((JournalEntry)JournalList[i]).Memo=FormJ.JournalEntryCur.Memo;
					}
				}
			}
			FillCompound();
		...
		private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			using FormJournalEntryEdit FormJ=new FormJournalEntryEdit();
			FormJ.JournalEntryCur=(JournalEntry)JournalList[e.Row];
			FormJ.ShowDialog();
			if(FormJ.JournalEntryCur==null){//user deleted
