...
namespace OpenDental {
	...
	public partial class FormImageFloatWindows:Form {
		//=========NOTE=======================================
		//Remember that this form is launched with reflection and late binding.
		//So various fields and events are set in FormImageFloat, even though there are no references here.
		//This late binding issue will be fixed soon.
		//====================================================
		public LayoutManagerForms LayoutManager;
		///<summary>This is set externally as the ControlImageDock. It will be null if FormOwner is set instead.</summary>
		public UserControl ControlOwner;
		///<summary>This is set externally as the FormImageFloat. It will be null if ControlOwner is set instead.</summary>
		...
		///<summary>Owner. Will be null if _formImageFloat is set instead.</summary>
		private ControlImageDock _controlImageDock;
		///<summary>Owner. Will be null if _controlImageDock is set instead.</summary>
		...
		System.Windows.Forms.Screen _screenThis;
		///<summary>Will be null if no other screen.</summary>
		private System.Windows.Forms.Screen _screen2;

		public FormImageFloatWindows() {
			InitializeComponent();
			Font=LayoutManagerForms.FontInitial;//but we will ignore this
			Lan.F(this);
		}

		#region Events
		///<summary>User wants to see a different window.  Bubbles up to FormImageFloat. Handled by its parent form ControlImages. Idx within the list of open windows, where 0 is always the docked window.</summary>
		[Browsable(false)]
		[DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		public event EventHandler<int> EventWindowClicked=null;

		///<summary>Bubbles up to FormImageFloat. Handled by its parent form ControlImages.</summary>
		[Browsable(false)]
		[DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		...
		private void FormImageFloatWindows_Load(object sender, EventArgs e){
			if(FormOwner!=null){
				...
				_screenThis=System.Windows.Forms.Screen.FromControl(FormOwner);
			}
			if(ControlOwner!=null){
				_controlImageDock=(ControlImageDock)ControlOwner;
				_screenThis=System.Windows.Forms.Screen.FromControl(ControlOwner);
			}
			...
			if(screenArray.Length==1){
				...
				if(_screen2.Bounds==_screenThis.Bounds){//probably a better way to do this
					_screen2=screenArray[1];
				}
			}
			//screen graphics----------------------------------------------------------------------------------
			_marginOuter=(int)(LayoutManager.ScaleF(10f));
			_marginInner=(int)(LayoutManager.ScaleF(5f));
			_sizeScreen=new Size((int)(LayoutManager.ScaleF(80f)),(int)(LayoutManager.ScaleF(48f)));
			int x,y,w,h;
			RectangleF rectangleF;
			using Font font=new Font(LayoutManagerForms.FontInitial.FontFamily,LayoutManager.ScaleFontODZoom(8.25f));
			y=(int)(LayoutManager.ScaleF(4f))+(int)LayoutManager.ScaleMS(font.Height)+(int)(LayoutManager.ScaleF(4f));//space for text
			//Half is at UL-------------------------------------------------------------------------------------
			x=_marginOuter;
			...
			h=(int)(LayoutManager.ScaleF(29f));
			rectangleF=new RectangleF(x+_sizeScreen.Width/5,y+LayoutManager.ScaleF(10f),w,h);
			...
			//Second Screen======================================================================================
			if(_screen2 !=null){
				y+=_sizeScreen.Height+(int)(LayoutManager.ScaleF(4f))+(int)LayoutManager.ScaleMS(font.Height)+(int)(LayoutManager.ScaleF(4f));//space for text
				//Half is at UL-------------------------------------------------------------------------------------
				x=_marginOuter;
				w=(_sizeScreen.Width-_marginInner)/2;
				h=_sizeScreen.Height;
			...
			labelActions.Font=new Font(LayoutManagerForms.FontInitial.FontFamily,LayoutManager.ScaleFontODZoom(8.25f));
			...
			listBoxWindows.LayoutManager=LayoutManager;
			listBoxWindows.Font=new Font(LayoutManagerForms.FontInitial.FontFamily,LayoutManager.ScaleFontODZoom(8.25f));
			...
			Rectangle rectangleWorking=_screenThis.WorkingArea;
		...
		private void FormImageFloatWindows_Paint(object sender, PaintEventArgs e){
			...
			using Font font=new Font(LayoutManagerForms.FontInitial.FontFamily,LayoutManager.ScaleFontODZoom(8.25f));
			if(_screen2 !=null){
				g.DrawString("This screen:",font,Brushes.Black,_marginOuter-LayoutManager.ScaleF(2f),LayoutManager.ScaleF(4f));
			}
			//Half----------------------------------------------------------------------------------
			...
			//Other screen==========================================================================
			if(_screen2 !=null){
				g.DrawString("Other screen:",font,Brushes.Black,_marginOuter-LayoutManager.ScaleF(2f),_graphicsPathQuarter_LR.GetBounds().Bottom+LayoutManager.ScaleF(6f));
