	...
	public partial class FormClaimPayTotal : FormODBase {
		...
		private void FillGrid(){
			...
			if(PrefC.GetYN(PrefName.ClaimEditShowPayTracking)) {
				gridMain.Columns.Add(new GridColumn(Lan.g("TableClaimProc","Pay Tracking"),90) { ListDisplayStrings=listDefDescripts,DropDownWidth=160 });
			}
		...
		private bool SaveGridChanges(bool skipChecks=false){
			...
			for(int i=0;i<ClaimProcsToEdit.Length;i++) {
				...
				if(PrefC.GetYN(PrefName.ClaimEditShowPayTracking)) {
					int colIndex=gridMain.Columns.GetIndex(Lan.g("TableClaimProc","Pay Tracking"));
				...
				ClaimProcsToEdit[i].Remarks=gridMain.ListGridRows[i].Cells[gridMain.Columns.GetIndex(Lan.g("TableClaimProc","Remarks"))].Text;
			}
			return true;
		}
		...
		private void SaveAllowedFees(){
			...
			for(int i=0;i<gridMain.ListGridRows.Count;i++){
				if(gridMain.ListGridRows[i].Cells[gridMain.Columns.GetIndex(Lan.g("TableClaimProc","Allowed"))].Text!=""){
			...
			for(int i=0;i<ClaimProcsToEdit.Length;i++){
				...
				if(gridMain.ListGridRows[i].Cells[gridMain.Columns.GetIndex(Lan.g("TableClaimProc","Allowed"))].Text.Trim()==""//Nothing is entered in allowed 
				...
				if(FeeCur==null) {
					...
					FeeCur.Amount=PIn.Double(gridMain.ListGridRows[i].Cells[gridMain.Columns.GetIndex(Lan.g("TableClaimProc","Allowed"))].Text);
					Fees.Insert(FeeCur);
				}
				else{
