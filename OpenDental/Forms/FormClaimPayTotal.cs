...
namespace OpenDental {
	...
	public partial class FormClaimPayTotal : FormODBase {
		...
		private void FillGrid(){
			...
			gridMain.Columns.Add(new GridColumn(Lan.g("TableClaimProc","Writeoff"),62,HorizontalAlignment.Right,_isWriteOffEditable));
			if(_doShowPatResp) {
			...
			for(int i=0;i<ClaimProcArray.Length;i++) {
				...
				if(ClaimProcArray[i].ProcNum==0) {
					...
					if(!Clinics.IsMedicalPracticeOrClinic(Clinics.ClinicNum)) {
						row.Cells.Add(procedure.ToothNum=="" ? Tooth.SurfTidyFromDbToDisplay(procedure.Surf,procedure.ToothNum) : Tooth.ToInternat(procedure.ToothNum));
					}
		...
		private void FillTotals(){
			...
			for(int i=0;i<gridMain.ListGridRows.Count;i++){
				...
				double writeOffCur=PIn.Double(gridMain.ListGridRows[i].Cells[gridMain.Columns.GetIndex(Lan.g("TableClaimProc","Writeoff"))].Text);
		...
		private void MakeAuditTrailEntries() {
			for(int i=0;i<ClaimProcArray.Length;i++) {
				string strProcCode=ProcedureCodes.GetStringProcCode(Procedures.GetOneProc(ClaimProcArray[i].ProcNum,false).CodeNum);
				ClaimProc claimProcOld=_listClaimProcsOld.FirstOrDefault(x => x.ProcNum==ClaimProcArray[i].ProcNum);
				if(claimProcOld!=null) {//Shouldn't be null, but if somehow it is, do not log changes since we do not know what it changed from.
					double procOldWriteOffAmt=claimProcOld.WriteOff;
					double procOldInsAmt=claimProcOld.InsPayAmt;
					if(ClaimProcArray[i].WriteOff!=procOldWriteOffAmt) {
						SecurityLogs.MakeLogEntry(Permissions.InsWriteOffEdit,ClaimProcArray[i].PatNum,$"Writeoff amount for procedure {strProcCode}, " +
							$"changed from {procOldWriteOffAmt.ToString("C")} to {ClaimProcArray[i].WriteOff.ToString("C")}");
					}
					if(ClaimProcArray[i].InsPayAmt==procOldInsAmt && IsCalledFromClaimEdit) {
						SecurityLogs.MakeLogEntry(Permissions.InsPayCreate,ClaimProcArray[i].PatNum,$"Insurance payment amount for procedure {strProcCode}, " +
							$"Insurance payment amount {ClaimProcArray[i].InsPayAmt.ToString("C")}");
					}
					else if(ClaimProcArray[i].InsPayAmt!=procOldInsAmt && IsCalledFromClaimEdit) {
						SecurityLogs.MakeLogEntry(Permissions.InsPayCreate,ClaimProcArray[i].PatNum,$"Insurance payment amount for procedure {strProcCode}, " +
							$"changed from {procOldInsAmt.ToString("C")} to {ClaimProcArray[i].InsPayAmt.ToString("C")}");
					}
					else if(ClaimProcArray[i].InsPayAmt!=procOldInsAmt && !IsCalledFromClaimEdit){
						SecurityLogs.MakeLogEntry(Permissions.InsPayEdit,ClaimProcArray[i].PatNum,$"Insurance payment amount for procedure {strProcCode}, " +
							$"changed from {procOldInsAmt.ToString("C")} to {ClaimProcArray[i].InsPayAmt.ToString("C")}");
					}
				}
			}
