...
namespace OpenDental {
	...
	public partial class FormRecallList:FormODBase {
		...
		private void FillComboEmail() {
			...
			List<EmailAddress> listEmails=EmailAddresses.GetEmailAddressesForComboBoxes(curUserNum);
			comboEmailFromReact.Items.AddList(listEmails,x=>EmailAddresses.GetDisplayStringForComboBox(x,curUserNum));
			comboEmailFromRecalls.Items.AddList(listEmails,x=>EmailAddresses.GetDisplayStringForComboBox(x,curUserNum));
		...
		private void FillRecalls() {
			...
			foreach(DisplayField f in fields) {
				...
				if(f.InternalName.In("Due Date","LastRemind")) {
					col.SortingStrategy=GridSortingStrategy.DateParse;
					col.TextAlign=HorizontalAlignment.Center;
				}
				else if(f.InternalName.In("Age","#Remind")) {
			...
			foreach(DataRow rowCur in _tableRecalls.Rows) {
				...
				row.Tag=new PatRowTag(
					...
					PIn.Enum<AutoCommStatus>(rowCur["webSchedSendStatus"].ToString(),defaultEnumOption:AutoCommStatus.Undefined));
		...
		private List<Appointment> SchedPatRecall(long recallNum,Patient pat,List<InsSub> subList,List<InsPlan> planList) {
			try {
		...
		public List<Appointment> SchedFamRecall(Family fam,List<InsSub> subList,List<InsPlan> planList) {
			Appointment apt;
			...
			for(int i=0;i<fam.ListPats.Length;i++) {
				...
				Recall recall=listFamRecalls.FirstOrDefault(x=>x.PatNum==fam.ListPats[i].PatNum);
				if(recall==null) {
					continue;
				}
				try{
		...
		private void SendWebSched() {
			...
			for(int i=gridRecalls.SelectedIndices.Length-1;i>=0;i--) {
				...
				if(listRestricted.Contains(rowTag.PatNum)){
					skippedRestricted++;
				...
				if(!listPatNumsInTableRecallCur.Contains(rowTag.PatNum)) {
					skippedNotInList++;
					gridRecalls.SetSelected(gridRecalls.SelectedIndices[i],false);
					continue;
				}
				//The eConnector will attempt to send a webschedrecall if WebSchedSendStatus is already SendNotAttempted
				if(rowTag.WebSchedSendStatus==AutoCommStatus.SendNotAttempted) {
			...
			List<string> listWebSchedErrors=
			...
			for(int i=0;i<addrTable.Rows.Count;i++){
				...
				if(isRecallGridSelected) {
					...
					for(int j=0;j<gridCur.ListGridRows.Count;j++) {
						if(listRecallNums.Contains(((PatRowTag)gridCur.ListGridRows[j].Tag).PriKeyNum)) {
					...
					for(int j=0;j<gridCur.ListGridRows.Count;j++) {
						if(listPatNums.Contains(((PatRowTag)gridCur.ListGridRows[j].Tag).PatNum)) {
							listRowsSent.Add((PatRowTag)gridCur.ListGridRows[j].Tag);
						}
					}
				}
...
#region printHeading
...
#endregion
			_gridCur.SetScaleAndFont(1);//Temporarily set scale to normal 96dpi for printing
			yPos=_gridCur.PrintPage(g,pagesPrinted,bounds,headingPrintH);
			_gridCur.SetScaleAndFont(LayoutManager.ScaleMy());//Change the grid back to normal zoom setting
			pagesPrinted++;
			if(yPos==-1) {
				e.HasMorePages=true;
			}
		...
		public List<Appointment> SchedPatReact(long patNum) {
			List<Appointment> listRet=new List<Appointment>();
			...
			using(FormApptEdit formAE=new FormApptEdit(0,patNum)) {
				if(formAE.ShowDialog()==DialogResult.OK) {
					listRet.Add(formAE.GetAppointmentCur());
				}
			}
			return listRet;
		}

		public List<Appointment> SchedFamReact(Family fam) {
			List<Appointment> listRet=new List<Appointment>();
		...
		private void butSched_Click(object sender,EventArgs e) {
			...
			switch(((UI.Button)sender).Tag.ToString()) {
				case "SchedPatRecall":
					listAppointmentsPinned=SchedPatRecall(_gridCur.SelectedTag<PatRowTag>().PriKeyNum,fam.GetPatient(_patNumCur),subList,planList);
					isRecall=true;
					break;
				case "SchedFamRecall":
					...
					listAppointmentsPinned=SchedFamRecall(fam,subList,planList);
					isRecall=true;
					break;
				case "SchedPatReact":
					...
					break;
				case "SchedFamReact":
					listAppointmentsPinned=SchedFamReact(fam);
					break;
			}
		...
		private class PatRowTag {
			...
			public AutoCommStatus WebSchedSendStatus;
			...
			public PatRowTag(long patNum,long priKeyNum,long statusDefNum,int numReminders,string email,ContactMethod recallMethodPreferred,long guarNum,
				...
				AutoCommStatus webSchedSendStatus=AutoCommStatus.Undefined)
			{
				PatNum=patNum;
				PriKeyNum=priKeyNum;
				...
				WebSchedSendStatus=webSchedSendStatus;
			}
		}
