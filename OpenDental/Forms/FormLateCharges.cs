...
namespace OpenDental {
	...
	public partial class FormLateCharges:FormODBase {
		...
		private List<long> CreateLateCharges(List<StatementData> listStatementDatas,DateTime dateForCharges,DateTime dateRangeEnd)	{
			...
			Dictionary<long,FamilyProdBalances> dictionaryFamilyProdBalances=listStatementDatas.SelectMany(x => x.ListGuarantorPatNums).Distinct()
			...
			for(int i=0;i<listStatementDatas.Count;i++) {
				StatementData statementData=listStatementDatas[i];
				decimal statementBalanceRemaining=CalcStatementBalanceRemaining(statementData,dictionaryFamilyProdBalances,
				...
				long provNum=GetProvNumForLateCharge(statementData.GuarantorPriProvNum);
				long adjNum=Adjustments.CreateLateChargeAdjustment((double)lateChargeAmount,dateForCharges
					,provNum,statementData.GuarantorPatNum,statementData.DateSent,
		...
		private decimal CalcStatementBalanceRemaining(StatementData statementData,Dictionary<long,FamilyProdBalances> dictionaryFamilyProdBalances
			,out List<long> listProcNumsOnLateCharge,out List<long> listAdjNumsOnLateCharge,out List<long> listPayPlanChargeNumsOnLateCharge)
		{
			...
			for(int i=0;i<statementData.ListStatementProds.Count;i++) {
				...
				decimal prodItemBalance=FindAndRemoveStatementProdBalance(statementProd,statementData.ListGuarantorPatNums,dictionaryFamilyProdBalances);
		...
		private decimal FindAndRemoveStatementProdBalance(StatementProd statementProd,List<long> listGuarantorPatNums
			,Dictionary<long,FamilyProdBalances> dictionaryFamilyProdBalances)
		{
			for(int i=0;i<listGuarantorPatNums.Count;i++) {
				long guarantorPatNumCur=listGuarantorPatNums[i];
				FamilyProdBalances familyProdBalances=dictionaryFamilyProdBalances[guarantorPatNumCur];
				if(familyProdBalances.GetAmountForStatementProdIfExists(statementProd,out decimal amount)) {
					familyProdBalances.RemoveEntryForStatementProd(statementProd);
					return amount;
				}
			}
			return 0;
