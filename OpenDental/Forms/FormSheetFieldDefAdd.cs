...
namespace OpenDental {
	public partial class FormSheetFieldDefAdd:FormODBase {
		...
		private void butGrid_Click(object sender,EventArgs e) {
			bool isDynamicSheetType=EnumTools.GetAttributeOrDefault<SheetLayoutAttribute>(SheetDefCur.SheetType).IsChartModule;
			...
			if(SheetDefs.IsDashboardType(SheetDefCur)) {
				...
				frmSheetFieldGridType.SheetDefCur=SheetDefCur;
				frmSheetFieldGridType.LayoutMode=SheetFieldLayoutMode_;
				frmSheetFieldGridType.ShowDialog();
				if(!frmSheetFieldGridType.IsDialogOK) {
					return;
				}
				formSheetFieldGrid.SheetFieldDefCur=SheetFieldDef.NewGrid(frmSheetFieldGridType.SheetGridTypeSelected,0,0,100,100); //is resized from dialog window.
