	...
	public partial class FormProdSelect:FormODBase {
		...
		private void FillGrid() {
			...
			for(int i=0;i<_listAccountEntriesAll.Count;i++) {
				...
				else if(_listAccountEntriesAll[i].GetType()==typeof(Adjustment)) {
					Adjustment adj=(Adjustment)_listAccountEntriesAll[i].Tag;
					row.Cells.Add(Lan.g(this,"Adjustment"));
					row.Cells.Add($"{Defs.GetName(DefCat.AdjTypes,adj.AdjType)}");
				}
				row.Cells.Add(_listAccountEntriesAll[i].AmountOriginal.ToString("C"));
				row.Cells.Add(_listAccountEntriesAll[i].AmountEnd.ToString("C"));
				row.Tag=_listAccountEntriesAll[i];
