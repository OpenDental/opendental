...
namespace OpenDental {
	public partial class FormEtrans835Edit:FormODBase {
		...
		private void RefreshFromDb() {
			_x835.RefreshAttachesAndClaimProcsFromDb(out _listAttaches,out _listClaimProcs);
		}
		...
		private void FillClaimDetails() {
			...
			gridClaimDetails.Columns.Add(new GridColumn(Lan.g(this,"Patient"),70,HorizontalAlignment.Left){ IsWidthDynamic=true });
			...
			gridClaimDetails.Columns.Add(new GridColumn(Lan.g(this,"Status"),70,HorizontalAlignment.Left){ IsWidthDynamic=true });//Claim Status Code Description (CLP02)
		...
		private void runAgingforClaims() {
			List<long> listPatsOnClaims = Claims.GetClaimsFromClaimNums(_listAttaches.Select(x => x.ClaimNum).ToList()).Select(x => x.PatNum).ToList();
			List<long> listGuarNums = Patients.GetGuarantorsForPatNums(listPatsOnClaims);
		...
		private void gridClaimDetails_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			if(claimPaid.IsSplitClaim) {
				...
				List<Hx835_Claim> listOtherNotDetachedSplitClaims=claimPaid.GetOtherNotDetachedSplitClaims();
				for(int i=0;i<listOtherNotDetachedSplitClaims.Count;i++) {
					Etrans835Attaches.CreateForClaim(_x835,listOtherNotDetachedSplitClaims[i],claim.ClaimNum,isAttachNeeded,_listAttaches);
				}
			}
			if(claimPaid.IsProcessed(_listClaimProcs,_listAttaches)) {
