	...
	public partial class FormEmailAddresses:FormODBase {
		...
		private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			if(IsSelectionMode) {
				...
				using FormEmailAddressEdit formEmailAddressEdit=new FormEmailAddressEdit(_listEmailAddresses[e.Row],true);
		...
		private void FillGrid() {
			...
			if(Security.IsAuthorized(Permissions.SecurityAdmin,true) && !IsSelectionMode) {
				listUserods.AddRange(Userods.GetUsers());//If authorized, get all non-hidden users.
			}
			else {
				listUserods.Add(Security.CurUser);//Otherwise, just this user.
			}
			foreach(Userod user in listUserods) {
				EmailAddress emailAddress=EmailAddresses.GetForUser(user.UserNum);
				if(emailAddress!=null) {
					_listEmailAddresses.Insert(0,emailAddress);
				}
			}
		...
		private void butAdd_Click(object sender,EventArgs e) {
			using FormEmailAddressEdit formEmailAddressEdit=new FormEmailAddressEdit(0,true);
			formEmailAddressEdit.ShowDialog();
			if(formEmailAddressEdit.DialogResult==DialogResult.OK) {
				FillGrid();
				IsChanged=true;
