			...
			if(PrefC.HasClinicsEnabled) {
				...
				if(checkAllClinics.Checked) {
					...
					if(listSelectedClinicNumLinks.Count==0 
						&& !_listProvClinicLinks.Any(x => x.ClinicNum > -1 && !listClinicNumsForUser.Contains(x.ClinicNum))) 
					{
						...
						listSelectedClinicNumLinks.AddRange(listClinicsAll.Where(x => !listClinicNumsForUser.Contains(x.ClinicNum)).Select(x => x.ClinicNum));
					}
				}
				List<ProviderClinicLink> listProvClinicLinksNew=_listProvClinicLinks.Select(x => x.Copy()).ToList();
				listProvClinicLinksNew.RemoveAll(x => listClinicNumsForUser.Contains(x.ClinicNum) || x.ClinicNum==-1);
