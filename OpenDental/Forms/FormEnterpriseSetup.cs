...
namespace OpenDental {
	public partial class FormEnterpriseSetup:FormODBase {
		...
		private YN _ynUsePhonenumTable=YN.Unknown;
		...
		private void FillStandardPrefs() {
			...
			_ynUsePhonenumTable=PrefC.GetEnum<YN>(PrefName.PatientPhoneUsePhonenumberTable);
			if(_ynUsePhonenumTable!=YN.Unknown) {
				checkUsePhoneNumTable.CheckState=CheckState.Unchecked;
				checkUsePhoneNumTable.Checked=_ynUsePhonenumTable==YN.Yes;
			}
		...
		private void FillOptionalPrefBool(CheckBox checkBox,PrefName prefName) {
		...
		private bool ValidateEntries() {
			...
			if(_ynUsePhonenumTable!=YN.Yes && checkUsePhoneNumTable.CheckState==CheckState.Checked) {//use CheckState since it can be indeterminate
		...
		private bool SyncPhoneNums() {
			...
			_ynUsePhonenumTable=YN.Yes;//so it won't sync again if you clicked the button
			return true;
		}
