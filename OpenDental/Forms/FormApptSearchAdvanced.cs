	...
	public partial class FormApptSearchAdvanced:FormODBase {
		...
		private void FillProviders(List<Provider> listProvidersForClinic,List<long> listProvNumsToSelect=null) {
			...
			for(int i=0;i<listProvidersForClinic.Count;i++) {
				if(ListTools.In(listProvidersForClinic[i].ProvNum,listProvNumsToSelect)) {
		...
		private void butProviders_Click(object sender,EventArgs e) {
			...
			formProvidersMultiPick.SelectedProviders=comboBoxMultiProv.GetListSelected<Provider>();
			...
			for(int i=0;i<formProvidersMultiPick.SelectedProviders.Count;i++) {
				listProvNums.Add(formProvidersMultiPick.SelectedProviders[i].ProvNum);
			}
