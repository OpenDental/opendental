...
namespace OpenDental{
	...
	public partial class FormJournal:FormODBase {
		...
		private void FillGrid(bool isPrinting=false,bool doRefresh=true) {
			...
			for(int i=0;i<_listJEntries.Count;i++) {
				...
				if(string.IsNullOrEmpty(textAmountTo.Text)) {//The AmountFrom textBox will be treated as an exact amount, not a range.
					if(filterAmtFrom!=0 && filterAmtFrom!=_listJEntries[i].CreditAmt && filterAmtFrom!=_listJEntries[i].DebitAmt){
						continue;
					}
				}
					...
					if(!_listJEntries[i].CreditAmt.Between(filterAmtFrom,filterAmtTo,true,true) && _listJEntries[i].DebitAmt==0 
						|| !_listJEntries[i].DebitAmt.Between(filterAmtFrom,filterAmtTo,true,true) && _listJEntries[i].CreditAmt==0) 
					{
						continue;
					}
				}
		...
		private void Print_Click(){
			...
			if(gridMainPrint.Columns.WidthAll()>800) {
