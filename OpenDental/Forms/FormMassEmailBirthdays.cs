...
namespace OpenDental {
	public partial class FormMassEmailBirthdays:FormODBase {
		...
		private string[] _stringArrayLanguages {
			get {
		...
		private bool _isClinicUsingDefaults {
			get {
				ClinicPref clinicPref=_listClinicPrefsDefaultNew.FirstOrDefault(x => x.ClinicNum==comboClinic.SelectedClinicNum);
		...
		private bool _isClinicEnabled {
			get {
				if(!PrefC.HasClinicsEnabled || comboClinic.SelectedClinicNum==0) {
					...
					ClinicPref clinicPref=_listClinicPrefsEnabledNew.FirstOrDefault(x => x.ClinicNum==comboClinic.SelectedClinicNum);
		...
		private void FormMassEmailBirthdays_Load(object sender,EventArgs e) {
			...
			#endregion
			#region Fill UI
			if(_stringArrayLanguages.Length==0 
				|| (_stringArrayLanguages.Length==1 && _stringArrayLanguages[0].ToLower().Trim()==PrefC.GetString(PrefName.LanguagesIndicateNone).ToLower().Trim())) 
			{
		...
		private void comboClinic_SelectionChangeCommitted(object sender,EventArgs e) {
			...
			if(!_isLoading) {
				...
				MapUiToRules(_listApptReminderRules.First(x => x.ClinicNum==previousClinicNum && x.Language==""));
			}
			FillForClinic();
		}
		...
		private string ValidateCurrentSelection() {
			...
			foreach(ApptReminderRule rule in _listApptReminderRules.FindAll(x => x.ClinicNum==_effectiveClinicNum)) {
				EmailHostingTemplate emailHostingTemplate=_listEmailHostingTemplates.FirstOrDefault(x => rule.EmailHostingTemplateNum==x.EmailHostingTemplateNum);
		...
		private void FillForClinic() {
			...
			foreach(ApptReminderRule rule in listApptReminderRules) {
				EmailHostingTemplate emailHostingTemplate=_listEmailHostingTemplates.FirstOrDefault(x => x.EmailHostingTemplateNum==rule.EmailHostingTemplateNum);
				...
				if(string.IsNullOrEmpty(rule.Language)) {
					...
					CultureInfo cultureInfo=MiscUtils.GetCultureFromThreeLetter(rule.Language);
					if(cultureInfo==null) {
		...
		private void butLangaugeAdd_Click(object sender,EventArgs e) {
			...
			listLanguageRulesCurrent=_listApptReminderRules.FindAll(x => x.ClinicNum==_effectiveClinicNum).Select(x => x.Language).ToList();
			foreach(string language in _stringArrayLanguages) {
				if(language.IsNullOrEmpty() || language.ToLower().Trim()==PrefC.GetString(PrefName.LanguagesIndicateNone).ToLower().Trim()) {
					continue;
				}
				if(listLanguageRulesCurrent.Contains(language))	{
		...
		private bool SwitchFromDefaultsBirthday() {
			...
			foreach(ApptReminderRule rule in _listApptReminderRules.FindAll(x => x.ClinicNum==0)) {
		...
		private void RemoveRules(params ApptReminderRule[] apptReminderRuleArray) {
			foreach(ApptReminderRule rule in apptReminderRuleArray) {
				_listApptReminderRules.Remove(rule);
			}
		}
		...
		private void MapUiToRules(ApptReminderRule apptReminderRule) {
			...
			foreach(ApptReminderRule langRule in _listApptReminderRules.FindAll(x => x.ClinicNum==apptReminderRule.ClinicNum && x.Language!="").ToList()) {
				langRule.TSPrior=apptReminderRule.TSPrior;
				langRule.IsSendForMinorsBirthday=apptReminderRule.IsSendForMinorsBirthday;
				langRule.MinorAge=apptReminderRule.MinorAge;
			}
		}
		...
		private void SaveAll() {
			...
			MapUiToRules(_listApptReminderRules.FindAll(x => x.ClinicNum==_effectiveClinicNum).First(x => x.Language==""));
			...
			if(PrefC.HasClinicsEnabled) {
				...
				foreach(Clinic clinic in comboClinic.ListClinics) {
					ApptReminderRules.SyncByClinicAndTypes(_listApptReminderRules.FindAll(x => x.ClinicNum==clinic.ClinicNum),clinic.ClinicNum,ApptReminderType.Birthday);
				}
			}
			else {
