...
namespace OpenDental {
	...
	public partial class FormPayment:FormODBase {
		...
		private Family _familyOrSuperFamily {
			get {
		...
		private List<long> _listClinicsFiltered {
			get {
		...
		private List<long> _listPatNumsFiltered {
			get {
		...
		private List<long> _listProcCodesFiltered {
			get {
		...
		private List<string> _listTypeFiltered {
			get {
		...
		private List<long> _listPatNums {
			get {
				return _familyOrSuperFamily.ListPats.Select(x => x.PatNum).ToList();
			}
		}
		#endregion
		...
		private void FormPayment_Load(object sender,System.EventArgs e) {
			...
			textPaidBy.Text=_familyOrSuperFamily.GetNameInFamFL(_payment.PatNum);
			...
			if(IsNew) {
				...
				if(transaction==null) {
					...
					foreach(JournalEntry journalEntry in JournalEntries.GetForTrans(transaction.TransactionNum)) {
		...
		private void butAddManualSplit_Click(object sender,EventArgs e) {
			...
			paySplit.IsNew=true;
			using FormPaySplitEdit formPaySplitEdit=new FormPaySplitEdit(_familyOrSuperFamily);
			formPaySplitEdit.ListSplitsCur=_listPaySplits;
      ...
      if(!ShowOverridePrompt()) {
		...
		private void butPay_Click(object sender,EventArgs e) {
			...
			List<List<AccountEntry>> listListsAccountEntriesGrid=GetAccountEntriesFromGrid((tabProcCharges.SelectedTab==tabPageCharges) ? gridCharges: gridTreatPlan);
      ...
      if(!ShowOverridePrompt()) {
		...
		private void gridSplits_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			formPaySplitEdit.ListSplitsCur=_listPaySplits;
			formPaySplitEdit.PaySplitCur=paySplit;
			if(formPaySplitEdit.ShowDialog()==DialogResult.OK) {//paySplit contains all the info we want.
      ...
      if(!ShowOverridePrompt()) {
		...
		private long GetProvNum() {
			...
			if(listProvNums.Count>1) { 
				...
				if(listProvNums.Contains(_patient.PriProv)){
					formProviderPick.SelectedProvNum=_patient.PriProv;
				}
				if(formProviderPick.ShowDialog()!=DialogResult.OK) {
					return -1;
				}
				return formProviderPick.SelectedProvNum;
			}
			else {
		...
		private List<PaySplit> GetSuggestedAutoSplits() {
			...
			PaymentEdit.ConstructResults constructResults=PaymentEdit.ConstructAndLinkChargeCredits(_listPatNums,_patient.PatNum,_listPaySplits,_payment
		...
		private void CreatPartialSplitForAccountEntries(params AccountEntry[] accountEntryArray) {
			...
			if(formAmountEdit.DialogResult==DialogResult.OK) {
				...
				foreach(AccountEntry entry in accountEntryArray) {
		...
		private MigraDoc.DocumentObjectModel.Document CreatePDFDoc(string receiptStr) {
			string[] stringArrayReceiptLines=receiptStr.Split(new string[] { "\r\n" },StringSplitOptions.None);
		...
		private void DeleteSelected(bool doCreateSecLog=true) {
			...
			foreach(PaySplit paySplit in gridSplits.SelectedTags<PaySplit>()) {
				...
				_listPaySplits.Remove(paySplit);
		...
		private bool DoShowAccountEntry(AccountEntry accountEntryCharge) {
			...
			if(!_familyOrSuperFamily.GetPatNums().Contains(accountEntryCharge.PatNum) 
			...
			if(!_listPatNumsFiltered.Contains(accountEntryCharge.PatNum)) {
				return false;
			}
			...
			if(PrefC.HasClinicsEnabled && comboGroupBy.SelectedIndex!=1 && !_listClinicsFiltered.Contains(accountEntryCharge.ClinicNum)) {
				return false;
			}
			//proc code filter
			if(_listProcCodesFiltered.Count>0
				&& (accountEntryCharge.Tag.GetType()!=typeof(Procedure) || !_listProcCodesFiltered.Contains(((Procedure)accountEntryCharge.Tag).CodeNum)))
			{
				return false;
			}
			...
			//Type Filter
			if(!_listTypeFiltered.Contains(accountEntryCharge.GetType().Name)) {
				return false;
			}
			return true;
		}
		...
		private void FillFilterPatients(bool doPreserveValues) {
			...
			List<long> listPatNumsSelected=_listPatNumsFiltered;
			...
			foreach(long patNum in _listAccountEntriesCharges.Select(x => x.PatNum).Distinct()) {
		...
		private void FillFilterClinics(bool doPreserveValues) {
			...
			List<long> listClinicNumsSelected=_listClinicsFiltered;
			...
			foreach(Clinic clinic in _listClinics.FindAll(x => listClinicNums.Contains(x.ClinicNum))) {
				...
				if(doPreserveValues && listClinicNumsSelected.Contains(clinic.ClinicNum)) {
		...
		private void FillFilterTypes(bool doPreserveValues) {
			...
			List<string> listTypesSelected=_listTypeFiltered;
		...
		private void FillGridCharges() {
			...
			if(comboGroupBy.SelectedIndex==1) {//Group by 'Provider'
				...
				var dictProvPatEntries=listAccountEntriesOutstandingCharges.Where(x => (checkShowAll.Checked || !CompareDecimal.IsZero(x.AmountEnd)))
			...
			#region Group By Clinic and Provider
				...
				gridCharges.Columns.Add(col);
				gridCharges.ListGridRows.Clear();
				var dictProvPatClinicEntries=listAccountEntriesOutstandingCharges.Where(x => (checkShowAll.Checked || !CompareDecimal.IsZero(x.AmountEnd)))
			...
			#region Group By None
				...
				gridCharges.ListGridRows.Clear();
				GridRow row;
				foreach(AccountEntry entryCharge in listAccountEntriesOutstandingCharges) {
					...
					if(entryCharge.Tag.GetType()==typeof(Procedure)) {
						procedure=(Procedure)entryCharge.Tag;
					...
					if(entryCharge.PayPlanChargeNum > 0) {
						AddIndexToDictForPaySplit(gridCharges.ListGridRows.Count,
							listAccountEntriesOutstandingCharges.FindAll(x => x.PayPlanChargeNum==entryCharge.PayPlanChargeNum),
		...
		private void FillGridSplits() {
			...
			foreach(PaySplit splitCur in _listPaySplits) {
				...
				Patient patient;
				if(!_dictPatients.TryGetValue(splitCur.PatNum,out patient)) {
					patient=Patients.GetLim(splitCur.PatNum);
				...
				if(splitCur.ProcNum!=0) {
					procedure=_loadData.ListProcsForSplits.FirstOrDefault(x => x.ProcNum==splitCur.ProcNum)??new Procedure();
				}
				...
				if(splitCur.ProcNum > 0) {//Procedure
					...
					bool isLegacyInterest=(splitCur.PayPlanDebitType==PayPlanDebitTypes.Unknown 
						&& _listAccountEntriesCharges.Where(x => x.GetType()==typeof(FauxAccountEntry) && x.PayPlanChargeNum==splitCur.PayPlanChargeNum)
				...
				if(!_family.GetPatNums().Contains(splitCur.PatNum)) {
		...
		private void FillGridTreatPlan() {
			...
			GridRow row;
			foreach(AccountEntry entryCharge in _listAccountEntriesCharges) {
		...
		private List<List<AccountEntry>> GetAccountEntriesFromGrid(GridOD grid) {
			...
			if(comboGroupBy.SelectedIndex > 0) {
				foreach(List<AccountEntry> listAccountEntries in grid.SelectedTags<List<AccountEntry>>()) {
					listListsAccountEntriesSelectedCharges.Add(listAccountEntries);
				}
			}
			else {
				foreach(AccountEntry accountEntry in grid.SelectedTags<AccountEntry>()) {
					listListsAccountEntriesSelectedCharges.Add(new List<AccountEntry>() { accountEntry });
				}
			}
			if(!hasSelectedIndices) {
		...
		private string GetCodesDescriptForEntries(int procCodeLimit,params AccountEntry[] accountEntryArray) {
			...
			foreach(AccountEntry accountEntry in accountEntryArray.Where(x => x.GetType()==typeof(Procedure))) {
		...
		private string GetXChargeTransactionTypeCommands(int tranType,bool hasXToken,bool isNotRecurring,CreditCard creditCard,string cashBack) {
			...
			switch(tranType) {
				...
				case 6:
					...
					foreach(string XCTrans in stringArrayNoteSplit) {
			...
			string aliasToken,string transactionId,decimal cashBackAmt,CreditCard creditCard)
		{
			string payNote="";
			try {
				EdgeExpress.RcmResponse rcmResponse=EdgeExpress.RCM.SendEdgeExpressRequest(_patient,_payment.ClinicNum,edgeExpressTransType,false,amt,
		...
		private bool SavePaymentToDb() {
			...
			if(_listPaySplits.Count==0) {//Existing payment with no splits.
				if(!_isCCDeclined && _rigorousAccounting!=RigorousAccounting.DontEnforce) {
					...
					&& _familyOrSuperFamily.ListPats.Length>1 //Has other family members
			...
			if(isAccountingSynchRequired && !checkPayTypeNone.Checked) {
				Payments.AlterLinkedEntries(accountingOldAmt,_payment.PayAmt,IsNew,_payment.PayNum,accountingNewAcct,_payment.PayDate,
					_familyOrSuperFamily.GetNameInFamFL(_payment.PatNum));
			}
			if(IsNew) {
		...
		private void VoidEdgeExpressTransaction(string transactionId) {
			...
			if(EdgeExpress.RCM.IsRCMRunning) {
				try {
					EdgeExpress.RcmResponse rcmResponse=EdgeExpress.RCM.VoidTransaction(_patient,_payment.ClinicNum,transactionId,false);
					Cursor=Cursors.Default;
			...
			if(!hasVoided) {
				try {
					...
					XWebResponse xWebResponseVoid=EdgeExpress.CNP.VoidTransaction(_patient.PatNum,transactionId,amount,false);
					XWebResponse xWebResponseProcessed=EdgeExpress.CNP.ProcessTransaction(xWebResponseVoid,_payment);
		...
		private void VoidPaySimpleTransaction(string refNum,string originalReceipt) {
			...
			string expDateStr="";
			string nameOnCard="";
			foreach(string receiptField in stringArrayReceiptFields) {
		...
		private void FormPayment_FormClosing(object sender,FormClosingEventArgs e) {
			...
			foreach(PaySplit split in _listPaySplits) {
				PaySplit paySplit=_listPaySplitsOld.FirstOrDefault(x => x.SplitNum==split.SplitNum);
				...
				changesMade+=SecurityLogEntryHelper(Providers.GetAbbr(paySplit.ProvNum),Providers.GetAbbr(split.ProvNum),"Provider");
				changesMade+=SecurityLogEntryHelper(Clinics.GetAbbr(paySplit.ClinicNum),Clinics.GetAbbr(split.ClinicNum),"Clinic");
				changesMade+=SecurityLogEntryHelper(paySplit.SplitAmt.ToString("F"),split.SplitAmt.ToString("F"),"Amount");
				changesMade+=SecurityLogEntryHelper(paySplit.PatNum.ToString(),split.PatNum.ToString(),"Patient number");
				if(changesMade!="") {
					SecurityLogs.MakeLogEntry(Permissions.PaymentEdit,split.PatNum,secLogText+changesMade,0,paySplit.SecDateTEdit);
				}
			}
			...
			foreach(string noteField in stringArrayNoteFields) {
