...
namespace OpenDental {
	...
	public partial class FormPayment:FormODBase {
      ...
      FormPSE.ListSplitsCur=_listSplitsCur;
		...
		private void FillGridSplits() {
			...
			foreach(PaySplit splitCur in _listSplitsCur) {
				...
				if(splitCur.ProcNum > 0) {//Procedure
					...
					if(listPayPlanChargeEntries.Any(x => !CompareDecimal.IsZero(x.Interest) && CompareDecimal.IsEqual(x.AmountAvailable,(decimal)splitCur.SplitAmt))) {
		...
		private void MakeEdgeExpressTransaction() {
			if(!HasEdgeExpress()) {
				return;
			}
			if(!ValidateForCreditCardPayment(isPrepaidCard: false,out CreditCard cc)) {
				return;
			}
			...
			bool doCreateToken=formEdgeExpressTrans.SaveToken && aliasToken.IsNullOrEmpty();
			...
			try {
				EdgeExpress.RcmResponse rcmResponse=EdgeExpress.SendEdgeExpressCloudRequest(_paymentCur.PatNum,_paymentCur.ClinicNum,transType,false,amt,
				...
				if(transType==EdgeExpressTransType.CreditReturn && CompareDouble.IsGreaterThan(approvedAmt,0)) {
					...
					HandleVoidPayment(rcmResponse.GetPayNote(),approvedAmt,receipt,CreditCardSource.EdgeExpressRCM);
					return;
				}
				...
				FriendlyException.Show(Lans.g(this,"Error processing card:")+" "+ex.Message+"\r\n\r\n"+Lan.g(this,"Please verify that RCM and pin-pad device are properly installed"
		...
		private void VoidEdgeExpressTransaction(string transactionId) {
			Cursor=Cursors.WaitCursor;
			try {
				EdgeExpress.RcmResponse response=EdgeExpress.VoidTransaction(_patCur.PatNum,_paymentCur.ClinicNum,transactionId,false);
				Cursor=Cursors.Default;
				...
				Cursor=Cursors.Default;
				FriendlyException.Show(Lans.g(this,"There was a problem voiding the transaction.  Please try again."),ex);
			}
		}
