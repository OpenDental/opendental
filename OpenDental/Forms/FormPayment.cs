...
namespace OpenDental {
	...
	public partial class FormPayment:FormODBase {
		...
		private void FormPayment_Load(object sender,System.EventArgs e) {
			...
			_isCareCredit= _paymentCur.PaymentSource.In(CreditCardSource.CareCredit);
			...
			if(_patCur.SuperFamily<=0) {
				...
				if(!IsNew && (_listSplitsCur.Any(x => listSuperFamExclusive.Select(y => y.PatNum).Contains(x.PatNum)))) {
      ...
      FormPSE.ListSplitsCur=_listSplitsCur;
		...
		private void butCareCredit_Click(object sender,MouseEventArgs e) {
			...
			using FormCareCreditWeb FormCCW=new FormCareCreditWeb(urlPurchasePage,_patCur);
			FormCCW.ShowDialog();
			if(string.IsNullOrEmpty(FormCCW.SessionId)) {
				MsgBox.Show("Error retrieving CareCredit web page.");
				DialogResult=DialogResult.OK;
				return;
			}
			CareCreditWebResponse careCreditWebResponse=CareCreditWebResponses.GetBySessionId(FormCCW.SessionId);
		...
		private void butPay_Click(object sender,EventArgs e) {
			if(checkPayTypeNone.Checked) {
				...
				if(PaymentEdit.TryCreateIncomeTransfer(constructResults.ListAccountEntries,DateTime.Now,out PaymentEdit.IncomeTransferData incomeTransferData)) {
					_listAccountCharges=constructResults.ListAccountEntries;
		...
		private void checkRecurring_Click(object sender,EventArgs e) {
			...
			if(!PrefC.GetBool(PrefName.RecurringChargesUseTransDate)) {
				_paymentCur.PayDate=formDateChoose.DatePay;
				textDate.Text=_paymentCur.PayDate.ToShortDateString();
				//Discuss: Should we alert user that we changed the PayDate.
			}
			comboCreditCards.Enabled=false;
			labelRecurringChargeWarning.Visible=true;
			_paymentCur.RecurringChargeDate=formDateChoose.DatePay;
		}
		...
		private long GetProvNum() {
			...
			if(listProvNums.Count>1) { 
				...
				return FormProvPick.SelectedProvNum;
			}
			else {
		...
		private bool DoShowAccountEntry(AccountEntry entryCharge) {
			...
			if(entryCharge.GetType().In(typeof(PaySplit),typeof(PayAsTotal))) {
				return false;
			}
			//Never show procedures or adjustments for patients outside of the family or super family.
			if(!_curFamOrSuperFam.GetPatNums().Contains(entryCharge.PatNum) 
				&& entryCharge.GetType().In(typeof(Adjustment),typeof(Procedure)))
			{
			...
			#region Group By None
				...
				foreach(AccountEntry entryCharge in listOutstandingCharges) {
					...
					if(entryCharge.Tag.GetType()==typeof(Procedure)) {
						...
						tth=proc.ToothNum=="" ? proc.Surf : Tooth.Display(proc.ToothNum);
		...
		private void FillGridSplits() {
			...
			foreach(PaySplit splitCur in _listSplitsCur) {
				...
				if(!_famCur.GetPatNums().Contains(splitCur.PatNum)) {
		...
		private void HandleVoidPayment(string payNote,double approvedAmt,string receipt,CreditCardSource ccSource) {
			if(IsNew) {
				...
				if(_printReceipt && receipt!="") {
					PrintReceipt(receipt,Lan.g(this, ccSource.In(CreditCardSource.EdgeExpressRCM,CreditCardSource.EdgeExpressCNP) 
			...
			if(!IsNew || _wasCreditCardSuccessful) {//Create a new negative payment if the void is being run from an existing payment
				...
				Payments.InsertVoidPayment(_paymentCur,_listSplitsCur,receipt,payNote,ccSource);
				if(_printReceipt && receipt!="") {
		...
		{
			Payment voidPayment=Payments.InsertVoidPayment(_paymentCur,_listSplitsCur,receipt,payNote,ccSource);
		...
		private void Init(bool doAutoSplit=false,bool doSelectAllSplits=false,bool doPayFirstAcctEntries=false,bool doPreserveValues=false) {
			...
			_listSplitsCur=initData.AutoSplitData.ListPaySplitsForPayment;
			_listAccountCharges=initData.AutoSplitData.ListAccountEntries;
			...
			string aliasToken,string transactionId,decimal cashBackAmt,CreditCard cc)
		{
			...
			try {
				...
				if(CompareDouble.IsGreaterThan(approvedAmt,0) && !CompareDouble.IsEqual(approvedAmt,amt) && !transType.In(EdgeExpressTransType.CreditVoid,EdgeExpressTransType.CreditReturn)) {
		...
		private void PayConnectVoid() {
			...
			Cursor=Cursors.WaitCursor;
			if(!PayConnectL.VoidOrRefundPayConnectPortalTransaction(_payConnectResponseWeb,_paymentCur,PayConnectService.transType.VOID,refNum,(decimal)amountCharged)) {
				Cursor=Cursors.Default;
				return;
			}
			Cursor=Cursors.Default;
		...
		private bool CheckDynamicPaymentPlanRebalance() {
			if(PayPlanEdit.AreAnyPayPlansOverpaid(_listSplitsCur)) {
				...
				bool isPayOnPrincipal=(result==DialogResult.Yes);
				List<PayPlanEdit.PayPlanRecalculationData> listRecalcData=PayPlanEdit.GetRecalculationDataForDynamicPaymentPlans(_patCur,_listSplitsCur);
				PayPlanEdit.CreateTransferForDynamicPaymentPlans(listRecalcData,isPayOnPrincipal);
			}
			return true;
		}
		...
		private void SelectPaySplitsForAccountEntries(List<AccountEntry> listAccountEntries) {
			...
			if(listPayPlanChargeNums.Count > 0) {
				List<AccountEntry> listPayPlanChargeEntries=_listAccountCharges.FindAll(x => listPayPlanChargeNums.Contains(x.PayPlanChargeNum));
		...
		private void SetComboDepositAccounts() {
			...
			if(autoPay==null) {
				...
				comboDepositAccount.Items.AddList(_arrayDepositAcctNums.Select(x => Accounts.GetDescript(x)).ToArray());
		...
		private void VoidPayConnectTransaction(string refNum,string amount) {
			...
			if(_payConnectRequest==null) {//The payment was made through the terminal.
				...
				payConnectResponse=PayConnectREST.ToPayConnectResponse(transResponse,_payConnectRequest);
			...
			if(payConnectResponse==null || payConnectResponse.StatusCode!="0") {//error in transaction
				...
				Payment voidPayment=Payments.InsertVoidPayment(_paymentCur,_listSplitsCur,receiptStr,payNote,CreditCardSource.PayConnect,payAmt:amountCharged);
		...
		private void VoidPaySimpleTransaction(string refNum,string originalReceipt) {
			...
			Cursor=Cursors.Default;
			Payment voidPayment=Payments.InsertVoidPayment(_paymentCur,_listSplitsCur,receiptStr,response.ToNoteString(),CreditCardSource.PaySimple);
			SecurityLogs.MakeLogEntry(Permissions.PaymentCreate,voidPayment.PatNum,
				Patients.GetLim(voidPayment.PatNum).GetNameLF()+", "+voidPayment.PayAmt.ToString("c"));
		}
		...
		private void XWebReturn() {
			...
			using FormXWeb FormXW=new FormXWeb(_patCur.PatNum,cc,XWebTransactionType.CreditReturnTransaction,createPayment:false,_paymentCur.PayAmt);
				...
				if(FormP.Response.StatusCode=="0") { //The transaction succeeded.
					...
					if(FormP.TranType==PayConnectService.transType.VOID) {//Close FormPayment window now so the user will not have the option to hit Cancel
						...
						if(!IsNew || _wasCreditCardSuccessful) {//Create a new negative payment if the void is being run from an existing payment
							...
							_paymentCur.IsSplit=_listSplitsCur.Count>1;
							Payments.InsertVoidPayment(_paymentCur,_listSplitsCur,FormP.ReceiptStr,resultNote,CreditCardSource.PayConnect,payAmt:amountCharged);
							string strErrorMsg=Ledgers.ComputeAgingForPaysplitsAllocatedToDiffPats(_patCur.PatNum,_listSplitsCur);
							if(!string.IsNullOrEmpty(strErrorMsg)) {
								MessageBox.Show(strErrorMsg);
							}
			...
			if(form.ApiResponseOut!=null) { //The transaction succeeded.
				...
				if(form.ApiResponseOut.TransType==PaySimple.TransType.VOID) {//Close FormPayment window now so the user will not have the option to hit Cancel
					...
					if(!IsNew || _wasCreditCardSuccessful) {//Create a new negative payment if the void is being run from an existing payment
						...
						_paymentCur.IsSplit=_listSplitsCur.Count>1;
						Payments.InsertVoidPayment(_paymentCur,_listSplitsCur,form.ApiResponseOut.TransactionReceipt,resultNote,CreditCardSource.PaySimple);
						string strErrorMsg=Ledgers.ComputeAgingForPaysplitsAllocatedToDiffPats(_patCur.PatNum,_listSplitsCur);
						if(!string.IsNullOrEmpty(strErrorMsg)) {
							MessageBox.Show(strErrorMsg);
						}
		...
		public string MakeEdgeExpressTransaction(double prepaidAmt=0) {
			...
			_promptSignature=formEdgeExpressTrans.DoPromptSignature;
			_printReceipt=formEdgeExpressTrans.DoPrintReceipt;
			...
			EdgeExpressApiType apiSelection=formEdgeExpressTrans.EdgeExpressApiTypeCur;
