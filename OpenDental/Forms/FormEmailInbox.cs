	...
	public partial class FormEmailInbox:FormODBase {
		...
		private void FillGridSent() {
			...
			for(int i=0;i<listEmailsFiltered.Count;i++) {
				...
				if(IsWebMail()) {
					if(!ListTools.In(listEmailsFiltered[i].SentOrReceived,EmailMessages.GetSentTypes(EmailPlatform.WebMail))) {
						continue;
					}
				}
				else {
					if(!ListTools.In(listEmailsFiltered[i].MsgType,listManualMessageSource.GetListSelected<EmailMessageSource>())
						&& !ListTools.In(listEmailsFiltered[i].MsgType,listAutomatedMessageSource.GetListSelected<EmailMessageSource>()))
					{
						continue;
					}
				}
		...
		private void butChangePat_Click(object sender,EventArgs e) {
			...
			for(int i=0;i<GetActiveGrid().SelectedIndices.Length;i++) {
				...
				emailMessage.PatNum=formPatientSelect.SelectedPatNum;
		...
		private void butPickPat_Click(object sender,EventArgs e) {
			...
			if(formPatientSelect.DialogResult==DialogResult.OK) {
				...
				textSearchPat.Text=GetPatientName(formPatientSelect.SelectedPatNum);
			}
		}
