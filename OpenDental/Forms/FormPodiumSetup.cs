...
namespace OpenDental {
	public partial class FormPodiumSetup:FormODBase {
		...
		private Dictionary<long,ProgramProperty> _dictionaryLocationIDs=new Dictionary<long, ProgramProperty>();
		...
		private void FormPodiumSetup_Load(object sender,EventArgs e) {
			...
			for(int i=0;i<listProgramPropertiesLocationID.Count;i++) {//If clinics is off, this will only grab the program property with a 0 clinic num (_listUserClinicNums will only have 0).
				if(_dictionaryLocationIDs.ContainsKey(listProgramPropertiesLocationID[i].ClinicNum) 
					|| !listUserClinicNums.Contains(listProgramPropertiesLocationID[i].ClinicNum)) 
				{
					continue;
				}
				_dictionaryLocationIDs.Add(listProgramPropertiesLocationID[i].ClinicNum,listProgramPropertiesLocationID[i]);
			}
		...
		private void comboClinic_SelectionChangeCommitted(object sender,EventArgs e) {
			...
			if(_dictionaryLocationIDs.ContainsKey(_clinicNum)) {
				textLocationID.Text=_dictionaryLocationIDs[_clinicNum].PropertyValue;
			}
			else {
				textLocationID.Text=_dictionaryLocationIDs[0].PropertyValue;//Default to showing the HQ value when filling info for a clinic with no program property.
			}
		}
		...
		private void SaveClinicCurProgramPropertiesToDict() {
			...
			if(_clinicNum==0) {
				...
				string locationIdOld=_dictionaryLocationIDs[_clinicNum].PropertyValue;
				foreach(KeyValuePair<long,ProgramProperty> item in _dictionaryLocationIDs) {
					ProgramProperty programProperty=item.Value;
					if(programProperty.PropertyValue==locationIdOld) {
						programProperty.PropertyValue=textLocationID.Text;
					}
				}
				return;//No other clinic specific changes could have been made, we need to return.
			}
			...
			if(_dictionaryLocationIDs.ContainsKey(_clinicNum)) {
				programPropertyLocationID=_dictionaryLocationIDs[_clinicNum];//Override the database's property with what is in memory.
			}
			...
			if(programPropertyLocationID.ClinicNum==0) {//No program property for current clinic, since _clinicNumCur!=0
				...
				if(!_dictionaryLocationIDs.ContainsKey(_clinicNum)) {//Should always happen
					_dictionaryLocationIDs.Add(_clinicNum,programPropertyLocationIDNew);
				}
				return;
			}
			...
			if(_dictionaryLocationIDs.ContainsKey(_clinicNum)) {//Should always happen
				...
				_dictionaryLocationIDs[_clinicNum]=programPropertyLocationID;
			}
			else {
				_dictionaryLocationIDs.Add(_clinicNum,programPropertyLocationID);//Should never happen.
			}
		}

		private void FillForm() {
			if(_dictionaryLocationIDs.Count==0) {
		...
		private void UpsertProgramPropertiesForClinics() {
			...
			List<ProgramProperty> listProgramPropertyLocationIDs=_dictionaryLocationIDs.Values.ToList();
			for(int i=0;i<listProgramPropertyLocationIDs.Count;i++) {
				if(listProgramPropertyLocationIDsFromDb.Exists(x => x.ProgramPropertyNum == listProgramPropertyLocationIDs[i].ProgramPropertyNum)) {
					UpdateProgramProperty(listProgramPropertyLocationIDsFromDb[listProgramPropertyLocationIDsFromDb.FindIndex(x => x.ProgramPropertyNum == listProgramPropertyLocationIDs[i].ProgramPropertyNum)],listProgramPropertyLocationIDs[i].PropertyValue);//ppCur.PropertyValue will match textLocationID.Text
					continue;
				}
				ProgramProperties.Insert(listProgramPropertyLocationIDs[i]);//Program property for that clinicnum didn't exist, so insert it into the db.
				_hasProgramPropertyChanged=true;
			}
		}
