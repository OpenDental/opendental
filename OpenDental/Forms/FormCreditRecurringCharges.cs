...
namespace OpenDental {
	public partial class FormCreditRecurringCharges:FormODBase {
		...
		private void FillGrid(bool isFromDb=false) {
			...
			gridMain.Columns.Add(new GridColumn(Lan.g("TableRecurring","Name"),PrefC.HasClinicsEnabled?190:220));
			if(PrefC.HasClinicsEnabled) {
				gridMain.Columns.Add(new GridColumn(Lan.g("TableRecurring","Clinic"),75));
			}
			gridMain.Columns.Add(new GridColumn(Lan.g("TableRecurring","Date"),PrefC.HasClinicsEnabled?80:80,HorizontalAlignment.Right));
			gridMain.Columns.Add(new GridColumn(Lan.g("TableRecurring","Family Bal"),PrefC.HasClinicsEnabled?70:85,HorizontalAlignment.Right));
			gridMain.Columns.Add(new GridColumn(Lan.g("TableRecurring","PayPlan Due"),PrefC.HasClinicsEnabled?75:85,HorizontalAlignment.Right));
			gridMain.Columns.Add(new GridColumn(Lan.g("TableRecurring","Total Due"),PrefC.HasClinicsEnabled?65:80,HorizontalAlignment.Right));
			gridMain.Columns.Add(new GridColumn(Lan.g("TableRecurring","Repeat Amt"),PrefC.HasClinicsEnabled?75:90,HorizontalAlignment.Right));//RptChrgAmt
			gridMain.Columns.Add(new GridColumn(Lan.g("TableRecurring","Charge Amt"),PrefC.HasClinicsEnabled?85:100,HorizontalAlignment.Right));
			if(Programs.HasMultipleCreditCardProgramsEnabled()) {
				if(Programs.IsEnabled(ProgramName.EdgeExpress)) {
					gridMain.Columns.Add(new GridColumn("EdgeExpress",PrefC.HasClinicsEnabled ? 90 : 80,HorizontalAlignment.Center));
				}
				if(Programs.IsEnabled(ProgramName.Xcharge)) {
			...
			foreach(RecurringChargeData chargeCur in listChargesCur) {
				...
				if(Programs.HasMultipleCreditCardProgramsEnabled()) {
					if(Programs.IsEnabled(ProgramName.EdgeExpress)) {
						if(!string.IsNullOrEmpty(chargeCur.XChargeToken) && (chargeCur.CCSource==CreditCardSource.EdgeExpressCNP || chargeCur.CCSource==CreditCardSource.EdgeExpressRCM)) {
							row.Cells.Add("X");
						}
						else {
							row.Cells.Add("");
					...
					if(Programs.IsEnabled(ProgramName.Xcharge)) {
						if(!string.IsNullOrEmpty(chargeCur.XChargeToken) && chargeCur.CCSource==CreditCardSource.XServer) {
							row.Cells.Add("X");
						}
						else {
							row.Cells.Add("");
