	...
	public partial class FormPrinterSetup : FormODBase {
		...
		private void SetControls(Control c,PrinterSettings.StringCollection installedPrinters) {
			for(int i=0;i<c.Controls.Count;i++) {
				...
				if(c.Controls[i].GetType()==typeof(CheckBox)){
					FillCheck((CheckBox)c.Controls[i]);
				}
			}
		}
		...
		private void FillCheck(CheckBox checkBox){
			...
			if(checkSimple.Checked && !PrefC.GetBool(PrefName.EasyHidePrinters)){
				...
				for(int j=0;j<panelSimple.Controls.Count;j++){
					if(panelSimple.Controls[j].GetType()!=typeof(ComboBoxOD)//skip anything but comboBoxes and CheckBoxes
						&& panelSimple.Controls[j].GetType()!=typeof(CheckBox))
					{
						continue;
					}
					...
					if(panelSimple.Controls[j].GetType()==typeof(ComboBoxOD)){
						...
						isChecked=((CheckBox)panelSimple.Controls[j]).Checked;
					}
				}
