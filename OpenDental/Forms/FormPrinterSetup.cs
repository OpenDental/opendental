	...
	public partial class FormPrinterSetup : FormODBase {
		...
		private void SetControls(Control c,PrinterSettings.StringCollection installedPrinters) {
			foreach(Control control in c.Controls){
				...
				if(control.GetType()==typeof(ComboBoxOD)){
					FillCombo((ComboBoxOD)control,installedPrinters);
				}
		...
		private void FillCombo(ComboBoxOD combo,PrinterSettings.StringCollection installedPrinters) {
		...
		private void butOK_Click(object sender, System.EventArgs e){
			...
			if(checkSimple.Checked && !PrefC.GetBool(PrefName.EasyHidePrinters)){
				...
				foreach(Control control in panelSimple.Controls){
					if(control.GetType()!=typeof(ComboBoxOD)//skip anything but comboBoxes and CheckBoxes
					...
					if(control.GetType()==typeof(ComboBoxOD)){
						if(((ComboBoxOD)control).SelectedIndex==0){
							printerName="";
						}
						else{
							printerName=((ComboBoxOD)control).SelectedItem.ToString();
						}
					}
					else{//checkBox
