	...
	public partial class FormImportXML : FormODBase {
		...
		private void butOK_Click(object sender, System.EventArgs e) {
			...
			if(patNum != 0){//a patient already exists, so only add missing fields
				...
				SecurityLogs.MakeLogEntry(EnumPermType.PatientCreate,_patient.PatNum,"Created from Import Patient XML tool.");
			...
			if(patientExisting==null){//only add or alter guarantor for new patients
				if(_guarRelat=="self"){
					...
					if(guarantorNum != 0){//a guar already exists, so simply attach. Make no other changes
						...
						SecurityLogs.MakeLogEntry(EnumPermType.PatientCreate,_patientGuarantor.PatNum,"Created from Import Patient XML tool.");
						//set guar for guar
			...
			if(_insRelat=="self"){
				...
				if(patNum != 0){//a subsc already exists, so simply attach. Make no other changes
					...
					SecurityLogs.MakeLogEntry(EnumPermType.PatientCreate,_patientSubscriber.PatNum,"Created from Import Patient XML tool.");
