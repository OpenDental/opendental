...
namespace OpenDental {
	public partial class FormMassEmailList:FormODBase {
		...
		private void FillFilters() {
			...
			checkCustomQuery.Enabled=Security.IsAuthorized(Permissions.UserQuery,true);
		}
		...
		private List<PatientInfo> RefreshPatients() {
			...
			if(listClinicNums.Count>0) {//at least one valid selected clinic 
				DataTable tablePatients=GetPatientDataTable(checkCustomQuery.Checked, listClinicNums);
		...
		private void checkCustomQuery_Click(object sender,EventArgs e) {
			//TODO - Might want to check user preferences or permissions
			groupBoxCustomQuery.Visible=checkCustomQuery.Checked;
			groupBoxFilters.Visible=!checkCustomQuery.Checked;
		}
