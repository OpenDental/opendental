	...
	public partial class FormVitalsignEdit2014:FormODBase {
		...
		private void FormVitalsignEdit2014_Load(object sender,EventArgs e) {
			...
			for(int i=0;i<_listLoincHeightCodes.Count;i++) {
				if(_listLoincHeightCodes[i].NameShort=="" || _listLoincHeightCodes[i].NameLongCommon.Length<30) {//30 is roughly the number of characters that will fit in the combo box
			...
			for(int i=0;i<_listLoincWeightCodes.Count;i++) {
				if(_listLoincWeightCodes[i].NameShort=="" || _listLoincWeightCodes[i].NameLongCommon.Length<30) {//30 is roughly the number of characters that will fit in the combo box
