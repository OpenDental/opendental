...
namespace OpenDental {
	public partial class FormCommItem : FormODBase {
		...
		private bool SaveCommItem(bool showMsg) {
			...
			if(_isPersistent && string.IsNullOrWhiteSpace(_commlog.Note)) { //in persistent mode, we don't want to save empty commlogs
				return false;
			}
			if(_commlogOld.IsNew || _isPersistent) {
				...
				textCommlogNum.Text=_commlog.CommlogNum.ToString();
				SecurityLogs.MakeLogEntry(Permissions.CommlogEdit,_commlog.PatNum,"Insert");
		...
		private void AutoSaveCommItem() {
			if(_commlogOld.IsNew) {
				...
				SecurityLogs.MakeLogEntry(Permissions.CommlogEdit,_commlog.PatNum,"Autosave Insert");
				_commlog.IsNew=false;
			}
			else {
				//Update
