...
namespace OpenDental {
	public partial class FormWiki:FormODBase {
		...
		private void Setup_Click() {
			using FormWikiSetup formWikiSetup=new FormWikiSetup();
			formWikiSetup.ShowDialog();
			if(formWikiSetup.DialogResult!=DialogResult.OK) {
				return;
			}
		...
		private void Rename_Click() {
			...
			using FormWikiRename formWikiRename=new FormWikiRename();
			...
			_listHistoryNav[_listHistoryNav.Count-(1+_idxHistoryNavBack)]="wiki:"+formWikiRename.PageTitle;//keep history updated, do not decrement historyNavBack, stay at the same index in history
