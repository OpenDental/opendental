...
namespace OpenDental {
	...
	public partial class FormMedical : FormODBase {
		...
		private void pd_PrintPage(object sender,System.Drawing.Printing.PrintPageEventArgs e) {
			...
			#endregion
			yPos=gridMedsPrint.PrintPage(g,_pagesPrinted,rectangleBounds,_headingPrintH);
			_pagesPrinted++;
			if(yPos==-1) {
				e.HasMorePages=true;
			}
		...
		private void comboTobaccoStatus_SelectionChangeCommitted(object sender,EventArgs e) {
			...
			if(!_listEhrCodesTobaccoStatuses.Any(x => x.CodeValue==formSnomeds.SelectedSnomed.SnomedCode)) {
				EhrCode ehrCode=new EhrCode();
				ehrCode.CodeValue=formSnomeds.SelectedSnomed.SnomedCode;
				ehrCode.Description=formSnomeds.SelectedSnomed.Description;
				_listEhrCodesCustomTobacco.Add(ehrCode);
				_listEhrCodesCustomTobacco=_listEhrCodesCustomTobacco.OrderBy(x => x.Description).ToList();
			...
			comboTobaccoStatus.SelectedIndex=_listEhrCodesTobaccoStatuses.FindIndex(x => x.CodeValue==formSnomeds.SelectedSnomed.SnomedCode);//add 1 for ...choose from
		}
