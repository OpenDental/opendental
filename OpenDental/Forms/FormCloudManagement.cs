...
namespace OpenDental {
	public partial class FormCloudManagement:FormODBase {
		...
		private List<string> _listIpAddresses;
		...
		private void FormCloudManagement_Load(object sender,EventArgs e) {
			...
			_listActiveInstances=ActiveInstances.GetAllResponsiveActiveInstances()
				...
				.ToList();
			_listIpAddresses=PrefC.GetString(PrefName.CloudAllowedIpAddresses).Split(",", StringSplitOptions.RemoveEmptyEntries).ToList<string>();
			try {
		...
		private void FillGridAddresses() {
			...
			gridAllowedAddresses.ListGridColumns.Add(new GridColumn(Lan.g(this,"IP Address"), 100));
			//Rows
			gridAllowedAddresses.ListGridRows.Clear();
			GridRow row;
			foreach(string address in _listIpAddresses) {
				row=new GridRow();
		...
		private void butDeleteAddress_Click(object sender,EventArgs e) {
			...
			List<string> listAddressesToRemove=gridAllowedAddresses.SelectedTags<string>();
			foreach(string address in listAddressesToRemove) {
				_listIpAddresses.Remove(address);
			}
			FillGridAddresses();
		}
		...
		private void butAddCurrent_Click(object sender,EventArgs e) {
			...
			if(_listIpAddresses.Contains(address)) {
				MsgBox.Show($"Address {address} is already allowed.");
			}
			else {
				_listIpAddresses.Add(address);
				FillGridAddresses();
			}
		}
		...
		private void butAllowAddress_Click(object sender,EventArgs e) {
			...
			if(!IPAddress.TryParse(address,out IPAddress addr)) {
				MsgBox.Show($"Address {address} is invalid.");
				return;
			}
		...
		private void butOK_Click(object sender,EventArgs e) {
			Prefs.UpdateString(PrefName.CloudAllowedIpAddresses, string.Join(",",_listIpAddresses));
