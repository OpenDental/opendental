...
namespace OpenDental {
	...
	public partial class FormEServicesMassEmail:FormODBase {
		...
		//==================== Promotion Variables ====================
		private Clinic _clinicPromoCur {	
			get {
		...
		private void FillPromotionForClinic() {			
			long clinicNum=_clinicPromoCur.ClinicNum;
			bool hasCredentials;
			if(clinicNum==0) {
		...
		private void ChangeEnabled(PrefName prefName,bool isEnabling) {
			...
			long clinicNum=_clinicPromoCur.ClinicNum;
			HostedEmailStatus status=HostedEmailStatus.NotActivated;
			if(clinicNum==0) {
				status=PrefC.GetEnum<HostedEmailStatus>(prefName);
				status=isEnabling ? EnumTools.AddFlag(status,HostedEmailStatus.Enabled) : EnumTools.RemoveFlag(status,HostedEmailStatus.Enabled);
				Prefs.UpdateInt(prefName,(int)status);
				Prefs.RefreshCache();
				_doSetInvalidPrefs=true;
			}
			else {
				status=PIn.Enum<HostedEmailStatus>(ClinicPrefs.GetInt(prefName,clinicNum));
				status=isEnabling ? EnumTools.AddFlag(status,HostedEmailStatus.Enabled) : EnumTools.RemoveFlag(status,HostedEmailStatus.Enabled);
				ClinicPrefs.Upsert(prefName,clinicNum,((int)status).ToString());
				ClinicPrefs.RefreshCache();
				_doSetInvalidClinicPrefs=true;
			}
		}
		...
		private void button1_Click(object sender,EventArgs e) {
			...
			if(clinicNum==0) {
				...
				status=status ^ HostedEmailStatus.SignedUp;
				...
				status=status ^ HostedEmailStatus.SignedUp;
				ClinicPrefs.Upsert(PrefName.EmailSecureStatus,clinicNum,$"{(long)status}");
				ClinicPrefs.RefreshCache();
			}
			FillPromotionForClinic();
		}
