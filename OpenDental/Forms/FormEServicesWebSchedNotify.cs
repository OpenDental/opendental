...
namespace OpenDental {
	public partial class FormEServicesWebSchedNotify:FormODBase {
		...
		private List<PrefName> _listWebSchedVerifyAsapPrefNames=new List<PrefName>() {
		...
		private List<PrefName> _listWebSchedVerifyExistingPatPrefNames=new List<PrefName> () {
		...
		private List<PrefName> _listWebSchedVerifyNewPatPrefNames=new List<PrefName>() {
		...
		private List<PrefName> _listWebSchedVerifyRecallPrefNames=new List<PrefName>() {
		...
		private void FormEServicesWebSchedNotify_Load(object sender,EventArgs e) {
			...
			switch(_webSchedType) {
				case WebSchedNotifyType.ASAP:
					...
					_listWebSchedVerifyPrefNames=_listWebSchedVerifyAsapPrefNames;
					groupBox.Text="ASAP";
					this.Text+=" - ASAP";
					break;
				case WebSchedNotifyType.ExistingPat:
					...
					_listWebSchedVerifyPrefNames=_listWebSchedVerifyExistingPatPrefNames;
				...
				case WebSchedNotifyType.NewPat:
					...
					_listWebSchedVerifyPrefNames=_listWebSchedVerifyNewPatPrefNames;
				...
				case WebSchedNotifyType.Recall:
					...
					_listWebSchedVerifyPrefNames=_listWebSchedVerifyRecallPrefNames;
					groupBox.Text="Recall";
					this.Text+=" - Recall";
					break;
			}
			for(int i=0;i<_listWebSchedVerifyPrefNames.Count;i++) {
				_listWebSchedVerifyClinicPrefs.AddRange(ClinicPrefs.GetPrefAllClinics(_listWebSchedVerifyPrefNames[i]));
				Pref pref=Prefs.GetPref(_listWebSchedVerifyPrefNames[i].ToString());
				_listWebSchedVerifyClinicPrefs.Add(new ClinicPref() { ClinicNum=CLINIC_NUM_DEFAULT,PrefName=_listWebSchedVerifyPrefNames[i],ValueString=pref.ValueString });
			}
		...
		private void butEditEmail_Click(object sender,EventArgs e) {
			...
			UpdateClinicPref(_prefNameEmailType,(formEmailEdit.IsRaw ? (int)EmailType.RawHtml : (int)EmailType.Html).ToString());
		...
		private void butRestore_Click(object sender,EventArgs e) {
			...
			if(isAccepted) {
				for(int i=0;i<_listWebSchedVerifyPrefNames.Count;i++) {
					TryRestoreClinicPrefOld(_listWebSchedVerifyPrefNames[i]);
				}
				FillTemplate();
			}
		}
		...
		private void SetRadioButtonVal(PrefName prefName) {
			WebSchedVerifyType type=(WebSchedVerifyType)PIn.Int(GetTemplateVal(prefName));
			RadioButton buttonMatch=groupBoxRadio.Controls.OfType<RadioButton>().FirstOrDefault(x => (WebSchedVerifyType)x.Tag==type);
		...
		private void TryRestoreClinicPrefOld(PrefName prefName) {
			...
			UpdateClinicPref(prefName,pref.ValueString);
		}
		...
		private void SaveWebSchedVerify() {
			...
			for(int i = 0;i<_listWebSchedVerifyPrefNames.Count;i++) {
				for(int j = 0;j<listClinics.Count;j++) {
					ClinicPref newClinicPref=_listWebSchedVerifyClinicPrefs.FirstOrDefault(x => x.PrefName==_listWebSchedVerifyPrefNames[i] && x.ClinicNum==listClinics[j]);
					ClinicPref oldClinicPref=_listWebSchedVerifyClinicPrefs_Old.FirstOrDefault(x => x.PrefName==_listWebSchedVerifyPrefNames[i] && x.ClinicNum==listClinics[j]);
				...
				ClinicPref newPref=_listWebSchedVerifyClinicPrefs.FirstOrDefault(x => x.PrefName==_listWebSchedVerifyPrefNames[i] && x.ClinicNum==CLINIC_NUM_DEFAULT);
				if(newPref!=null) {
					Prefs.UpdateString(_listWebSchedVerifyPrefNames[i],newPref.ValueString);
				}
			}
		}
