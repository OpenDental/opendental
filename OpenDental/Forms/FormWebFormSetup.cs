...
namespace OpenDental {
	...
	public partial class FormWebFormSetup:FormODBase {
		...
		public FormWebFormSetup(long clinicNum,string urlWebForm,bool isWebSchedSetup=true):this() {
			...
			for(int i = 0;i<Controls.Count;i++) {
				if(Controls[i]==gridMain || Controls[i]==groupConstructURL || Controls[i]==butOk || Controls[i]==butCancel || Controls[i]==checkEnableAutoDownload) {
					continue;
				}
	...
	private void FormWebFormSetup_Load(object sender,EventArgs e) {
			if(PrefC.HasClinicsEnabled) {
				if(_isWebSchedSetup){
					comboClinic.SelectedClinicNum=_clinicNumDefault;
				}
				else{
		...
		private void butAdd_Click(object sender,EventArgs e) {
			using FormSheetPicker formSheetPicker=new FormSheetPicker(isWebForm:true);
			...
			List<SheetDef> listSheetDefs=formSheetPicker.ListSheetDefsSelected;
		...
		private void butUpdate_Click(object sender,EventArgs e) {
			...
			if(sheetDef==null) {//This web form has never had a SheetDefNum assigned or the sheet has been deleted.
				...
				formSheetPicker.HideKioskButton=true;
				formSheetPicker.ShowDialog();
				if(formSheetPicker.DialogResult != DialogResult.OK || formSheetPicker.ListSheetDefsSelected.Count==0) {
					return;
				}
				sheetDef=formSheetPicker.ListSheetDefsSelected.FirstOrDefault();
			}
