	...
	public partial class FormDefEdit : FormODBase {
		...
		private void FormDefEdit_Load(object sender, System.EventArgs e) {
			if(DefCur.Category==DefCat.ApptConfirmed) {
				...
				_listExcludeArrivalSendNums.Remove(0);
				checkIncludeSend.Checked=!_listExcludeSendNums.Contains(DefCur.DefNum);
				checkIncludeConfirm.Checked=!_listExcludeConfirmNums.Contains(DefCur.DefNum);
				checkIncludeRemind.Checked=!_listExcludeRemindNums.Contains(DefCur.DefNum);
				checkIncludeThanks.Checked=!_listExcludeThanksNums.Contains(DefCur.DefNum);
				checkIncludeArrivalSend.Checked=!_listExcludeArrivalSendNums.Contains(DefCur.DefNum);
				checkIncludeArrivalResponse.Checked=!_listExcludeArrivalResponseNums.Contains(DefCur.DefNum);
				checkIncludeEClipboard.Checked=!_listExcludeEClipboardNums.Contains(DefCur.DefNum);
				checkByod.Checked=_listByodEnabled.Contains(DefCur.DefNum);
			}
			else {
			...
			if(ListTools.In(DefCur.DefNum,PrefC.GetLong(PrefName.AppointmentTimeArrivedTrigger),PrefC.GetLong(PrefName.AppointmentTimeDismissedTrigger),
			...
			if(ListTools.In(DefCur.DefNum,PrefC.GetLong(PrefName.AppointmentTimeArrivedTrigger))) {
		...
		private void butSelect_Click(object sender,EventArgs e) {
			...
			using FormDefinitionPicker FormDP=new FormDefinitionPicker(DefCur.Category,_defsList.ToList().FindAll(x => x.DefNum==defNumParent),DefCur.DefNum);
		...
		private void butOK_Click(object sender, System.EventArgs e) {
			...
			switch(DefCur.Category){
				...
				case DefCat.PaySplitUnearnedType:
					...
					Pref tempPref=Prefs.GetPref(PrefName.TpUnearnedType.GetDescription());
					if(DefCur.DefNum.ToString()==tempPref.ValueString && checkHidden.Checked) {//If the current selected Def is the default, and we're setting to hidden, set the new default to the first 'shown' Def
						List<Def> listDefsForUnearnedType=_defsList.FindAll(x => x.Category==DefCat.PaySplitUnearnedType);
						List<Def> listDefsNonHiddenForUnearnedType=listDefsForUnearnedType.FindAll(x => x.IsHidden==false && x.DefNum!=DefCur.DefNum && x.ItemValue.ToLower().Contains("x"));
						if(listDefsNonHiddenForUnearnedType.Count==0) {
							listDefsNonHiddenForUnearnedType=listDefsForUnearnedType.FindAll(x => x.IsHidden==false && x.DefNum!=DefCur.DefNum);//Get remaining nonhidden defs.
						}
						tempPref.ValueString=listDefsNonHiddenForUnearnedType[0].DefNum.ToString();//Set the default TpUnearnedType to the first remaining nonhidden def. Must have atleast one non-hidden. 
						Prefs.Update(tempPref);//update the DB entry for the TpUnearnedType
						Prefs.UpdateValueForKey(tempPref);//Load the DB entry for the TpUnearnedType into cache
					}
					break;
			...
			if(DefCur.Category==DefCat.ApptConfirmed) {
				//==================== EXCLUDE SEND ====================
				UpdateAutoCommExcludes(checkIncludeSend,_listExcludeSendNums,DefCur,PrefName.ApptConfirmExcludeESend);
				//==================== EXCLUDE CONFIRM ====================
				UpdateAutoCommExcludes(checkIncludeConfirm,_listExcludeConfirmNums,DefCur,PrefName.ApptConfirmExcludeEConfirm);
				//==================== EXCLUDE REMIND ====================
				UpdateAutoCommExcludes(checkIncludeRemind,_listExcludeRemindNums,DefCur,PrefName.ApptConfirmExcludeERemind);
				//==================== EXCLUDE THANKYOU ====================
				UpdateAutoCommExcludes(checkIncludeThanks,_listExcludeThanksNums,DefCur,PrefName.ApptConfirmExcludeEThankYou);
				//==================== EXCLUDE ARRIVAL SEND ====================
				UpdateAutoCommExcludes(checkIncludeArrivalSend,_listExcludeArrivalSendNums,DefCur,PrefName.ApptConfirmExcludeArrivalSend);
				//==================== EXCLUDE ARRIVAL RESPONSE ====================
				UpdateAutoCommExcludes(checkIncludeArrivalResponse,_listExcludeArrivalResponseNums,DefCur,PrefName.ApptConfirmExcludeArrivalResponse);
				//==================== EXCLUDE ECLIPBOARD CHECKIN ======================
				UpdateAutoCommExcludes(checkIncludeEClipboard,_listExcludeEClipboardNums,DefCur,PrefName.ApptConfirmExcludeEclipboard);
				//==================== ENABLED BYOD FOR DEF ====================
				UpdateByod(checkByod,_listByodEnabled,DefCur,PrefName.ApptConfirmByodEnabled);
				DataValid.SetInvalid(InvalidType.Prefs);
			}
			if(DefCur.Category==DefCat.MiscColors){
				if(DefCur.ItemOrder==(int)DefCatMiscColors.MainBorder){
					SetBorderColor(DefCatMiscColors.MainBorder,DefCur.ItemColor);
				}
				if(DefCur.ItemOrder==(int)DefCatMiscColors.MainBorderOutline){
					SetBorderColor(DefCatMiscColors.MainBorderOutline,DefCur.ItemColor);
				}
				if(DefCur.ItemOrder==(int)DefCatMiscColors.MainBorderText){
					SetBorderColor(DefCatMiscColors.MainBorderText,DefCur.ItemColor);
				}
			}
			DialogResult=DialogResult.OK;
		}

		private static void UpdateAutoCommExcludes(CheckBox check,List<long> listExcludeNums,Def def,PrefName prefName) {
			if(check.Checked) {
