...
namespace OpenDental{
	public partial class FormModuleSetup:FormODBase {
		...
		private long _defNumDefaultImageCategory;
		...
		private bool SaveAccount(){
			...
			_changed|=Prefs.UpdateBool(PrefName.ClaimPrimaryRecievedForceSecondaryStatus,checkClaimPrimaryRecievedForceSecondaryStatus.Checked);
		...
		private void butBrowseImageCategory_Click(object sender,EventArgs e) {
			List<Def> listDefDefaultImageCategories=Defs.GetDefs(DefCat.ImageCats,new List<long>(){ _defNumDefaultImageCategory });
			using FormDefinitionPicker formDefinitionPicker=new FormDefinitionPicker(DefCat.ImageCats,listDefDefaultImageCategories);
			...
			if(formDefinitionPicker.ListDefsSelected.Count==0) {
				_defNumDefaultImageCategory=0;
				textDefaultImageCategory.Text="";
				return;
			}
