...
///<summary></summary>
    ...
    public FormRpPrintPreview pView = new FormRpPrintPreview();
		...
		private void FillComboProvNum() { 
			...
			if(PrefC.GetBool(PrefName.RxHideProvsWithoutDEA)) {
				...
				if(defaultClinicProvNum != 0 && listProviders.FindIndex(x => x.ProvNum == defaultClinicProvNum) == -1) {
					Provider providerDefaultClinic=Providers.GetProv(defaultClinicProvNum);
					if(!providerDefaultClinic.IsNotPerson) {
						listProviders.Add(providerDefaultClinic);
					}
				}
				//make sure list has the user's assigned provider if any
				if(userProvNum != 0 && listProviders.FindIndex(x => x.ProvNum == userProvNum) == -1) {
					Provider providerUser=Providers.GetProv(userProvNum);
					if(!providerUser.IsNotPerson) {
						listProviders.Add(providerUser);
					}
				}
				if(RxPatCur.ProvNum != 0 && listProviders.FindIndex(x=> x.ProvNum==RxPatCur.ProvNum) == -1) {//because List<T>.Contains can't manage to find a provider that is in the list
					Provider providerCur = Providers.GetProv(RxPatCur.ProvNum);
					if(!providerCur.IsNotPerson) {
						listProviders.Add(providerCur);
					}
				}
			}
		...
		private void butPick_Click(object sender,EventArgs e) {
			...
			FormP.PharmacyNumSelected=RxPatCur.PharmacyNum;
			FormP.ShowDialog();
			if(FormP.DialogResult!=DialogResult.OK){
				return;
			}
			RxPatCur.PharmacyNum=FormP.PharmacyNumSelected;
			textPharmacy.Text=Pharmacies.GetDescription(RxPatCur.PharmacyNum);
		}
		...
		private bool SaveRx(){
			...
			if(IsNew){
				...
				SecurityLogs.MakeLogEntry(Permissions.RxCreate,RxPatCur.PatNum,"CREATED("+RxPatCur.RxDate.ToShortDateString()+","+RxPatCur.Drug+",ProvNum"
					+RxPatCur.ProvNum+",Disp:"+RxPatCur.Disp+",Refills:"+RxPatCur.Refills+")",RxPatCur.RxNum,DateTime.MinValue);//No date previous needed, new Rx Pat
		...
		private void butPrint_Click(object sender, System.EventArgs e) {
			...
			SecurityLogs.MakeLogEntry(Permissions.RxEdit,RxPatCur.PatNum,"Printed as: "+RxPatCur.RxDate.ToShortDateString()+","+RxPatCur.Drug+",ProvNum:"+RxPatCur.ProvNum+",Disp:"+RxPatCur.Disp+",Refills:"+RxPatCur.Refills+")",RxPatCur.RxNum, RxPatCur.DateTStamp);
			if(RxPatCur.IsNew) {
