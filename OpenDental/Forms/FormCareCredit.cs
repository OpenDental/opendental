...
namespace OpenDental {
	public partial class FormCareCredit:FormODBase {
		...
		private void butApply_Click(object sender,EventArgs e) {
			...
			List<PatField> listPatFields=PatFields.GetPatientData(_patient.PatNum);
			...
			if(careCreditStatus.ToLower().In(CareCreditWebStatus.PreApproved.GetDescription().ToLower())) {
				CareCreditL.LaunchQuickScreenIndividualPage(_patient,comboProviders.GetSelectedProvNum(),comboClinics.ClinicNumSelected,1);
			}
			else {
				CareCreditL.LaunchCreditApplicationPage(_patient,comboProviders.GetSelectedProvNum(),comboClinics.ClinicNumSelected,0,1);
			}
		...
		private void butLookup_Click(object sender,EventArgs e) {
			...
			CareCreditL.LaunchLookupPage(_patient,comboProviders.GetSelectedProvNum(),comboClinics.ClinicNumSelected);
			DialogResult=DialogResult.None;//Don't close the form yet.
		}
