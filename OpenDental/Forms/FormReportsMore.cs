...
namespace OpenDental {
	...
	public partial class FormReportsMore:FormODBase {
		...
		private void FillLists() {
			...
			_listReportPermissions=GroupPermissions.GetPermsForReports(Security.CurUser);
			...
			foreach(DisplayReport report in _listProdInc) {
				if(GroupPermissions.HasPermission(Security.CurUser,Permissions.Reports,report.DisplayReportNum,listGroupPermissions:_listReportPermissions)) {
		...
		private void butUserQuery_Click(object sender,EventArgs e) {
			...
			if(Security.IsAuthorized(Permissions.UserQueryAdmin,true)) {
				using FormUserQuery FormQ=new FormUserQuery(null);
				FormQ.ShowDialog();
		...
		private void listProdInc_MouseDown(object sender,MouseEventArgs e) {
			...
			if(!GroupPermissions.HasPermission(Security.CurUser,Permissions.Reports,_listProdInc[selected].DisplayReportNum,listGroupPermissions:_listReportPermissions)) {
