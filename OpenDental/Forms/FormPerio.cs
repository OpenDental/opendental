...
namespace OpenDental{
	...
	public partial class FormPerio : FormODBase {
		...
		private void ResumePath() {
			...
			List<PerioAdvancePos> listAutoAdvancePositions=PerioAdvancePos.GetDefaultPath();
			if(radioFacialsFirst.Checked) {
				listAutoAdvancePositions=PerioAdvancePos.GetFacialFirstPath();
			}
			ColRow colRowNextAvailable;
			for(int i=0;i<listAutoAdvancePositions.Count;i++) {
				if(listAutoAdvancePositions[i]==null || listSkippedTeeth.Contains(listAutoAdvancePositions[i].ToothNum)) {
					continue;
				}
				if(radioFacialsFirst.Checked) {//Auto advance Facials First.
					colRowNextAvailable=GetNextAvailableColRowForTooth(listAutoAdvancePositions[i],ContrPerio.GetNext(listAutoAdvancePositions[i],listAutoAdvancePositions),
						ContrPerio.GetPrevious(listAutoAdvancePositions[i],listAutoAdvancePositions));
					if(colRowNextAvailable.Col==-1 || colRowNextAvailable.Row==-1) {
						continue;
					}
				}
				else {//Auto advance NOT Facials First.
					colRowNextAvailable=TryGetNextAvailableColRowForTooth(listAutoAdvancePositions[i].ToothNum,listAutoAdvancePositions[i].IsFacial);
					if(colRowNextAvailable.Col==-1 || colRowNextAvailable.Row==-1) {
						continue;
					}
					if(listAutoAdvancePositions[i].IsFacial) {
		...
		private ColRow GetNextAvailableColRowForTooth(PerioAdvancePos autoAdvancePosition,PerioAdvancePos autoAdvancePositionNext,PerioAdvancePos autoAdvancePositionPrev) {
			ColRow colRowNextAvailable;
			if(autoAdvancePosition==null) {//This shouldn't happen.
				return new ColRow(-1,-1);
			}
			//Get the default colrow for the current tooth.
			ColRow colRowForTooth=GetColRowForTooth(autoAdvancePosition.ToothNum,autoAdvancePosition.IsFacial);
			...
			colRowForTooth=contrPerio.GetAutoAdvanceColRow(autoAdvancePosition.ToothNum,autoAdvancePosition.IsFacial,perioSequenceType,isReverse:false);
		...
		private void RefreshListExams(bool skipRefreshMeasures=false) {
			...
			for(int i=0;i<PerioExams.ListExams.Count;i++) {
				...
				gridRow.Cells.Add(PerioExams.ListExams[i].ExamDate.ToShortDateString());
				gridRow.Cells.Add(Providers.GetAbbr(PerioExams.ListExams[i].ProvNum));
				...
				if(PerioExams.ListExams[i].Note.Length > noteEnd || index5thNewLine > -1) {
					...
					gridRow.Cells.Add(PerioExams.ListExams[i].Note.Substring(0,noteEnd)+"(...)");
				} 
				else {
					gridRow.Cells.Add(PerioExams.ListExams[i].Note);
				}
		...
		private void FillGrid(bool selectCell=true) {
			if(gridODExam.GetSelectedIndex()!=-1){
				...
				if(!Security.IsAuthorized(EnumPermType.PerioEdit,PerioExams.ListExams[gridODExam.GetSelectedIndex()].ExamDate,suppressMessage:true)) {
		...
		private void gridODExam_CellDoubleClick(object sender,UI.ODGridClickEventArgs e) {
			...
			if(!Security.IsAuthorized(EnumPermType.PerioEdit,PerioExams.ListExams[gridODExam.GetSelectedIndex()].ExamDate)) {
				return;
			}
