...
namespace OpenDental{
	...
	public partial class FormPerio : FormODBase {
		...
		private void FormPerio_Load(object sender, System.EventArgs e) {
			...
			List<ToothInitial> initialList=ToothInitials.GetPatientData(PatCur.PatNum);
		...
		public override void ProcessSignalODs(List<Signalod> listSignals) {
			...
			if(PerioExamCur!=null && listSignals.Any(x=>x.FKeyType==KeyType.PatNum && x.FKey==PerioExamCur.PatNum && x.IType==InvalidType.PerioExams)) {
		...
		private void CheckMobileActivity() {
			...
			bool isExamInUseCur=MobileAppDevices.IsInUse(PatCur.PatNum,MADPage.PerioExamEditPage,MADPage.PerioExamListPage,MADPage.PerioExamOverviewPage,MADPage.FileViewerPage,MADPage.Undefined);
			bool hasChanged=!(_isExamInUse && isExamInUseCur); //This checks if the state has changed, we don't want to keep refreshing UI if it hasn't.
			if(!hasChanged) {
				return;
			}
			_isExamInUse=isExamInUseCur;
		...
		private void SetEClipBoardEditing(bool isEditingOnEClipBoard) {
			gridP.perioEdit=!isEditingOnEClipBoard;
		...
		private void butDefault_Click(object sender,EventArgs e) {
			...
			FillGrid();
			SecurityLogs.MakeLogEntry(Permissions.PerioEdit,PatCur.PatNum,"Default perio exam created");
		}
		...
		private void SetDefault(string rawPerioMeasures) {
			...
			for(int i=0;i<16;i++) {
				...
				perioMeasureUpper.PerioExamNum=PerioExamCur.PerioExamNum;
			...
			for(int i=0;i<16;i++) {
				...
				perioMeasureLower.PerioExamNum=PerioExamCur.PerioExamNum;
...
//		}
		...
		private void butAllExamsEclip_Click(object sender,EventArgs e) {
			...
			MobileDataByte funcInsertDataForUnlockCode(string unlockCode) {
				if(MobileDataBytes.TryInsertPatientPerio(PatCur,unlockCode,0,out string errorMsg,out MobileDataByte mobileDataByte)) {
		...
		private void butEClipboard_Click(object sender,EventArgs e) {
			if(PerioExamCur==null || gridODExam.GetSelectedIndex()==-1) {
				MsgBox.Show("No Perio exam selected, please select an exam first.");
				return;
			}
			gridP.SaveCurExam(PerioExamCur);
			MobileDataByte funcInsertDataForUnlockCode(string unlockCode) {
				if(MobileDataBytes.TryInsertPatientPerio(PatCur,unlockCode,PerioExamCur.PerioExamNum,out string errorMsg,out MobileDataByte mobileDataByte)) {
					return mobileDataByte;
				}
				MsgBox.Show(errorMsg);
				return null;
		...
		private void butUnlockEClip_Click(object sender,EventArgs e) {
			bool _isExamInUse=MobileAppDevices.IsInUse(PatCur.PatNum,MADPage.PerioExamEditPage,MADPage.PerioExamListPage);
			bool IsAllowingEditing=false;
			if(_isExamInUse) {
				IsAllowingEditing=MsgBox.Show(MsgBoxButtons.YesNo,"This patients exam(s) are currently being edited in eClipboard. Would you like to edit this exam and close the eClipboard session?");
			}
			else {
				SetEClipBoardEditing(false);
				return;
			}
			if(IsAllowingEditing) {
				MobileAppDevice device=MobileAppDevices.GetForPat(PatCur.PatNum);
				if(device==null) {
					return;
				}
				OpenDentBusiness.WebTypes.PushNotificationUtils.CI_GoToCheckin(device.MobileAppDeviceNum);
				device.DevicePage=MADPage.CheckinPage;
				MobileAppDevices.Update(device);
				SetEClipBoardEditing(false);
			}
			return;
		}
		...
		private void butClose_Click(object sender,System.EventArgs e) {
			if(_userPrefCurrentOnly==null) {
				...
				UserOdPref userOdPrefOld=_userPrefCurrentOnly.Clone();
				_userPrefCurrentOnly.ValueString=POut.Bool(checkShowCurrent.Checked);
				if(UserOdPrefs.Update(_userPrefCurrentOnly,userOdPrefOld)) {
			...
			if(_userPrefCustomAdvance==null) {
				...
				UserOdPref userOdPrefOld=_userPrefCustomAdvance.Clone();
				_userPrefCustomAdvance.ValueString=POut.Bool(radioCustom.Checked);
				if(UserOdPrefs.Update(_userPrefCustomAdvance,userOdPrefOld)) {
					//Only need to signal cache refresh on change.
					DataValid.SetInvalid(InvalidType.UserOdPrefs);
				}
			}
