	...
	public partial class FormXWebTransactions:FormODBase {
		...
		private void contextMenu_Opening(object sender,System.ComponentModel.CancelEventArgs e) {
			if(gridMain.SelectedIndices.Length<=0) {
				...
				//We don't allow more than one row to be selected in this form
				if(gridMain.SelectedIndices.All(x=>IsXWebTransaction(x))) {
					...
					switch(PIn.String(_tableTrans.Rows[gridMain.SelectedIndices[0]]["TransactionStatus"].ToString())) {
						case "SALE":
							voidPaymentToolStripMenuItem.Visible=true;
							processReturnToolStripMenuItem.Visible=true;
							break;
