...
namespace OpenDental {
	public partial class FormRecurringChargesHistory:FormODBase {
		...
		private void FormRecurringChargesHistory_Load(object sender,EventArgs e) {
			...
			comboAutomated.Items.AddList(new[] {
		...
		private void FillGrid() {
			...
			gridMain.Columns.Add(new GridColumn(Lan.g(this,"Family Bal"),PrefC.HasClinicsEnabled ? 70 : 85,HorizontalAlignment.Right,
				GridSortingStrategy.AmountParse));
			gridMain.Columns.Add(new GridColumn(Lan.g(this,"PayPlan Due"),PrefC.HasClinicsEnabled ? 80 : 90,HorizontalAlignment.Right,
				GridSortingStrategy.AmountParse));
			gridMain.Columns.Add(new GridColumn(Lan.g(this,"Total Due"),PrefC.HasClinicsEnabled ? 65 : 80,HorizontalAlignment.Right,
				GridSortingStrategy.AmountParse));
			gridMain.Columns.Add(new GridColumn(Lan.g(this,"Repeat Amt"),PrefC.HasClinicsEnabled ? 75 : 90,HorizontalAlignment.Right,
				GridSortingStrategy.AmountParse));
			gridMain.Columns.Add(new GridColumn(Lan.g(this,"Charge Amt"),PrefC.HasClinicsEnabled ? 85 : 95,HorizontalAlignment.Right,
				GridSortingStrategy.AmountParse));
			...
			foreach(RecurringCharge charge in _listRecurringCharges.OrderBy(x => x.DateTimeCharge)) {
				...
				if(!datePicker.IsInDateRange(charge.DateTimeCharge) 
					|| (PrefC.HasClinicsEnabled && !comboClinics.ListSelectedClinicNums.Contains(charge.ClinicNum))
					|| !comboStatuses.GetListSelected<RecurringChargeStatus>().Contains(charge.ChargeStatus)
