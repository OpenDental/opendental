	...
	public partial class FormAutoNoteCompose:FormODBase {
		...
		private void FillTreeView() {
			...
			for(int i=0;i<listDefsRoots.Count;i++){
				treeView.Nodes.Add(CreateNodeAndChildren(listDefsRoots[i]));//child cats and categorized auto notes added in recursive function
			}
			...
			for(int i=0;i<listAutoNotes.Count;i++){
				...
				treeNode.Text=listAutoNotes[i].AutoNoteName;
				treeNode.ImageIndex=1;
				treeNode.SelectedImageIndex=1;
				treeNode.Tag=listAutoNotes[i];
				treeView.Nodes.Add(treeNode);
			}
			List<TreeNode> listTreeNodesFlat=new List<TreeNode>();
			for(int i=0;i<treeView.Nodes.Count;i++){
				List<TreeNode> listTreeNodesChildren=GetNodeAndChildren(treeView.Nodes[i]);
				listTreeNodesFlat.AddRange(listTreeNodesChildren);
			}
			for(int i=0;i<listTreeNodesFlat.Count;i++){
				if(!(listTreeNodesFlat[i].Tag is Def)){
					continue;
				}
				Def def=(Def)listTreeNodesFlat[i].Tag;
		...
		private TreeNode CreateNodeAndChildren(Def def) {
			...
			List<TreeNode> listTreeNodesChildren=new List<TreeNode>();
			//child folders
			List<Def> listDefsFolders=_listDefs.FindAll(x => x.ItemValue==def.DefNum.ToString());
			for(int i=0;i<listDefsFolders.Count;i++){
				listTreeNodesChildren.Add(CreateNodeAndChildren(listDefsFolders[i]));
			}
			//autonotes
			List<AutoNote> listAutoNotes=AutoNotes.GetWhere(x => x.Category==def.DefNum);
			for(int i=0;i<listAutoNotes.Count;i++){
				TreeNode treeNodeAN=new TreeNode();
				treeNodeAN.Text=listAutoNotes[i].AutoNoteName;
				treeNodeAN.ImageIndex=1;
				treeNodeAN.SelectedImageIndex=1;
				treeNodeAN.Tag=listAutoNotes[i];
				listTreeNodesChildren.Add(treeNodeAN);
			}
			TreeNode treeNode=new TreeNode();
			treeNode.Text=def.ItemName;
			treeNode.ImageIndex=0;
			treeNode.SelectedImageIndex=0;
			treeNode.Tag=def;
			listTreeNodesChildren=listTreeNodesChildren.OrderBy(x => x.Tag is AutoNote).ThenBy(x => x.Name).ToList();
			treeNode.Nodes.AddRange(listTreeNodesChildren.ToArray());
			return treeNode;
		}
		...
		private List<TreeNode> GetNodeAndChildren(TreeNode treeNode) {
			...
			listTreeNodes.Add(treeNode);//add self
			for(int i=0;i<treeNode.Nodes.Count;i++){
				List<TreeNode> listTreeNodesChildren=GetNodeAndChildren(treeNode.Nodes[i]);
				listTreeNodes.AddRange(listTreeNodesChildren);
			}
		...
		private void InsertSelectedAutoNote() {
			if(treeView.SelectedNode==null || !(treeView.SelectedNode.Tag is AutoNote)) {
				return;
			}
			string note=((AutoNote)treeView.SelectedNode.Tag).MainText;
			...
			treeView.SelectedNode=null;//clear selected node
		}
			...
			for(int i=0;i<listMatchesPrompts.Count;i++) {
				...
				if(autoNoteControl.ControlType=="Text") {//Response just inserts text. No choosing options here.
					...
					if(!frmAutoNotePromptText.IsDialogOK && frmAutoNotePromptText.IsRetry) {//user clicked the go back button
						...
						if(listAutoNoteItems.Count>i) {//reponse already exists for this control type. Update it
		...
		private void FormAutoNoteCompose_FormClosing(object sender,FormClosingEventArgs e) {
			...
			List<long> listDefNumsExpanded=treeView.Nodes.OfType<TreeNode>().SelectMany(x => GetNodeAndChildren(x))
