	...
	public partial class FormCreditCardManage:FormODBase {
		...
		private void FillGrid() {
          ...
          if(cc.DateStop.Year<1880) {
				...
				if(Programs.IsEnabled(ProgramName.EdgeExpress)) {
					if(!string.IsNullOrEmpty(cc.XChargeToken)&& (cc.CCSource==CreditCardSource.EdgeExpressCNP || cc.CCSource==CreditCardSource.EdgeExpressRCM)) {
						row.Cells.Add("X");
					}
					else {
						row.Cells.Add("");
				...
				if(Programs.IsEnabled(ProgramName.Xcharge)) {
					if(!string.IsNullOrEmpty(cc.XChargeToken)&& !cc.IsXWeb() && cc.CCSource==CreditCardSource.XServer) {
						row.Cells.Add("X");
					}
					else {
						row.Cells.Add("");
