...
namespace OpenDental {
	...
	public partial class FormApptReminderRuleEdit:FormODBase {
		...
		private void FillTabs() {
			...
			foreach(ApptReminderRule apptReminderRule in ListLanguageRules) {
				TabPage languageTab=new TabPage();
				CultureInfo culture=MiscUtils.GetCultureFromThreeLetter(apptReminderRule.Language);
				if(culture==null) {
					languageTab.Text=apptReminderRule.Language;
				}
				else {
					languageTab.Text=culture.DisplayName;
				}
				languageTab.Tag=apptReminderRule;
				LayoutManager.Add(languageTab,tabControl);
				UserControlReminderMessage userControlReminderMessageLang=new UserControlReminderMessage(apptReminderRule,LayoutManager);
		...
		private void butLanguage_Click(object sender,EventArgs e) {
			...
			foreach(string language in _arrLanguages) {
				...
				if(ListTools.In(language,listCurrentControls.Select(x => x.Language).ToList()))	{
