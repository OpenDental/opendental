...
namespace OpenDental {
	public partial class FormJobTeams:FormODBase {
		...
		private void FillListTeamUsers() {
			...
			List<Userod> listUserodsCur=_listUsers.FindAll(x => ListTools.In(x.UserNum,_listJobTeamUsersCur.Select(x => x.UserNumEngineer).ToList()));
		...
		private void butAddTeamMember_Click(object sender,EventArgs e) {
			List<Userod> listUsersNotInTeam=_listUsers.FindAll(x => !ListTools.In(x.UserNum,_listJobTeamUsersCur.Select(x => x.UserNumEngineer)));
