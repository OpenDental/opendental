	...
	public partial class FormJobManagerOverview:FormODBase {
		...
		private List<Job> ApplyFilters(List<Job> listJobs) {
			...
			if(comboTeams.SelectedIndex > 0 && comboEngineers.SelectedIndex==0) {//All engineers on one team selected.
				...
				listJobs=listJobs.Where(x => ListTools.In(x.UserNumConcept,listUserNums)
					|| ListTools.In(x.UserNumEngineer,listUserNums)
					|| ListTools.In(x.UserNumExpert,listUserNums)
					|| ListTools.In(x.UserNumQuoter,listUserNums)).ToList();
			}
			...
			if(!string.IsNullOrWhiteSpace(textWithinLastXDays.Text)) {
				...
				listJobs=listJobs
					.Where(x => x.ListJobTimeLogs.Any(y => y.DateTStamp >= DateTime.Now.AddDays(-withinLastXDays) && ListTools.In(y.ReviewerNum,x.UserNumExpert,x.UserNumEngineer)))
					.ToList();
			}
			listJobs=listJobs.Where(x => ListTools.In(x.Priority,listJobPrioritiesDefNums)
				&& ListTools.In(x.PhaseCur,listJobPhases)
				&& ListTools.In(x.Category,listJobCategories)).OrderByDescending(x => x.HoursActual).ToList();
		...
		private void comboTeams_SelectionChangeCommitted(object sender,EventArgs e) {
			...
			List<Userod> listUsers=JobHelper.ListEngineerUsers.FindAll(x => ListTools.In(x.UserNum,listUserNumsEngineer));
