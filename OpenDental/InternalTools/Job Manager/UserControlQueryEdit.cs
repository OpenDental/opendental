...
namespace OpenDental.InternalTools.Job_Manager {
	...
	public partial class UserControlQueryEdit:UserControl {
		...
		public void LoadJob(Job job,TreeNode treeNode=null) {
			...
			textDeadlineDate.Text=_jobCur.DateTimeTested.Year<1880?"":_jobCur.DateTimeTested.ToShortDateString();
			SetHoursLeft();
			FillAllGrids();
			IsChanged=false;
			CheckPermissions();
		...
		private void textDeadlineDate_Leave(object sender,EventArgs e) {
			if(_isLoading || IsNew) {
				return;
			}
			DateTime date;
			if(!DateTime.TryParse(textDeadlineDate.Text,out date)) {
				MsgBox.Show(this,"Not a valid date");
			}
			else {
				_jobCur.DateTimeTested=date;
			}
			textDeadlineDate.Text=_jobCur.DateTimeTested.Year<1880?"":_jobCur.DateTimeTested.ToString();
			Jobs.Update(_jobCur);
			Signalods.SetInvalid(InvalidType.Jobs,KeyType.Job,_jobCur.JobNum);
		}
