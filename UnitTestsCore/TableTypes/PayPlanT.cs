...
namespace UnitTestsCore {
	public class PayPlanT {
		...
		public static List<PayPlanCharge> CloseDynamicPaymentPlan(PayPlan payPlan,Family fam) {
			List<PayPlanCharge> _listPayPlanChargesDb=PayPlanCharges.GetForPayPlans(new List<long>{ payPlan.PayPlanNum });
			List<PayPlanLink> _listPayPlanLinks=PayPlanLinks.GetForPayPlans(new List<long>{ payPlan.PayPlanNum });
			List<PayPlanProductionEntry> _listPayPlanProductionEntries=PayPlanProductionEntry.GetProductionForLinks(_listPayPlanLinks);
			//make payment plan charges for the remaining amount of each piece of production
			List<PayPlanCharge> listChargesClosing=PayPlanEdit.GetCloseoutChargesForDynamicPaymentPlan(payPlan,fam,ref _listPayPlanChargesDb,ref _listPayPlanLinks,ref _listPayPlanProductionEntries);
			...
			PayPlanEdit.ApplyPrepaymentToDynamicPaymentPlan(payPlan.PatNum,hiddenUnearnedTotal,listChargesClosing,payPlanNum:payPlan.PayPlanNum);
			PayPlans.Update(payPlan);
