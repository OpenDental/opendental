			...
			protected override async Task PerformIO() {
				...
				try {
					...
					if(File.Exists(stashFilePath)) {
						...
						if(stashFileInfo.LastWriteTime==attribute.LastWriteTime) {
							FileContent=ProtectedData.Unprotect(File.ReadAllBytes(stashFilePath),null,DataProtectionScope.CurrentUser);
							return;
						}
					}
					else {
				...
				using(MemoryStream stream=new MemoryStream()) {
					...
					if(!stashFilePath.IsNullOrEmpty()) {
						File.WriteAllBytes(stashFilePath,ProtectedData.Protect(FileContent,null,DataProtectionScope.CurrentUser));
