...
namespace OpenDentBusiness{
	...
	public class Faqs {
		...
		///<summary>Grabs FaqNums linked to invalid manual pages and returns a dictionary of FaqNums and ManualPage statuses</summary>
		public static Dictionary<long,string> GetUnlinkedFaqDictionary() {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetSerializableDictionary<long,string>(MethodBase.GetCurrentMethod());
			}
			string command=$@"
			...
			DataAction.RunManualPublisherHQ(() => {
				dt=Db.GetTable(command);
			});
			Dictionary<long,string> retVal=dt.Select().ToDictionary(x=>PIn.Long(x["FaqNum"].ToString()),
				x=>PIn.Long(x["ManualPageNum"].ToString())==0 ? "Unlinked" : "Manual page does not exist");
			return retVal;
		}
