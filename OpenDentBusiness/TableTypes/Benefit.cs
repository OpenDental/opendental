...
namespace OpenDentBusiness{
	...
	public class Benefit:TableBase, IComparable {
		...
		private int GetFrequencyGroupNum() {
			if(ProcedureCodes.GetCodeNumsForPref(PrefName.InsBenBWCodes).Contains(CodeNum)) {
				return 1;
			}
			if(ProcedureCodes.GetCodeNumsForPref(PrefName.InsBenExamCodes).Contains(CodeNum)) {
				return 2;
			}
			if(ProcedureCodes.GetCodeNumsForPref(PrefName.InsBenPanoCodes).Contains(CodeNum)) {
				return 3;
			}
			if(ProcedureCodes.GetCodeNumsForPref(PrefName.InsBenCancerScreeningCodes).Contains(CodeNum)) {
				return 4;
			}
			if(ProcedureCodes.GetCodeNumsForPref(PrefName.InsBenProphyCodes).Contains(CodeNum)) {
				return 5;
			}
			if(ProcedureCodes.GetCodeNumsForPref(PrefName.InsBenFlourideCodes).Contains(CodeNum)) {
				return 6;
			}
			if(ProcedureCodes.GetCodeNumsForPref(PrefName.InsBenSealantCodes).Contains(CodeNum)) {
				return 7;
			}
			if(ProcedureCodes.GetCodeNumsForPref(PrefName.InsBenCrownCodes).Contains(CodeNum)) {
				return 8;
			}
			if(ProcedureCodes.GetCodeNumsForPref(PrefName.InsBenSRPCodes).Contains(CodeNum)) {
				return 9;
			}
			if(ProcedureCodes.GetCodeNumsForPref(PrefName.InsBenFullDebridementCodes).Contains(CodeNum)) {
				return 10;
			}
			if(ProcedureCodes.GetCodeNumsForPref(PrefName.InsBenPerioMaintCodes).Contains(CodeNum)) {
				return 11;
			}
			if(ProcedureCodes.GetCodeNumsForPref(PrefName.InsBenDenturesCodes).Contains(CodeNum)) {
				return 12;
			}
			if(ProcedureCodes.GetCodeNumsForPref(PrefName.InsBenImplantCodes).Contains(CodeNum)) {
				return 13;
			}
			return 0;
		}
