	...
	public class RpDPPOvercharged {
		...
		public static DataTable GetDPPOvercharged(DateTime dateStart,DateTime dateEnd,List<long> listClinicNums,List<long> listProvNums,long patNum) {
			...
			string query="SELECT payplan.DatePayPlanStart,"
				...
				+"ROUND("
					+"CASE payplanlink.LinkType "
						+"WHEN "+POut.Int((int)PayPlanLinkType.Adjustment)+" THEN IF(payplanlink.AmountOverride!=0,payplanlink.AmountOverride,adjustment.AdjAmt) "
						+"WHEN "+POut.Int((int)PayPlanLinkType.Procedure)+" THEN IF(payplanlink.AmountOverride!=0,payplanlink.AmountOverride,procedurelog.ProcFee*GREATEST(1,procedurelog.BaseUnits+procedurelog.UnitQty))"
