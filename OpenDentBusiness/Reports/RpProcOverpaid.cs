	...
	public class RpProcOverpaid {
			...
			bool isOnlyShowingInsOrWoOverpaid=false) 
		{
			...
			foreach(KeyValuePair<long,DataRow> kvp in dictCompletedProcRows) {
				...
				if(isOnlyShowingInsOrWoOverpaid) {//Bool for filtering out procs with no insurance or writeoff amount
					if(CompareDecimal.IsZero(insPaidAmt) && CompareDecimal.IsZero(woAmt)) {//Exclude from table when insurance and writeoff are 0
						continue;
					}
				}
				row=table.NewRow();
