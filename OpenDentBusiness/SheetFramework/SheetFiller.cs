	...
	public class SheetFiller {
		...
		private static void FillFieldsInStaticText(Sheet sheet,Patient pat,Family fam=null,StaticTextData data=null) {
			...
			if(pat!=null) {
				...
				//Procedure CodeNums-------------------------------------------------------------------------------------------------------------
				List<long> listProcCodeNums=SheetFiller.GetListProcCodeNumsForStaticText(out List<long> listCodeNumsPano,out List<long> listCodeNumsExam,out List<long> listCodeNumsProphy,
			...
			if((sheet.SheetType==SheetTypeEnum.PatientLetter || sheet.SheetType==SheetTypeEnum.ReferralSlip || sheet.SheetType==SheetTypeEnum.ReferralLetter) && pat!=null) {
				foreach(SheetField field in sheet.SheetFields) {
					if(field.FieldType!=SheetFieldType.StaticText) {
						continue;
					}
					...
					while(match.Success) {
						...
						if(examFields[0].RadioButtonGroup!="") {//a user defined 'misc' radio button check box, find the selected item and replace with reportable name
							...
							if(examFields[0].FieldType==SheetFieldType.ComboBox && match.Value.Contains(examFields[0].ReportableName)) {
								fldval=fldval.Replace(match.Value,examFields[0].FieldValue.Substring(0,examFields[0].FieldValue.IndexOf(';')));
							}
							else {
