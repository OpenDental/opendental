	...
	public class BillingL {
			...
			for(int i=0; i<sendStatementsIO.CurStatementBatch.ListStatements.Count; i++) {
				...
				if(statement.Mode_==StatementMode.Email) {
					...
					try {
						...
						sendStatementsIO.ActionSendEmail(clinicNumPat,message,emailAddress,useSecureEmail);
		...
		private static bool SendTextMessages(SendStatementsIO sendStatementsIO) {
			...
			for(int i=0; i<sendStatementsIO.CurStatementBatch.ListStatements.Count; i++) {
				...
				try {
					textToSend.MsgText=Statements.ReplaceVarsForSms(PrefC.GetString(PrefName.BillingDefaultsSmsTemplate),patient,statement);
				}
				catch(Exception e) {
