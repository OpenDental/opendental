...
namespace OpenDentBusiness {
	public class PayConnect {
		...
		public static string GetHpfUrlForPayment(Patient pat,string accountToken,string email,string payNote,bool isMobile,double amount,bool saveToken,CreditCardSource ccSource) {
			if(pat==null) {
		...
		public static void MakePaymentWithAlias(Patient pat,string email,string payNote,double amount,CreditCard cc) {
			if(pat==null) {
				throw new ODException("No Patient Found",ODException.ErrorCodes.NoPatientFound);
			}
			if(amount<0.00 || amount>99999.99) {
