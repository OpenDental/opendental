	...
	public class AvaTax {
		...
		public static bool DoSendProcToAvalara(Procedure procedure,bool isSilent=false) {
			...
			if(!Patients.HasValidUSZipCode(patient)) {
				if(isSilent) {
					_logWritter.WriteLine($"Invalid ZipCode for PatNum {procedure.PatNum} while running Repeat Charge Tool on {DateTime.Today}",LogLevel.Error);
				}
				else {
		...
		public static string GetTaxOverrideIfNeeded(Patient patient,ProcedureCode procedureCode) {
			...
			foreach(string entry in stringArrayOverrides) {
				...
				if(stringArrayParts.Count()!=3) {
					_logWritter.WriteLine("Tax Code Override entry is incorrect: "+entry+".  "
		...
		public static decimal GetEstimate(long codeNum,long patNum,double procFee,bool hasExceptions=false) {
			...
			try {
				...
				_logWritter.WriteLine($"Error getting estimate from Avatax for PatNum: {patNum}",LogLevel.Error);
				if(hasExceptions) {
		...
		public static bool DidUpdateAdjustment(Procedure procedure,Adjustment adjustment) {
			...
			try {
				...
				_logWritter.WriteLine("Encountered an Avatax error: "+JsonConvert.SerializeObject(avaTaxError.error.error),LogLevel.Error);
				message=avaTaxError.error.error.message;
			}
			catch(Exception ex) {
				_logWritter.WriteLine("Unable to send or receive transaction: "+JsonConvert.SerializeObject(ex),LogLevel.Error);
				message=ex.Message;
			}
		...
		private static LogWriter _logWritter=new LogWriter(GetLogLevelDetail(),"AvaTax");
