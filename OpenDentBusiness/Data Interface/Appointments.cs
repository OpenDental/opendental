...
namespace OpenDentBusiness{
	...
	public class Appointments {
		...
		public static List<Appointment> GetAppointmentsForApi(int limit,int offset,DateTime dateStart,DateTime dateEnd,DateTime dateTStamp,long clinicNum){
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<List<Appointment>>(MethodBase.GetCurrentMethod(),limit,offset,dateStart,dateEnd,dateTStamp,clinicNum);
			}
		...
		public static DataTable GetAddrTable(List<long> aptNums, bool groupByFamily) {
			...
			for(int i=0;i<rawTable.Rows.Count;i++) {
				if(!groupByFamily) {
					...
					row["clinicNum"]=rawTable.Rows[i]["ClinicNum"].ToString();
					row["AptDateTime"]=PIn.Date(rawTable.Rows[i]["AptDateTime"].ToString());
					row["DateTimeASkedToArrive"]=PIn.Date(rawTable.Rows[i]["DateTimeAskedToArrive"].ToString());
					...
					row["MiddleI"]=rawTable.Rows[i]["MiddleI"].ToString();
					row["patNums"]=rawTable.Rows[i]["PatNum"].ToString();
					row["PatNum"]=rawTable.Rows[i]["PatNum"].ToString();
					row["Preferred"]=PIn.String(rawTable.Rows[i]["Preferred"].ToString());
				...
				if(familyAptList=="") {//if this is the first patient in the family
						...
						|| rawTable.Rows[i]["Guarantor"].ToString()!=rawTable.Rows[i+1]["Guarantor"].ToString())//or if the guarantor on next line is different
					{
						...
						familyAptList=rawTable.Rows[i]["FName"].ToString()+":  "
							+PIn.Date(rawTable.Rows[i]["AptDateTime"].ToString()).ToShortDateString()+" at "
							+PIn.DateT(rawTable.Rows[i]["AptDateTime"].ToString()).ToShortTimeString();
						patNumStr=rawTable.Rows[i]["PatNum"].ToString();
						continue;
					}
				}
				else{//not the first patient
					familyAptList+="\r\n"+rawTable.Rows[i]["FName"].ToString()+":  "
						+PIn.Date(rawTable.Rows[i]["AptDateTime"].ToString()).ToShortDateString()+" at "
						+PIn.DateT(rawTable.Rows[i]["AptDateTime"].ToString()).ToShortTimeString();
					patNumStr+=","+rawTable.Rows[i]["PatNum"].ToString();
				}
