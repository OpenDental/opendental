	...
	public class SmsPhones{
		...
		public static bool UpdateOrInsertFromList(List<SmsPhone> listSmsPhonesSync) {
			...
			for(int i=0;i<listSmsPhonesSync.Count;i++) {
				SmsPhone smsPhoneOld=listSmsPhonesDb.FirstOrDefault(x => x.PhoneNumber==listSmsPhonesSync[i].PhoneNumber);
				//Upsert.
				...
				smsPhoneOld.ClinicNum=listSmsPhonesSync[i].ClinicNum; //The clinic may have changed so set it to the new clinic.
				smsPhoneOld.CountryCode=listSmsPhonesSync[i].CountryCode;
				smsPhoneOld.DateTimeActive=listSmsPhonesSync[i].DateTimeActive;
				smsPhoneOld.DateTimeInactive=listSmsPhonesSync[i].DateTimeInactive;
				smsPhoneOld.InactiveCode=listSmsPhonesSync[i].InactiveCode;
				Update(smsPhoneOld);
				isChanged=true;
