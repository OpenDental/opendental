	...
	public class Procedures {
			...
			long procCodeNum,long procProvNum,long procClinicNum,string procMedicalCode,List<Benefit> listBenefits=null,List<Fee> listFees=null)
		{
			...
			if(listPatPlans.Count>0) {
				InsSub insSubPrimary=InsSubs.GetSub(listPatPlans.Find(x => x.Ordinal==1).InsSubNum,listInsSubs);
				if(insSubPrimary.InsSubNum!=0) {
		...
		public static List<ProcedureForApi> GetProceduresForApi(long patNum,DateTime dateTStamp) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
		...
		///<summary>Can throw exception if codeNum is in a hidden category. Creates the auto ortho procedure for the passed in patient.</summary>
		public static Procedure CreateOrthoAutoProcsForPat(long patNum,long codeNum,long provNum,long clinicNum, DateTime procDate) {
			...
			if(ProcedureCodes.AreAnyProcCodesHidden(codeNum)) {
				//Note: this thrown message does not show up because the one reference (FormOrthoAutoClaims) catches it and does not display it.
				throw new ODException($"Cannot create auto ortho procedure because procedure is in a hidden category: {ProcedureCodes.GetProcCode(codeNum)}",1);
			}
			Procedure proc=new Procedure {
				PatNum=patNum,
				CodeNum=codeNum,
			...
			//lookupFees passed in will also contain all possible alternate codes and medical codes
		{
			...
			for(int i=0;i<claimProcs.Count;i++) {
				...
				if(HasMetFrequencyLimitation(claimProcs[i],histList,benefitList,proc,procCode,PlanCur,planList,loopList) && hasEstimateCalculation) {
		...
		public static bool HasMetFrequencyLimitation(ClaimProc claimProc,List<ClaimProcHist> histList,List<Benefit> benefitList,Procedure proc,
			ProcedureCode procCode,InsPlan planCur,List<InsPlan> listInsPlans,List<ClaimProcHist> loopList=null) 
		{
			//No need to check RemotingRole; no call to db.
			...
			foreach(Benefit ben in listBensForGroup) {
				...
				if(ben.QuantityQualifier==BenefitQuantity.Months || ben.QuantityQualifier==BenefitQuantity.Years) {
					...
					ClaimProcHist claimProcHist=listAll.Find(x => x.StrProcCode==benCodeStr 
						&& ListTools.In(x.Status,arrayClaimProcStatuses)
						&& x.ProcDate>=datePast 
				...
				if(ben.QuantityQualifier==BenefitQuantity.NumberOfServices) {
					...
					if(ben.TimePeriod==BenefitTimePeriod.NumberInLast12Months) {
						...
						claimProcHistList=claimProcHistList
							.Where(x => ListTools.In(x.Status,arrayClaimProcStatuses)
						...
						claimProcHistList=claimProcHistList.FindAll(x => ListTools.In(x.Status,arrayClaimProcStatuses)
							&& x.ProcDate>=datePast 
