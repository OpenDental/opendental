	...
	public class Procedures {
			...
			ref List<ClaimProc> listClaimProcsForProc)
		{
			...
			if(procedureOld.ProcStatus!=ProcStat.C && procedure.ProcStatus==ProcStat.C){//if status was changed to complete
				...
				if(!isQuickAdd && !Security.IsAuthorized(Permissions.ProcComplCreate, textDate, actionNotAuthorized: actionOnFailure)){//use the new date
					return false;
				}
			}
			...
			Action<string> actionNotAuthorized=null)
		{
			...
			if(procOld.ProcStatus!=procNew.ProcStatus && !Security.IsAuthorized(perm,dateToUseProcOld,actionNotAuthorized:actionNotAuthorized)) {//block old date
				return false;
			}
			if(procNew.ProcStatus.In(ProcStat.C,ProcStat.EO,ProcStat.EC)) {
				...
				if(procOld.ProcStatus!=procNew.ProcStatus) {
					...
					if(!Security.IsAuthorized(perm,dateToUseProcCur,actionNotAuthorized:actionNotAuthorized)) {//block new date, too
						return false;
					}
				}
		...
		public static void UpdateProcedureFields(Procedure procedure,Patient patient,string textMedicalCodeText,double discount,Snomed snomedBodySite,
			bool checkIcdVersionChecked, List<string> diagnosticCodes,bool checkIsPrincDiagChecked,long selectedProvOrderNum,Referral _referralOrdering,
			...
			float drugQty,ProcUrgency procUrgency,long selectedProvNum,long clinicNum)
		{
			...
			if(_referralOrdering==null) {
				...
				procedure.OrderingReferralNum=_referralOrdering.ReferralNum;
			}
