...
namespace OpenDentBusiness{
	...
	public class ApptReminderRules{
			...
			,Action<string> logOnError=null) 
		{
			...
			else if(aptCur.AptStatus.In(ApptStatus.Broken,ApptStatus.UnschedList,ApptStatus.Complete)) {//Broken,Unscheduled, and Complete appointments should not be confirmed.
				logError("when AptStatus is "+aptCur.AptStatus.GetDescription());
				return ConfStatusUpdate.Failure;
			}
			else if(request.ApptDateTime!=aptCur.AptDateTime) {//Appointments moved from their confirmationrequest original time should not be confirmed.
			...
			switch(ruleType) {
				case ApptReminderType.Reminder:
					rule=new ApptReminderRule() {
						...
						TemplateSMS="Appointment Reminder: [NameF] is scheduled for [ApptTime] on [ApptDate] at [ClinicName]. [Premed]If you have questions call [ClinicPhone].",//default message
						TemplateEmail=@"[NameF],

Your appointment is scheduled for [ApptTime] on [ApptDate] at [OfficeName]. [Premed]If you have questions, call <a href=""tel:[OfficePhone]"">[OfficePhone]</a>.",
