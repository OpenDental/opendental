...
namespace OpenDentBusiness{
	...
	public class ProcedureCodes {
		...
		//The method below used to be the property BitewingCode, and it used PrefName.InsBenBWCodes
		///<summary>Attempts to get ProcCode from Prefs, or returns hard-coded defaults.</summary>
		public static string GetProcCode(PrefName prefName) {
			List<long> listCodeNums=GetCodeNumsForPref(prefName);
			//return GetProcCodeForCodeNum(GetListBWCodeNums(),"D0274","02144");
			...
			if(CultureInfo.CurrentCulture.Name.EndsWith("CA")) {
				switch(prefName){
					case PrefName.InsBenBWCodes:
						return "02144";
					case PrefName.InsBenPanoCodes:
						if(Canadian.IsQuebec()) {
							return "02600";
						}
						return "02601";
					case PrefName.InsBenCancerScreeningCodes:
						return "01401";
					case PrefName.InsBenProphyCodes:
						return "11101";
					case PrefName.InsBenFlourideCodes:
						return "12111";
					case PrefName.InsBenSealantCodes:
						return "13401";
					case PrefName.InsBenCrownCodes:
						return "27201";
					case PrefName.InsBenSRPCodes:
						return "43421";
					case PrefName.InsBenFullDebridementCodes:
						return "42121";
					case PrefName.InsBenPerioMaintCodes:
						return "49101";
					case PrefName.InsBenDenturesCodes:
						return "51711";
					case PrefName.InsBenImplantCodes:
						return "79921";
				}
			}
			switch(prefName){
		...
		public static long GetCodeNumForPref(PrefName prefName) {
			//No need to check MiddleTierRole; no call to db.
			string strProcCode=GetProcCode(prefName);
			if(IsValidCode(strProcCode)) {
				return _procedureCodeCache.GetOne(strProcCode).CodeNum;
			}
			return 0;
		}
		...
		public static List<long> GetCodeNumsForPref(PrefName prefName) {
			List<string> listCodes=PrefC.GetString(prefName).Split(',').Select(x => x.Trim()).ToList();//ProcCode aka D Code
		...
		public static List<long> GetCodeNumsForInsHistPref(PrefName prefName) {
			...
			switch(prefName) {
				case PrefName.InsHistBWCodes:
					listCodeNums.AddRange(GetCodeNumsForPref(PrefName.InsBenBWCodes));
					break;
				case PrefName.InsHistExamCodes:
					listCodeNums.AddRange(GetCodeNumsForPref(PrefName.InsBenExamCodes));
					break;
				case PrefName.InsHistPanoCodes:
					listCodeNums.AddRange(GetCodeNumsForPref(PrefName.InsBenPanoCodes));
					break;
				...
				case PrefName.InsHistPerioURCodes:
					listCodeNums.AddRange(GetCodeNumsForPref(PrefName.InsBenSRPCodes));
					break;
				case PrefName.InsHistPerioMaintCodes:
					listCodeNums.AddRange(GetCodeNumsForPref(PrefName.InsBenPerioMaintCodes));
					break;
				case PrefName.InsHistProphyCodes:
					listCodeNums.AddRange(GetCodeNumsForPref(PrefName.InsBenProphyCodes));
					break;
				default:
