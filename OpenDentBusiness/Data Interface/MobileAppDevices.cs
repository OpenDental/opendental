...
namespace OpenDentBusiness{
	...
	public class MobileAppDevices{
		...
		public static bool IsInUse(MADPage devicePage,long patNum=0) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetBool(MethodBase.GetCurrentMethod(),devicePage,patNum);
			}
			string command=$"SELECT COUNT(*) FROM mobileappdevice WHERE DevicePage={POut.Enum<MADPage>(devicePage)} ";
			if(patNum>0) {
