...
namespace OpenDentBusiness{
	...
	public class Claims{
		...
		public static List<Claim> GetSecondaryClaimsNotReceived(List<ClaimProc> listClaimProcsForPriClaims) {
			...
			List<long> listProcNumsOnPriClaims=listClaimProcsForPriClaims.FindAll(x => x.ProcNum>0).Select(x => x.ProcNum).ToList();
			...
			return GetClaimsFromClaimNums(listClaimProcsForPriProcsNotReceived.Select(x => x.ClaimNum).ToList())
				.FindAll(x=> x.ClaimStatus.In("U","H") && x.ClaimType=="S");
		}
		...
		public static void CalculateAndUpdateSecondariesFromPrimaries(List<Claim> listPrimaryClaims) {
			...
			List<Claim> listSecondaryClaims=Claims.GetSecondaryClaimsNotReceived(listClaimProcs);
