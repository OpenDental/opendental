	...
	public class Tasks {
			...
			bool doIncludeTaskNote,bool doIncludeCompleted,bool doIncludeAttachments,bool limit) 
		{
			...
			if(taskIncluding!="") {
				...
				string[] arrayTaskIncluding=TaskQuoteHelper(taskSearch:taskIncluding);
				for(int i=0;i<arrayTaskIncluding.Length;i++) {
					if(doIncludeAttachments) {
						listWhereClauses.Add("(task.Descript LIKE '%"+POut.String(arrayTaskIncluding[i])+"%' OR taskattachment.Description LIKE '%"+POut.String(arrayTaskIncluding[i])+"%' "
							+"OR taskattachment.TextValue LIKE '%"+POut.String(arrayTaskIncluding[i])+"%')");
					}
					else {
						listWhereClauses.Add("task.Descript LIKE '%"+POut.String(arrayTaskIncluding[i])+"%'");
					}
					listWhereNoteClauses.Add("tasknote.Note LIKE '%"+POut.String(arrayTaskIncluding[i])+"%'");
				}
			}
			...
			if(taskExcluding!="") {
				string[] arrayTaskExcluding=TaskQuoteHelper(taskSearch:taskExcluding);
		...
		private static string[] TaskQuoteHelper(string taskSearch) {
			...
			for(int i=0;i<listTaskSearches.Count;i++) {
				if(char.IsWhiteSpace(listTaskSearches[i],0)) {//0th position is whitespace.
					string[] arraySubstrings=listTaskSearches[i].Trim().Split(" ",StringSplitOptions.RemoveEmptyEntries);
					listTaskSearches.RemoveAt(i--);
					listTaskSearches.AddRange(arraySubstrings);
				}
				else {
					listTaskSearches[i]=listTaskSearches[i].Trim();
				}
			}
			return listTaskSearches.ToArray();
		}
		...
		public static SerializableDictionary<long,string> GetApptObjDescripts(List<long> listPatApts){
			...
			foreach(DataRow aptRow in table.Rows) {
				...
				string dateTimeStr=apt.AptDateTime.ToString();
				if(PIn.Enum<ApptStatus>(aptRow["AptStatus"].ToString())==ApptStatus.UnschedList) {
				...
				dictTaskString.Add(apt.AptNum,patname+" "+dateTimeStr+" "+apt.ProcDescript+" "+apt.Note+" - ");
			}
		...
		public static int CompareAptDateTimes(DataRow x,DataRow y) {
			TaskAptShort xTaskAptShort=new TaskAptShort() { AptDateTime=DateTime.MaxValue };
			TaskAptShort yTaskAptShort=new TaskAptShort() { AptDateTime=DateTime.MaxValue };
			if(_dictTaskApts.ContainsKey(PIn.Long(x["KeyNum"].ToString()))) {
				xTaskAptShort=_dictTaskApts[PIn.Long(x["KeyNum"].ToString())];
			...
			if(_dictTaskApts.ContainsKey(PIn.Long(y["KeyNum"].ToString()))) {
				yTaskAptShort=_dictTaskApts[PIn.Long(y["KeyNum"].ToString())];
