	...
	public class ClockEvents {
      ...
      return clockEventNum;
		...
		public static void ClockIn(long employeeNum,bool isAtHome) {
			...
			SecurityLogs.MakeLogEntry(EnumPermType.UserLogOnOff,0,emp.FName+" "+emp.LName+" "+"clocked in from "+clockEvent.ClockStatus.ToString()+".");
		}
		...
		public static void ClockOut(long employeeNum,TimeClockStatus clockStatus) {
			...
			Employee emp=Employees.GetEmp(employeeNum);
			SecurityLogs.MakeLogEntry(EnumPermType.UserLogOnOff,0,emp.FName+" "+emp.LName+" "+"clocked out for "+clockEvent.ClockStatus.ToString()+".");
		}
		...
		public static DataTable GetTimeCardManage(DateTime startDate,DateTime stopDate,long clinicNum,bool isAll) {
			...
			foreach(Employee employee in listEmployees) {
				...
				for(int i=0;i<listEmpClockEvents.Count;i++) {
					...
					//Rate3
					if(listEmpClockEvents[i].Rate3Hours!=TimeSpan.FromHours(-1)) {//Manual override
