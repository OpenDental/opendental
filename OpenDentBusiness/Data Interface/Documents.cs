...
namespace OpenDentBusiness {
	...
	public class Documents {
		...
		/// <summary>Returns a bool indicating if the file was created sucessfully for a temporary PDF file for passed in treatment plan.</summary>
		public static bool TryCreateTreatmentPlanPdfFile(TreatPlan treatPlan, out string filePath) {
			try {
				SheetDrawingJob sheetDrawingJob=new SheetDrawingJob();
		...
		public static bool CreateAndSaveTreatmentPlanPDF(TreatPlan treatPlan) {
			try {
				...
				if(!TryCreateTreatmentPlanPdfFile(treatPlan,out string tempFile)) {
					return false;
				}
				string rawBase64="";
				if(PrefC.AtoZfolderUsed!=DataStorageType.LocalAtoZ) {
