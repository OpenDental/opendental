...
namespace OpenDentBusiness {
	...
	public class Documents {
		...
		public static DataSet RefreshForPatient(long patNum,bool isImagingOrderDescending=false) {
			...
			retVal.Tables.Add(GetTreeListTableForPatient(patNum,isImagingOrderDescending));
			return retVal;
		}
		...
		public static DataTable GetTreeListTableForPatient(long patNum,bool isImagingOrderDescending){
			...
			command="SELECT DocNum FROM document WHERE PatNum="+POut.Long(patNum)+" AND (DocCategory NOT IN ("+string.Join(",",listImageCatDefs.ToArray())+") OR DocCategory < 0)";
			...
			resultSet.Sort(delegate(Object o1,Object o2) {
				...
				if(isImagingOrderDescending){
					return PIn.Date(r2["DateCreated"].ToString()).CompareTo(PIn.Date(r1["DateCreated"].ToString()));
				}
		...
		public static DataTable GetTreeListTableForPatientForApi(long patNum,string filePath,int limit,int offset,string dateTimeFormatString) {
			...
			for(int i=0;i<tableDocuments.Rows.Count;i++) {
				...
				row["PatNum"]=PIn.Long(tableDocuments.Rows[i]["PatNum"].ToString());
				...
				row["ProvNum"]=PIn.Long(tableDocuments.Rows[i]["ProvNum"].ToString());
				row["PrintHeading"]=PIn.Bool(tableDocuments.Rows[i]["PrintHeading"].ToString()).ToString().ToLower();//Since we format bools as lowercase in API
			...
			for(int i=0;i<tableMounts.Rows.Count;i++) {
				...
				row["PatNum"]=PIn.Long(tableMounts.Rows[i]["PatNum"].ToString());
				...
				row["ProvNum"]=PIn.Long(tableMounts.Rows[i]["ProvNum"].ToString());
				row["PrintHeading"]=""; //not a field for Mounts
