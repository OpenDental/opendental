...
namespace OpenDentBusiness{
	...
	public class Payments {
		...
		public static long ProcessPaymentForWeb(Payment odbPayment,Patient odbPatient,double payAmt,bool isPatientPreferred=false) {
			...
			autoSplitData.ListAutoSplits.RemoveAll(x => CompareDouble.IsZero(x.SplitAmt));
			//All payments must have one split
			if(autoSplitData.ListAutoSplits.Count==0) {
				PaySplit odbPaySplit=CreateSinglePaySplitForWeb(odbPayment,odbPatient,odbPayment.PayAmt);
				autoSplitData.ListAutoSplits.Add(odbPaySplit);
			}
			//If there is extra or negative money leftover when making splits, put it into one last split.
			double payDifference=odbPayment.PayAmt-autoSplitData.ListAutoSplits.Sum(x => x.SplitAmt);
			if(payDifference!=0) {
				PaySplit odbPaySplit=CreateSinglePaySplitForWeb(odbPayment,odbPatient,payDifference);
				autoSplitData.ListAutoSplits.Add(odbPaySplit);
			}
			long ret=Payments.Insert(odbPayment,autoSplitData.ListAutoSplits);
			//Compute Aging
			Family odbFamily=Patients.GetFamily(odbPatient.PatNum);
			Ledgers.ComputeAgingForPaysplitsAllocatedToDiffPats(odbPatient.PatNum,autoSplitData.ListAutoSplits);
