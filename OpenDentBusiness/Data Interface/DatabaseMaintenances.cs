...
HAVING cnt>1";
		...
		public static string PatientMissing() {
			...
			string command="SELECT PatNum FROM (\r\n"+string.Join("\r\nUNION\r\n",listTablesToCheck.Select(x =>
					$"SELECT DISTINCT(PatNum) PatNum FROM {POut.String(x)} WHERE PatNum NOT IN (SELECT PatNum FROM patient)"))+"\r\n"+
				") missing ";
			...
			for(int i=0;i<listMissingPatNums.Count;i++) {
				...
				if(listMissingPatNums[i]==0) {
					for(int j=0;j<listTablesToCheck.Count;j++) {
						command=$"SELECT COUNT(PatNum) FROM {POut.String(listTablesToCheck[j])} WHERE PatNum=0";
						if(Db.GetCount(command)=="0") {
							continue;
						}
						command=$"UPDATE {POut.String(listTablesToCheck[j])} SET PatNum="+POut.Long(tempPat.PatNum)+" WHERE PatNum=0";
						Db.NonQ(command);
					}
				}
