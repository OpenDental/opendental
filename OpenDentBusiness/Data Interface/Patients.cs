...
namespace OpenDentBusiness {
	...
	public class PatComm : WebTypes.WebBase {
		...
		public static string BuildConfirmMessage(ContactMethod contactMethod,Patient pat,DateTime apptDateTime,DateTime dateTimeAskedToArrive) {
			...
			PatComm patComm=Patients.GetPatComms(new List<Patient> { pat }).FirstOrDefault();
			message=message.Replace("[NameF]",patComm?.GetFirstOrPreferred()??"");
			...
			message=message.Replace("[NameFL]",patComm?.GetFirstOrPreferred()??"");//Last name should NOT be included in confirmations.
			message=message.Replace("[date]",dateTime.ToString(PrefC.PatientCommunicationDateFormat));
			message=message.Replace("[time]",dateTime.ToShortTimeString());
			return message;
		}
