...
namespace OpenDentBusiness {
	...
	public class Patients {
		...
		public static List<PatientForApi> GetPatientsSimpleForApi(int limit,int offset,string lName,string fName,DateTime birthdate,int patStatus,long clinicNum,DateTime dateTStamp) {
			...
			List<Patient> listPatients=Crud.PatientCrud.SelectMany(command);
			List<PatientForApi> listPatientForApis=new List<PatientForApi>();
			for(int i = 0;i<listPatients.Count;i++) {
				PatientForApi patientForApi=new PatientForApi();
