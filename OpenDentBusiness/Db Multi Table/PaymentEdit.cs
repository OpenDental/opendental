...
namespace OpenDentBusiness {
	public class PaymentEdit {
			...
			,bool isIncomeTransfer,bool doIncludeTreatmentPlanned=false)
		{
			...
			data.ListClaimProcsFiltered=ClaimProcs.Refresh(listPatNums).FindAll(x => x.ProcNum!=0);
		...
		public static ConstructResults ConstructAndLinkChargeCredits(long patNum,List<long> listPatNums=null,bool isIncomeTxfr=false,
			LoadData loadData=null,bool doIncludeTreatmentPlanned=false)
		{
			//No remoting role check; no call to db
			if(listPatNums==null) {
			...
			return ConstructAndLinkChargeCredits(listPatNums,patNum,new List<PaySplit>(),new Payment(),new List<AccountEntry>(),
				isIncomeTxfr:isIncomeTxfr,loadData:loadData,doIncludeTreatmentPlanned:doIncludeTreatmentPlanned);
		}
			...
			,bool doIncludeExplicitCreditsOnly=false,bool isAllocateUnearned=false,DateTime dateAsOf=default,bool doIncludeTreatmentPlanned=false)
		{
			...
			retVal.ListAccountCharges=ConstructListCharges(listPatNums,
				...
				|| (isIncomeTxfr && isAllocateUnearned))
			{
				...
				if(!listPayFirstAcctEntries.IsNullOrEmpty()) {
					...
					List<long> listPayPlanNums=listPayFirstAcctEntries.Where(x => x.PayPlanNum > 0).Select(x => x.PayPlanNum).ToList();
					listAccountEntriesRelated.AddRange(retVal.ListAccountCharges.Where(x => x.GetType()==typeof(FauxAccountEntry)
							&& CompareDecimal.IsGreaterThanZero(x.AmountEnd)
							&& ListTools.In(x.PayPlanNum,listPayPlanNums))
						.Cast<FauxAccountEntry>()
						.OrderBy(x => CompareDecimal.IsGreaterThanZero(x.Interest)));//Pay interest first.
					//Related Procedures==============================================================================================================
					List<long> listProcNums=listPayFirstAcctEntries.Where(x => x.GetType()==typeof(Procedure)).Select(x => x.ProcNum).ToList();
					listAccountEntriesRelated.AddRange(retVal.ListAccountCharges.FindAll(x => x.GetType()==typeof(Procedure)
							&& CompareDecimal.IsGreaterThanZero(x.AmountEnd)
							&& ListTools.In(x.ProcNum,listProcNums)));
					//Related Adjustments=============================================================================================================
					List<long> listAdjNums=listPayFirstAcctEntries.Where(x => x.GetType()==typeof(Adjustment)).Select(x => x.AdjNum).ToList();
					listAccountEntriesRelated.AddRange(retVal.ListAccountCharges.FindAll(x => x.GetType()==typeof(Adjustment)
							&& CompareDecimal.IsGreaterThanZero(x.AmountEnd)
							&& ListTools.In(x.AdjNum,listAdjNums)));
					...
					double amtToAllocate=payCur.PayAmt;
					...
					for(int i=0;i<listAccountEntriesRelated.Count;i++) {
						...
						PaySplit split=CreatePaySplitHelper(listAccountEntriesRelated[i],splitAmt,payCur.PayDate,payNum:payCur.PayNum,isNew:true);
						...
						retVal.ListSplitsCur.Add(split);
					}
				}
				#endregion
			...
			List<ClaimProc> listClaimProcs,List<PayPlan> listInsPayPlans=null)
		{
			...
			if(!listPayPlanCharges.IsNullOrEmpty() || !listPayPlanLinks.IsNullOrEmpty()) {
				...
				List<AccountEntry> listAccountEntryPayPlanProcsOutsideFam=listAccountEntryProcsOutsideFam
					.FindAll(x => ListTools.In(x.ProcNum,listPatientPayPlanProcNums) || ListTools.In(x.ProcNum,listDynamicPayPlanProcNums));
				...
				List<AccountEntry> listAccountEntryAdjsOutsideFam=listCharges.FindAll(x => x.AdjNum > 0
					...
					&& ListTools.In(x.AdjNum,listAdjPayPlanLinks.Select(y => y.FKey)));
				//Set the AmountEnd for every single entry based on how much value is associated to payment plans.
		...
		private static List<AccountEntry> ExplicitlyLinkCredits(List<AccountEntry> listAccountEntries,List<PaySplit> listSplitsCurrentAndHistoric) {
			...
			foreach(var kvpPatProvClinicSplits in dictPatProvClinicSplits) {
				...
				List<FauxAccountEntry> listPayPlanEntries=listExplicitAccountCharges
					.FindAll(x => x.GetType()==typeof(FauxAccountEntry))
					.Cast<FauxAccountEntry>()
			...
			//Positive and negative unattached adjustments should offset each other (regardless of the 'treat negative production as income' preference).
		...
		public static AutoSplit AutoSplitForPayment(long patNum,Payment paymentCur,bool isIncomeTxfr=false,bool isPatPrefer=false,long payPlanNum=0,
			List<AccountEntry> listPayFirstAcctEntries=null) 
		{
			//No need to check RemotingRole; no call to db.
			if(listPayFirstAcctEntries==null) {
				listPayFirstAcctEntries=new List<AccountEntry>();
			...
			bool doIncludeExplicitCreditsOnly=false,long payPlanNum=0)
		{
			...
			AutoSplit autoSplit=AutoSplitForPayment(constructResults,doAutoSplit,payPlanNum:payPlanNum,listPayFirstAcctEntries:listPayFirstAcctEntries);
			return autoSplit;
		}

		public static AutoSplit AutoSplitForPayment(ConstructResults constructResults,bool doAutoSplit=true,long payPlanNum=0,List<AccountEntry> listPayFirstAcctEntries=null) {
			...
			autoSplitData.ListAutoSplits=new List<PaySplit>(constructResults.ListSplitsCur);
			...
			if(!listPayFirstAcctEntries.IsNullOrEmpty()) {
				...
				listAutoSplitAccountEntries=listAutoSplitAccountEntries
					.OrderByDescending(x => listPayFirstAcctEntries.Any(y => y.PriKey==x.PriKey && y.GetType()==x.GetType()))
					.ThenByDescending(x => listPayFirstAcctEntries.Any(y => y.PayPlanNum > 0 && y.PayPlanNum==x.PayPlanNum))
					.ToList();
			}
			//Create a variable to keep track of the money that can be allocated for this payment.
			double amtToAllocate=(autoSplitData.Payment.PayAmt - autoSplitData.ListAutoSplits.Sum(x => x.SplitAmt));
			...
			foreach(AccountEntry charge in listAutoSplitAccountEntries) {
				...
				//Make a new split that will apply as much value as possible from the account entry.
				PaySplit split=CreatePaySplitHelper(charge, splitAmt,autoSplitData.Payment.PayDate,payNum:autoSplitData.Payment.PayNum,isNew:true);
				//Remove the value from the account entry
		...
		private static IncomeTransferData TransferLoopHelper(IncomeTransferBucket bucket,DateTime datePay) {
			//No remoting role check; private method
