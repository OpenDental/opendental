...
namespace OpenDentBusiness {
	public class PaymentEdit {
			...
			List<AccountEntry> listCharges)
		{
			...
			foreach(FauxAccountEntry entry in listPayPlanAccountEntries) {
				...
				//No charges show in the Account module when using PayPlanVersions NoCharges.
				//Ensure that patnum is set to guarantor whom is responsible for making payments on the plan.
				//PaySplits made from this FauxAccountEntry need to use the guarantor unless using PayPlanVersion No Charge.
				if(PrefC.GetEnum<PayPlanVersions>(PrefName.PayPlansVersion)!=PayPlanVersions.NoCharges) {
					entry.PatNum=entry.Guarantor;
				}
			}
