...
namespace OpenDentBusiness {
	public class PayPlanEdit {
		...
		public static List<PayPlanCharge> GetCloseoutChargesForDynamicPaymentPlan(PayPlan payPlan,Family fam,ref List<PayPlanCharge> listPayPlanCharges,ref List<PayPlanLink> listPayPlanLinks,ref List<PayPlanProductionEntry> listPayPlanProductionEntries) {
			//no remoting role check; no call to db
			...
			List<long> listProcNumsForChargesInDb=listPayPlanCharges.FindAll(x => x.LinkType==PayPlanLinkType.Procedure).Select(x => x.FKey).ToList();
			List<long> listAdjNumsForChargesInDb=listPayPlanCharges.FindAll(x => x.LinkType==PayPlanLinkType.Adjustment).Select(x => x.FKey).ToList();
			List<PayPlanProductionEntry> listProductionEntries=PayPlanProductionEntry.GetWithAmountRemaining(listPayPlanLinks,listPayPlanCharges);
			foreach(PayPlanProductionEntry entry in listProductionEntries) {
				...
				if(entry.LinkType==PayPlanLinkType.Adjustment && !ListTools.In(entry.PriKey,listAdjNumsForChargesInDb)) {
					listPayPlanLinks.RemoveAll(x => x.LinkType==PayPlanLinkType.Adjustment && x.FKey==entry.PriKey);//Sync will delete when saving.
					listPayPlanProductionEntries.RemoveAll(x => x.LinkType==PayPlanLinkType.Adjustment && x.PriKey==entry.PriKey);
					isDetatchedFromPlan=true;
				}
				if(entry.LinkType==PayPlanLinkType.Procedure && !ListTools.In(entry.PriKey,listProcNumsForChargesInDb)) {
					listPayPlanLinks.RemoveAll(x => x.LinkType==PayPlanLinkType.Procedure && x.FKey==entry.PriKey);//Sync will delete when saving.
					listPayPlanProductionEntries.RemoveAll(x => x.LinkType==PayPlanLinkType.Procedure && x.PriKey==entry.PriKey);
					isDetatchedFromPlan=true;
				}
				...
				PayPlanCharge charge=PayPlanEdit.CreateDebitChargeDynamic(payPlan,fam,entry.ProvNum,entry.ClinicNum,(double)entry.AmountRemaining,0
					,DateTime.Today,Lans.g("PayPlanEdit","Close Out Charge"),entry.PriKey,entry.LinkType);
				listChargesClosing.Add(charge);
			}
			return listChargesClosing;
		}
