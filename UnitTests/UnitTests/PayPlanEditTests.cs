...
namespace UnitTests.PayPlanEdit_Tests {
	...
	public class PayPlanEditTests:TestBase {
		...
		public void PayPlanEdit_CreateTransferForDynamicPaymentPlans_RebalanceNoUnknownSplits() {
			...
			results.ListAutoSplits.FirstOrDefault(x=>x.PayPlanDebitType==PayPlanDebitTypes.Interest).SplitAmt=30;
			PaySplits.InsertMany(results.ListAutoSplits);
		...
		public void PayPlanEdit_CreateTransferForDynamicPaymentPlans_RebalanceSomeUnknownSplits() {
			...
			Payment payment=PaymentT.MakePaymentNoSplits(pat.PatNum,periodPayAmount,payDate:DateTime.Today);
			PaymentEdit.AutoSplit results=PaymentEdit.AutoSplitForPayment(pat.PatNum,payment,payPlanNum:payPlan.PayPlanNum);
			results.ListAutoSplits.ForEach(x=>x.PayPlanDebitType=PayPlanDebitTypes.Unknown);
			PaySplits.InsertMany(results.ListAutoSplits);
			...
			results.ListAutoSplits.FirstOrDefault(x=>x.PayPlanDebitType==PayPlanDebitTypes.Interest).SplitAmt=6;
			PaySplits.InsertMany(results.ListAutoSplits);
		...
		public void PayPlanEdit_CreateTransferForDynamicPaymentPlans_RebalanceSomeUnknownSplitsNeedsNewCharge() {
			...
			payment=PaymentT.MakePaymentNoSplits(pat.PatNum,periodPayAmount,payDate:DateTime.Today);
			results=PaymentEdit.AutoSplitForPayment(pat.PatNum,payment,payPlanNum:payPlan.PayPlanNum);
			results.ListAutoSplits.FirstOrDefault(x=>x.PayPlanDebitType==PayPlanDebitTypes.Interest).SplitAmt=35;
			PaySplits.InsertMany(results.ListAutoSplits);
		...
		public void PayPlanEdit_ApplyPrepaymentToDynamicPaymentPlan_PreviousPrincipalPaid() {
			...
			List<Adjustment> listAdj=new List<Adjustment>();
			recalcData.Pat=pat;
			recalcData.PayPlan=PayPlanT.CreateDynamicPaymentPlan(recalcData.Pat.PatNum,recalcData.Pat.Guarantor,DateTime.Today,downPayment,APR,periodPayAmount,listProcs,listAdj,frequency:PayPlanFrequency.Monthly);
			recalcData.ListPayPlanLinks=PayPlanLinks.GetListForPayplan(recalcData.PayPlan.PayPlanNum);
			recalcData.Terms=PayPlanEdit.GetPayPlanTerms(recalcData.PayPlan,recalcData.ListPayPlanLinks);
			recalcData.ListPayPlanCharges=PayPlanCharges.GetForPayPlan(recalcData.PayPlan.PayPlanNum);
			//Make Payment
			Payment payment=PaymentT.MakePaymentNoSplits(pat.PatNum,30,payDate:DateTime.Today);
			PaymentEdit.AutoSplit results=PaymentEdit.AutoSplitForPayment(new List<long>() { pat.PatNum },pat.PatNum,new List<PaySplit>(),
				payment,new List<AccountEntry>(),false,false,null);
			results.ListAutoSplits.First(x=>x.PayPlanDebitType==PayPlanDebitTypes.Interest).SplitAmt=10; //overpays by $9
			PaySplits.InsertMany(results.ListAutoSplits);
			recalcData.ListPaySplits=PaySplits.GetForPayPlans(new List<long>{ recalcData.PayPlan.PayPlanNum });
			//Get Production Entries
			recalcData.ListProductionEntry=PayPlanProductionEntry.GetWithAmountRemaining(recalcData.ListPayPlanLinks,recalcData.ListPayPlanCharges);
			recalcData.ListProductionEntry=PayPlanT.FilterProductionEntryForDPP(recalcData.PayPlan,recalcData.ListPayPlanLinks,recalcData.ListProductionEntry);
			recalcData.PayPlan.CompletedAmt=PayPlanProductionEntry.GetDynamicPayPlanCompletedAmount(recalcData.PayPlan,recalcData.ListProductionEntry);
		...
		public void PayPlanEdit_ApplyPrepaymentToDynamicPaymentPlan_PreviousPrincipalUnpaid() {
			...
			List<Adjustment> listAdj=new List<Adjustment>();
			recalcData.Pat=pat;
			recalcData.PayPlan=PayPlanT.CreateDynamicPaymentPlan(recalcData.Pat.PatNum,recalcData.Pat.Guarantor,DateTime.Today.AddMonths(-1),downPayment,APR,periodPayAmount,listProcs,listAdj,frequency:PayPlanFrequency.Monthly,runService:false);
			recalcData.ListPayPlanLinks=PayPlanLinks.GetListForPayplan(recalcData.PayPlan.PayPlanNum);
			recalcData.Terms=PayPlanEdit.GetPayPlanTerms(recalcData.PayPlan,recalcData.ListPayPlanLinks);
			recalcData.ListPayPlanCharges=PayPlanCharges.GetForPayPlan(recalcData.PayPlan.PayPlanNum);
			//Make Payment
			...
			PaymentEdit.AutoSplit results=PaymentEdit.AutoSplitForPayment(new List<long>() { pat.PatNum },pat.PatNum,new List<PaySplit>(),
				payment,new List<AccountEntry>(),false,false,null);
			results.ListAutoSplits.First(x=>x.PayPlanDebitType==PayPlanDebitTypes.Interest).SplitAmt=30; //overpays by $29
			results.ListAutoSplits.First(x=>x.PayPlanDebitType==PayPlanDebitTypes.Principal).SplitAmt=10; //underpays by $19
			PaySplits.InsertMany(results.ListAutoSplits);
			recalcData.ListPaySplits=PaySplits.GetForPayPlans(new List<long>{ recalcData.PayPlan.PayPlanNum });
			//Get Production Entries
			recalcData.ListProductionEntry=PayPlanProductionEntry.GetWithAmountRemaining(recalcData.ListPayPlanLinks,recalcData.ListPayPlanCharges);
		...
		public void PayPlanEdit_ApplyPrepaymentToDynamicPaymentPlan_PreviousPrincipalUnpaidOverPaidInterestAndPrincipal() {
			...
			PaymentEdit.AutoSplit results=PaymentEdit.AutoSplitForPayment(new List<long>() { pat.PatNum },pat.PatNum,new List<PaySplit>(),
				payment,new List<AccountEntry>(),false,false,null);
			results.ListAutoSplits.First(x=>x.PayPlanDebitType==PayPlanDebitTypes.Interest).SplitAmt=3; //overpays by $2
			results.ListAutoSplits.First(x=>x.PayPlanDebitType==PayPlanDebitTypes.Principal).SplitAmt=40; //overpays by $11
			PaySplits.InsertMany(results.ListAutoSplits);
			recalcData.ListPaySplits=PaySplits.GetForPayPlans(new List<long>{ recalcData.PayPlan.PayPlanNum });
			//Get Production Entries
			recalcData.ListProductionEntry=PayPlanProductionEntry.GetWithAmountRemaining(recalcData.ListPayPlanLinks,recalcData.ListPayPlanCharges);
		...
		public void PayPlanEdit_ApplyPrepaymentToDynamicPaymentPlan_TreatAsComplete_CloseOut() {
			...
			PaymentEdit.AutoSplit results=PaymentEdit.AutoSplitForPayment(new List<long>() { pat.PatNum },pat.PatNum,new List<PaySplit>(),
				payment,new List<AccountEntry>(),false,false,null);
			results.ListAutoSplits.First(x=>x.PayPlanDebitType==PayPlanDebitTypes.Interest).SplitAmt=40; //overpays by $38.00
			PaySplits.InsertMany(results.ListAutoSplits);
		...
		public void PayPlanEdit_ApplyPrepaymentToDynamicPaymentPlan_AwaitComplete_CloseOut() {
			...
			PaymentEdit.AutoSplit results=PaymentEdit.AutoSplitForPayment(new List<long>() { pat.PatNum },pat.PatNum,new List<PaySplit>(),
				payment,new List<AccountEntry>(),false,false,null);
			results.ListAutoSplits.First(x=>x.PayPlanDebitType==PayPlanDebitTypes.Interest).SplitAmt=40; //overpays by $39.00
			PaySplits.InsertMany(results.ListAutoSplits);
		...
		public void PayPlanEdit_IssueChargesDueForDynamicPaymentPlans_ClosedPlansAreSkipped() {
			...
			PaymentEdit.AutoSplit results=PaymentEdit.AutoSplitForPayment(new List<long>() { pat.PatNum },pat.PatNum,new List<PaySplit>(),
				payment,new List<AccountEntry>(),false,false,null);
			results.ListAutoSplits.First(x=>x.PayPlanDebitType==PayPlanDebitTypes.Principal).SplitAmt=39; //Overpays by $10.00
			results.ListAutoSplits.First(x=>x.PayPlanDebitType==PayPlanDebitTypes.Interest).SplitAmt=40; //overpays by $39.00
			PaySplits.InsertMany(results.ListAutoSplits);
		...
		public void PayPlanEdit_BalanceOverpaidChargesForDynamicPaymentPlans_PlanOverPaymentMovedToUnearned() {
			...
			PaymentEdit.AutoSplit results=PaymentEdit.AutoSplitForPayment(new List<long>() { pat.PatNum },pat.PatNum,new List<PaySplit>(),
				payment,new List<AccountEntry>(),false,false,null);
			results.ListAutoSplits.First(x=>x.PayPlanDebitType==PayPlanDebitTypes.Principal).SplitAmt=99; //Overpays by $70.00
			results.ListAutoSplits.First(x=>x.PayPlanDebitType==PayPlanDebitTypes.Interest).SplitAmt=40; //overpays by $39.00
			PaySplits.InsertMany(results.ListAutoSplits);
		...
		public void PayPlanEdit_CreateTransferForDynamicPaymentPlans_PlanOverPaymentMovedToUnearnedPrepayTPAllUnknownSplits() {
			...
			PaymentEdit.AutoSplit results=PaymentEdit.AutoSplitForPayment(new List<long>() { pat.PatNum },pat.PatNum,new List<PaySplit>(),
				payment,new List<AccountEntry>(),false,false,null);
			results.ListAutoSplits.First(x=>x.PayPlanDebitType==PayPlanDebitTypes.Principal).SplitAmt=99; //Overpays by $70.00
			results.ListAutoSplits.First(x=>x.PayPlanDebitType==PayPlanDebitTypes.Principal).PayPlanDebitType=PayPlanDebitTypes.Unknown;
			results.ListAutoSplits.First(x=>x.PayPlanDebitType==PayPlanDebitTypes.Interest).SplitAmt=40; //overpays by $39.00
			results.ListAutoSplits.First(x=>x.PayPlanDebitType==PayPlanDebitTypes.Interest).PayPlanDebitType=PayPlanDebitTypes.Unknown;
			PaySplits.InsertMany(results.ListAutoSplits);
		...
		public void PayPlanEdit_CreateTransferForDynamicPaymentPlans_PlanOverPaymentMovedToUnearnedMultipleIterations() {
			...
			PaymentEdit.AutoSplit results=PaymentEdit.AutoSplitForPayment(new List<long>() { pat.PatNum },pat.PatNum,new List<PaySplit>(),
				payment,new List<AccountEntry>(),false,false,null);
			results.ListAutoSplits.First(x=>x.PayPlanDebitType==PayPlanDebitTypes.Principal).SplitAmt=11; //Overpays by $2.00
			results.ListAutoSplits.First(x=>x.PayPlanDebitType==PayPlanDebitTypes.Interest).SplitAmt=3; //Overpays by $2.00
			PaySplits.InsertMany(results.ListAutoSplits);
			...
			results=PaymentEdit.AutoSplitForPayment(new List<long>() { pat.PatNum },pat.PatNum,new List<PaySplit>(),
				payment,new List<AccountEntry>(),false,false,null);
			results.ListAutoSplits.FirstOrDefault(x=>x.PayPlanDebitType==PayPlanDebitTypes.Principal).SplitAmt=87.91;
			PaySplits.InsertMany(results.ListAutoSplits);
			//Get recalculation data and rebalance on prepay.
			recalcData=PayPlanT.GetRecalculationData(dynamicPayPlanAwait,pat,fam);
		...
		public void PayPlanEdit_GetCloseoutChargesForDynamicPaymentPlan_CloseOut() {
			...
			Assert.IsTrue(!listPayPlanChargesAfterCloseout.Exists(x=>ListTools.In(x.ProcNum,listProcsExpectedToBeDetached.Select(y=>y.ProcNum).ToList())));
