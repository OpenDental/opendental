	...
	public class ProgramPropertiess:TestBase {
		...
		public void PDMP_GetPropertyFromSource_UseBusinessValueWithDownload() {
			...
			if(notHqProp!=null) {
				...
				notHqProp=new ProgramProperty() {
					ProgramNum=prog.ProgramNum,
					PropertyDesc=propDesc,
					PropertyValue=propVal
				};
				ProgramProperties.Insert(notHqProp);
			}
			ProgramProperties.RefreshCache();
			HqProgram.Download();
			string actualValue=ProgramProperties.GetFirstOrDefault(x=>x.ProgramNum==prog.ProgramNum && x.PropertyDesc==propDesc).PropertyValue;
		...
		public void PDMP_GetPropertyFromSource_UseHQ() {
			...
			ProgramPropertyT.CreateProgramProperty(Programs.GetProgramNum(ProgramName.PDMP),propDesc,0);
			ProgramProperty propertyFromCache=ProgramProperties.GetPropForProgByDesc(Programs.GetCur(ProgramName.PDMP).ProgramNum,propDesc);
