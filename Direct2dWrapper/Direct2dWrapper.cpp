...
Direct2DWrapper::Direct2DWrapper() :
	m_ID2D1Factory1(NULL),
	m_ID2D1BitmapRenderTarget(NULL),
	m_ID2D1DCRenderTarget(NULL),
	m_width(0),
	m_height(0),
	m_hdc(0),
	//Device independent
	m_ID2D1DrawingStateBlock(NULL),
...
Direct2DWrapper::~Direct2DWrapper(){
	m_ID2D1BitmapRenderTarget.Reset();//redundant
	m_ID2D1DCRenderTarget.Reset();//redundant
	m_ID2D1Factory1.Reset();
	//Device independent
	m_ID2D1DrawingStateBlock.Reset();
...
void Direct2DWrapper::CreateGradientBrush(UINT32 brushNum, UINT32 color1, UINT32 color2, FLOAT x1, FLOAT y1, FLOAT x2, FLOAT y2) {
	...
	m_ID2D1BitmapRenderTarget->CreateGradientStopCollection(
		gradientStops,
		2,
		D2D1_GAMMA_2_2,
	...
	m_ID2D1BitmapRenderTarget->CreateLinearGradientBrush(
...
HRESULT Direct2DWrapper::BeginDraw(UINT32 width,UINT32 height,HDC hdc){
	...
	m_height=height;
	m_hdc=hdc;
	if(!m_ID2D1DCRenderTarget){//render target does not exist
		...
		hr=m_ID2D1Factory1->CreateDCRenderTarget(
			&renderTargetProperties,
			m_ID2D1DCRenderTarget.ReleaseAndGetAddressOf());
		if(FAILED(hr)) return hr;
		m_ID2D1DCRenderTarget->SetAntialiasMode(D2D1_ANTIALIAS_MODE_PER_PRIMITIVE);//already default
		RECT rect=Rect<int>(0,0,width,height);
		hr=m_ID2D1DCRenderTarget->BindDC(hdc, &rect);
		if(FAILED(hr)) return hr;
		hr=m_ID2D1DCRenderTarget->CreateCompatibleRenderTarget(m_ID2D1BitmapRenderTarget.ReleaseAndGetAddressOf());
		if(FAILED(hr)) return hr;
		//D2D1_DRAWING_STATE_DESCRIPTION drawingState=DrawingStateDescription();
		m_ID2D1Factory1->CreateDrawingStateBlock(m_ID2D1DrawingStateBlock.ReleaseAndGetAddressOf());
		m_ID2D1Factory1->CreateDrawingStateBlock(m_ID2D1DrawingStateBlock1.ReleaseAndGetAddressOf());
	}
	m_ID2D1BitmapRenderTarget->BeginDraw();
	return S_OK;
}

HRESULT Direct2DWrapper::EndDraw(){
	HRESULT hr=S_OK;
	hr=m_ID2D1BitmapRenderTarget->EndDraw();
	ComPtr<ID2D1Bitmap> bitmap;
	m_ID2D1BitmapRenderTarget->GetBitmap(bitmap.GetAddressOf());
	RECT rect=Rect<int>(0,0,m_width,m_height);
	m_ID2D1DCRenderTarget->BindDC(m_hdc,&rect);
	m_ID2D1DCRenderTarget->BeginDraw();
	m_ID2D1DCRenderTarget->DrawBitmap(bitmap.Get());
	hr=m_ID2D1DCRenderTarget->EndDraw();
	if(hr==D2DERR_RECREATE_TARGET){//for bitmaps, we're only drawing once, so we don't care about this
		m_ID2D1DCRenderTarget.Reset();
		m_ID2D1BitmapRenderTarget.Reset();
	}
	return hr;
}
...
void Direct2DWrapper::Clear(UINT32 color){
	m_ID2D1BitmapRenderTarget->Clear(ColorF(color));
}

void Direct2DWrapper::DrawEllipse(UINT32 color,FLOAT cx,FLOAT cy,FLOAT rx,FLOAT ry,FLOAT strokeWidth){
	m_ID2D1BitmapRenderTarget->CreateSolidColorBrush(ColorF(color),m_brush.ReleaseAndGetAddressOf());
	m_ID2D1BitmapRenderTarget->DrawEllipse(Ellipse(Point2F(cx,cy),rx,ry),m_brush.Get(),strokeWidth);
}

void Direct2DWrapper::DrawLine(UINT32 color,FLOAT x1,FLOAT y1,FLOAT x2,FLOAT y2,FLOAT strokeWidth){
	m_ID2D1BitmapRenderTarget->CreateSolidColorBrush(ColorF(color),m_brush.ReleaseAndGetAddressOf());
	m_ID2D1BitmapRenderTarget->DrawLine(Point2F(x1,y1),Point2F(x2,y2),m_brush.Get(),strokeWidth);
}

void Direct2DWrapper::DrawRectangle(UINT32 color,FLOAT x,FLOAT y,FLOAT width,FLOAT height,FLOAT strokeWidth){
	m_ID2D1BitmapRenderTarget->CreateSolidColorBrush(ColorF(color),m_brush.ReleaseAndGetAddressOf());
	m_ID2D1BitmapRenderTarget->DrawRectangle(RectF(x,y,x+width,y+height),m_brush.Get(),strokeWidth);
}
...
void Direct2DWrapper::EndPath(bool isFilled,bool isOutline,int colorFill,int colorOutline,float strokeWidth){
	...
	m_ID2D1GeometrySink.Reset();
	if(isFilled){
		m_ID2D1BitmapRenderTarget->CreateSolidColorBrush(ColorF(colorFill),m_brush.ReleaseAndGetAddressOf());
		m_ID2D1BitmapRenderTarget->FillGeometry(m_ID2D1PathGeometry.Get(),m_brush.Get());
	}
	if(isOutline){
		m_ID2D1BitmapRenderTarget->CreateSolidColorBrush(ColorF(colorOutline),m_brush.ReleaseAndGetAddressOf());
		...
		m_ID2D1BitmapRenderTarget->DrawGeometry(m_ID2D1PathGeometry.Get(),m_brush.Get(),strokeWidth);//,pID2D1StrokeStyle.Get());
...
void Direct2DWrapper::FillEllipse(UINT32 color,FLOAT cx,FLOAT cy,FLOAT rx,FLOAT ry){
	m_ID2D1BitmapRenderTarget->CreateSolidColorBrush(ColorF(color),m_brush.ReleaseAndGetAddressOf());
	m_ID2D1BitmapRenderTarget->FillEllipse(Ellipse(Point2F(cx,cy),rx,ry),m_brush.Get());
}

void Direct2DWrapper::FillRectangle(UINT32 color,FLOAT x,FLOAT y,FLOAT width,FLOAT height){
	m_ID2D1BitmapRenderTarget->CreateSolidColorBrush(ColorF(color),m_brush.ReleaseAndGetAddressOf());
	m_ID2D1BitmapRenderTarget->FillRectangle(RectF(x,y,x+width,y+height),m_brush.Get());
}

void Direct2DWrapper::FillRectangleGradient(int gradientNum, FLOAT x, FLOAT y, FLOAT width, FLOAT height) {
	if (gradientNum == 0) {
		m_ID2D1BitmapRenderTarget->FillRectangle(RectF(x, y, x + width, y + height), m_ID2D1LinearGradientBrush0.Get());
	}
...
void Direct2DWrapper::FillRoundedRectangle(UINT32 color, FLOAT x, FLOAT y, FLOAT width, FLOAT height,FLOAT radiusX) {
	...
	m_ID2D1BitmapRenderTarget->FillRoundedRectangle(roundedRect,m_brush.Get());
}

void Direct2DWrapper::RestoreDrawingState(int levelNum){
	if(levelNum==0){
		m_ID2D1BitmapRenderTarget->RestoreDrawingState(m_ID2D1DrawingStateBlock.Get());
	}
	if(levelNum==1){
		m_ID2D1BitmapRenderTarget->RestoreDrawingState(m_ID2D1DrawingStateBlock1.Get());
	}
}

void Direct2DWrapper::Rotate(float angle){
	D2D1_MATRIX_3X2_F matrixOld;
	m_ID2D1BitmapRenderTarget->GetTransform(&matrixOld);
	D2D1_MATRIX_3X2_F matrixRotate=D2D1::Matrix3x2F::Rotation(angle);
	m_ID2D1BitmapRenderTarget->SetTransform(matrixRotate * matrixOld);
}

void Direct2DWrapper::SaveDrawingState(int levelNum){
	if(levelNum==0){
		m_ID2D1BitmapRenderTarget->SaveDrawingState(m_ID2D1DrawingStateBlock.Get());
	}
	if(levelNum==1){
		m_ID2D1BitmapRenderTarget->SaveDrawingState(m_ID2D1DrawingStateBlock1.Get());
	}
}
...
void Direct2DWrapper::Scale(float scale){
	...
	m_ID2D1BitmapRenderTarget->SetTransform(matrixScale * matrixOld); 
}
...
void Direct2DWrapper::Translate(float x,float y){
	...
	m_ID2D1BitmapRenderTarget->SetTransform(matrixTranslation * matrixOld);
}
